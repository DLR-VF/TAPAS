...
public class SimulationServerData {
    ...
    private final Map<ServerInfo, String> serverprocessdefaultinfo = new HashMap<ServerInfo, String>() {
        {
     ...
     * @throws SQLException         This exception is thrown if anything with the ResultSet is
    ...
    public String getFormattedTimestamp() {
        if (timestamp != null) return SDF.format(timestamp);
        return "";
    }
    ...
    public Map<ServerInfo, String> getServerProcessInfo() {
        if (this.serverprocessinfo == null || this.shuttingdown || !this.online) return this.serverprocessdefaultinfo;
        else return this.serverprocessinfo;

    }

    public void setServerProcessInfo(ResultSet serverinfo) throws SQLException {

        if (this.serverprocessinfo == null) this.serverprocessinfo = new HashMap<>();
        ...
        switch (hashstatus) {
            case -1:
                this.hashstatus = HashStatus.INVALID;
                this.hashstring = "";
                break;
            case 0:
                this.hashstatus = HashStatus.NON_DOMINANT;
                this.hashstring = serverinfo.getString("sha512");
                break;
            case 1:
                this.hashstatus = HashStatus.DOMINANT;
                this.hashstring = serverinfo.getString("sha512");
                break;
            case 2:
                this.hashstatus = HashStatus.DEBUG;
                this.hashstring = serverinfo.getString("sha512");
                break;
            default:
                this.hashstatus = HashStatus.UNKNOWN;
    ...
    public boolean setName(String name, Connection connection, String db_table_servers) {
        try {
            if (1 == connection.createStatement().executeUpdate(
                    "UPDATE " + db_table_servers + " SET server_name = '" + name + "' WHERE server_ip = '" +
                            ip.getHostAddress() + "'")) {
                this.name = name;
            } else {
                return false;
    ...
    public void update(ResultSet rs) throws SQLException {
        ...
        if (this.serverprocessinfo != null) serverprocessinfo.put(ServerInfo.SHUTDOWN,
                Boolean.toString(remote_control));
    ...
    public enum HashStatus {
        ...
        UNKNOWN(-2), INVALID(-1), NON_DOMINANT(0), DOMINANT(1), DEBUG(2);

        private final int status;

        HashStatus(int status) {
            this.status = status;
        }
    ...
    public enum ServerInfo {
        ...
        IDENTIFIER, RUNSINCE, PID, WORKON, SHUTDOWN
    }
