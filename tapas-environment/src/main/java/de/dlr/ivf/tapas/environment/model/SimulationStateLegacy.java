 ...
 * @author mark_ma
 */
public enum SimulationStateLegacy {
    /**
     ...
     */
    SimulationStateLegacy(int value) {
        this.value = value;
    }
     ...
     */
    public static SimulationStateLegacy getState(SimulationData simulation) {
        if (simulation.isFinished()) {
            if (simulation.getProgress() == simulation.getTotal()) return SimulationStateLegacy.FINISHED;
            return SimulationStateLegacy.STOPPED;
        } else if (simulation.isStarted()) {
            return SimulationStateLegacy.STARTED;
        } else if (simulation.isReady()) {
            return SimulationStateLegacy.STOPPED;
        }
        return SimulationStateLegacy.INSERTED;
    }
}
