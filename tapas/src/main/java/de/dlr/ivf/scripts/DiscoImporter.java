...
//        this.dbCon.execute(query, this);
        ...
        query = "CREATE TABLE " + tableBasename + tablename_households + "(" + "	hhid integer," +
                "   hh_gew double precision," + "   polgk integer," + "   male_fs integer," + "   alt integer," +
                "   size1 integer," + "   fuel1 integer," + "	vmt1 integer," + "   size2 integer," +
                "   fuel2 integer," + "   vmt2 integer," + "   value1 double precision," +
                "   cost1 double precision," + "   tax1 double precision," + "   value2 double precision," +
                "   cost2 double precision," + "   tax2 double precision," + "   ewqkm integer," +
                "   n_cars integer," + "   large integer," + "   erwerb double precision," + "   p double precision," +
                "   income double precision," + "   fs integer," + "   hhsize integer," + "   CONSTRAINT " + basename +
                tablename_households + "_pkey PRIMARY KEY (hhid, alt)" + ") WITH (  OIDS = FALSE);";
...
//        this.dbCon.execute(query, this);
        ...
        query = "CREATE TABLE " + tableBasename + tablename_cars + "(" + "	car_id integer," + "   size integer," +
                "   fuel integer," + "	vmt integer," + "   value double precision," + "   cost double precision," +
                "   tax double precision," + "   CONSTRAINT " + basename + tablename_cars +
                "_pkey PRIMARY KEY (car_id)" + ") WITH (  OIDS = FALSE);";
...
//        this.dbCon.execute(query, this);
        ...
        query = "CREATE TABLE " + tableBasename + tablename_alternatives + "(" + "	alt integer," +
                "   num_car integer," + "   car_ids integer[]," + "   CONSTRAINT " + basename + tablename_alternatives +
                "_pkey PRIMARY KEY (alt)" + ") WITH (  OIDS = FALSE);";
...
//            ResultSet rs, rs2;
//            int i = 0;
//            query = "SELECT DISTINCT value1, tax1, cost1, size1, fuel1, vmt1 FROM " + tableName + tablename_households +
//                    " ORDER BY size1, fuel1, vmt1, cost1, value1, tax1";
//            rs = this.dbCon.executeQuery(query, this);
//            while (rs.next()) {
//                //see if we have this car
//                query = "SELECT car_id FROM " + tableName + this.tablename_cars + " WHERE " + "value	=" +
//                        rs.getDouble("value1") + " AND " + "tax  	=" + rs.getDouble("tax1") + " AND " + "cost	=" +
//                        rs.getDouble("cost1") + " AND " + "size	=" + rs.getInt("size1") + " AND " + "fuel	=" +
//                        rs.getInt("fuel1") + " AND " + "vmt	=" + rs.getInt("vmt1");
...
//                query = "INSERT INTO " + tableName + this.tablename_cars + " VALUES( " + i + ", " + rs.getInt("size1") +
//                        ", " + +rs.getInt("fuel1") + ", " + +rs.getInt("vmt1") + ", " + +rs.getDouble("value1") + ", " +
//                        +rs.getDouble("cost1") + ", " + +rs.getDouble("tax1") + ")";
//                this.dbCon.execute(query, this);
//                i++;
//            }
//            rs.close();
//            //now the second car
//            query = "SELECT DISTINCT value2, tax2, cost2, size2, fuel2, vmt2 FROM " + tableName + tablename_households +
//                    " ORDER BY size2, fuel2, vmt2, cost2, value2, tax2";
//            rs = this.dbCon.executeQuery(query, this);
//            while (rs.next()) {
//                //see if we have this car
//                query = "SELECT car_id FROM " + tableName + this.tablename_cars + " WHERE " + "value	=" +
//                        rs.getDouble("value2") + " AND " + "tax  	=" + rs.getDouble("tax2") + " AND " + "cost	=" +
//                        rs.getDouble("cost2") + " AND " + "size	=" + rs.getInt("size2") + " AND " + "fuel	=" +
//                        rs.getInt("fuel2") + " AND " + "vmt	=" + rs.getInt("vmt2");
//                rs2 = this.dbCon.executeQuery(query, this);
//                if (rs2.next()) { //we have it!
//                    rs2.close();
//                    continue; // next element
...
//                rs2.close();
//                //new car: insert into db
//                query = "INSERT INTO " + tableName + this.tablename_cars + " VALUES( " + i + ", " + +rs.getInt(
//                        "size2") + ", " + +rs.getInt("fuel2") + ", " + +rs.getInt("vmt2") + ", " + +rs.getDouble(
//                        "value2") + ", " + +rs.getDouble("cost2") + ", " + +rs.getDouble("tax2") + ")";
//                this.dbCon.execute(query, this);
//                i++;
//            }
//            rs.close();
...
//            int fs;
//            int hh_size;
//            PreparedStatement st = dbCon.getConnection(this).prepareStatement(
//                    "INSERT INTO " + tableName + " VALUES (" + "?,?,?,?,?," + "?,?,?,?,?," + "?,?,?,?,?," +
//                            "?,?,?,?,?," + "?,?,?,?,? )");
//
//            fr.readLine();
//            globalCounter++;//header
//
//            counter = 0;
//
...
//
//                i = 0;
//                st.setInt(i + 1, Integer.parseInt(tokens[i]));
//                i++;        //hhid
//                st.setDouble(i + 1, Double.parseDouble(tokens[i]));
//                i++;    //hh_gew
//                st.setInt(i + 1, Integer.parseInt(tokens[i]));
//                i++;        //polgk
//                st.setInt(i + 1, fs);
...
//                        if (check[j] == PreparedStatement.EXECUTE_FAILED) {
//                            System.err.println(
//                                    "Error in statement for line number " + (globalCounter - check.length + j));
//                        }
//                    }
//                    counter = 0;
//                }
...
//            query = "UPDATE " + tableName + tablename_households + " AS hh SET car1_id= " + "	(SELECT car_id FROM " +
//                    tableName + this.tablename_cars + " AS car WHERE " + "car.value	= hh.value1 AND " +
//                    "car.tax  	= hh.tax1 AND " + "car.cost	= hh.cost1 AND " + "car.size	= hh.size1 AND " +
//                    "car.fuel	= hh.fuel1 AND " + "car.vmt	= hh.vmt1)";
//            this.dbCon.execute(query, this);
//
//            //now update the column car2
//            query = "UPDATE " + tableName + tablename_households + " AS hh SET car2_id= " + "	(SELECT car_id FROM " +
//                    tableName + this.tablename_cars + " AS car WHERE " + "car.value	= hh.value2 AND " +
//                    "car.tax  	= hh.tax2 AND " + "car.cost	= hh.cost2 AND " + "car.size	= hh.size2 AND " +
//                    "car.fuel	= hh.fuel2 AND " + "car.vmt	= hh.vmt2)";
//            this.dbCon.execute(query, this);
//
//            //now fill the alternative-table
//            query = "SELECT DISTINCT alt, n_cars, car1_id, car2_id FROM  " + tableName + tablename_households +
//                    " ORDER BY alt, n_cars, car1_id, car2_id";
...
//                //prepare statement
//                query = "INSERT INTO " + tableName + tablename_alternatives + " VALUES (" + rs.getInt("alt") + ", " +
//                        numCar + ", ";
//                //construct car array
//                switch (numCar) {
//                    case 0:
