    ...
    public void fixMissing() {
        ...
        for (i = 0; i < size; ++i) {
            for (j = 0; j < size; ++j) {
                ...
                if (i != j && this.matrixTT[i][j] == TPS_Mode.VISUM_NO_CONNECTION &&
                        this.matrixTT[j][i] != TPS_Mode.VISUM_NO_CONNECTION) {
                    this.matrixTT[i][j] = this.matrixTT[j][i];
                }

                if (i != j && this.matrixDist[i][j] == TPS_Mode.VISUM_NO_CONNECTION &&
                        this.matrixDist[j][i] != TPS_Mode.VISUM_NO_CONNECTION) {
                    this.matrixDist[i][j] = this.matrixDist[j][i];
                }

                if (i != j && this.matrixAcc[i][j] == TPS_Mode.VISUM_NO_CONNECTION &&
                        this.matrixEgr[j][i] != TPS_Mode.VISUM_NO_CONNECTION) {
                    this.matrixAcc[i][j] = this.matrixEgr[j][i];
                }

                if (i != j && this.matrixEgr[i][j] == TPS_Mode.VISUM_NO_CONNECTION &&
                        this.matrixAcc[j][i] != TPS_Mode.VISUM_NO_CONNECTION) {
