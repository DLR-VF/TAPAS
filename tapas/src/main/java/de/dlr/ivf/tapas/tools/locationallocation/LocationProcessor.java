...
public class LocationProcessor {
    ...
    /**
     * @param args 0: filename for input, 1: filename for output, 2: mode
        ...
        try {
            ...
            while ((line = input.readLine()) != null) {
                ...
                if (tmp.isCorrectInitialized()) {
                    ...
                    if (tmp.getValue("Loc_ID") != null) maxID = Math.max(maxID,
                            Integer.parseInt(tmp.getValue("Loc_ID")));
...
//			outRelocated.close();
            ...
            for (Location loc : worker.getLocationsForAddressProcessing()) {
                ...
                if (processor.processStreet(streetValue)) {
                    ...
                    if (processor.getStreetNumber() != null && loc.getValue("Hausnummer") == null) loc.addValue(
                            "Hausnummer", processor.getStreetNumber());
                    if (processor.getZipCode() != null && loc.getValue("PLZ") == null) loc.addValue("PLZ",
            ...
            if (args.length == 3) {
                ...
                for (int i = 0; i < worker.getLocationsForRelocation().size(); ++i) {
                    ...
                    if (counter % 100 == 0 || counter == worker.getLocationsForRelocation().size()) {
                        System.out.println(
                                "Processed " + counter + " from " + worker.getLocationsForRelocation().size() +
                                        " locations");
                    }
     ...
     * filter list
     *
     * @param entry the location, which holds the data
     */
    ...
    public boolean hasDuplicate(Location loc, int startSearchAt) {
        ...
        if (!Constants.ENABLE_CHANGES_OF_MARCO) {
            if (loc.getValue("Typ der Gelegenheit").equals("Gesundheit")) return false;
            else if (loc.getValue("Quelle") != null && loc.getValue("Quelle").equals("Klicktel")) // a
                // lot
                // of
                // services
                // share
        ...
        for (int i = startSearchAt; i < this.getLocationsForRelocation().size(); ++i) {
            ...
            if (locRef.isDuplicateImport(loc)) return true;
        }
        return false;
    }
    ...
    public void loadHeaders(String headerString) {
        ...
        while (tok.hasMoreTokens()) {
            ...
            if (header.equals("Loc_ID")) posOfIdColumn = i;
    ...
    public String locationToString(Location loc) {
        ...
        for (int i = 0; i < this.headerMap.size(); ++i) {
            ...
            if (i == this.headerMap.size() - 1) separator = "";
    ...
    public class Location {
        ...
        public Location(String entry) {
            ...
            if (tok.countTokens() != LocationProcessor.this.headerMap.size()) {
                System.out.println("Error: Found " + tok.countTokens() + " tokens. Expected: " +
                        LocationProcessor.this.headerMap.size() + " tokens.");
                while (tok.hasMoreTokens()) {
         ...
         *
         * @param valID The column in the csv-file.
        ...
        /**
         * @param isUpdatedByDB true if the values set are inserted by a row from the
         *                      database
         */
        ...
        public String locationToString() {
            ...
            for (int i = 0; i < LocationProcessor.this.headerMap.size(); ++i) {
                // last entry should not finish with a separator
                if (i == LocationProcessor.this.headerMap.size() - 1) separator = "";
