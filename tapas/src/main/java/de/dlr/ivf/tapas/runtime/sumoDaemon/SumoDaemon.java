...
public class SumoDaemon extends Thread {
                ...
                if (updateStatement) {
                    ...
                    if (array instanceof Double[]) {
                        ...
                        for (int j = 0; j < distribution.length; ++j) {
                            if (j < distribution.length - 1) query.append(distribution[j].toString()).append(",");
    ...
    private String getOutputPath() {
        return new File(this.parameterClass.getString(ParamString.FILE_WORKING_DIRECTORY) +
                System.getProperty("file.separator") + this.parameterClass.OUTPUT_DIR +
                this.parameterClass.getString(ParamString.DB_TABLE_TRIPS) + "_iter_" + this.actualIter)
        ...
        try {
            if (this.checkAndReestablishCon()) {
                ...
                if (!sumoStatusTable.equals("")) {
                    ...
                    if (rs.next()) {
                        if (rs.getInt("num") > 0) returnVal = rs.getInt("iter");
                    }
                    rs.close();
                }
            }
    ...
    private void resetSimulation(String sim_key) {
        ...
        try {
            if (this.checkAndReestablishCon()) {
                ...
                query = "UPDATE simulations SET sim_finished = FALSE, sim_progress = 0, timestamp_started =NULL, timestamp_finished =NULL WHERE sim_key ='" +
                        sim_key + "'";
        ...
        try {
            if (this.checkAndReestablishCon()) {
                ...
                for (String sim : possibleSims) {
                    int maxIter = (int) (Double.parseDouble(
                            this.manager.readSingleParameter(sim, "MAX_SUMO_ITERATION")) + 0.5);
                    if (maxIter > 0) {
                        int actTapasIter = (int) (Double.parseDouble(
                                this.manager.readSingleParameter(sim, "ITERATION")) + 0.5);
