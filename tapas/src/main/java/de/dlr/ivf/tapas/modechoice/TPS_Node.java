...
public class TPS_Node implements ExtendedWritable, Comparable<TPS_Node> {
     ...
     * direct child nodes
     */
    private final List<TPS_Node> children = new Vector<>();
    /**
     * node id
     */
     ...
     */
    private final TPS_Attribute splitVariable;
     ...
     * @param distribution    the mode distribution
     */
    public TPS_Node(int id, TPS_Attribute splitVariable, Collection<Integer> attributeValues, TPS_DiscreteDistribution<TPS_Mode> distribution, TPS_Node parent) {
        this.id = id;
        this.distribution = distribution;
        this.splitVariable = splitVariable;
        this.myAttValues = new ArrayList<>();
        this.parent = parent;
        if (attributeValues != null) this.myAttValues.addAll(attributeValues);
    }
