    ...
    public static Object[] checkArguments(String[] args, TPS_ArgumentType<?>... types) throws IllegalArgumentException {
        if (types.length > args.length) {
            throw new IllegalArgumentException(
                    "given arguments and necessary arguments have different sizes " + "-> |args|=" + args.length +
        ...
        for (int i = 0; i < array.length; i++) {
            if (args[i] == null || "null".equals(args[i])) {
                if (types[i].required) throw new IllegalArgumentException(
                        "Required argument " + types[i] + " not set at position " + i);
                ...
                if ("true".equalsIgnoreCase(args[i]) || "false".equalsIgnoreCase(args[i])) {
                    ...
                    throw new IllegalArgumentException(
                            "Value '" + args[i] + "' for argument " + types[i] + " at position " + i +
                ...
                if (!file.exists()) {
                    throw new IllegalArgumentException(
                            "File '" + file + "' for argument " + types[i] + " at position " + i + " does not exist");
                } else if (!file.canRead()) {
                    throw new IllegalArgumentException(
                            "Can't read from file '" + file + "' for argument " + types[i] + " at position " + i);
                }
                array[i] = file;
            } else {
                try {
                    @SuppressWarnings("rawtypes") EnumSet<?> enumSet = EnumSet.allOf((Class<Enum>) types[i].clazz);
                    ...
                    if (array[i] == null) {
                        ...
                        throw new RuntimeException(
                                "Enum value '" + args[i] + "' for argument " + types[i] + " at position " + i +
