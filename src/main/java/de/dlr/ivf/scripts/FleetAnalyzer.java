...
public class FleetAnalyzer extends TPS_BasicConnectionClass {
	...
	public void readTrips(){
		...
		try{
			...
			do{
				recordsFound=false;
				query = "SELECT trip.p_id, trip.hh_id, trip.start_time_min, trip.mode, trip.car_type, trip.distance_real_m as dist,  cars.kba_no, cars.engine_type FROM "+regionName+ "_trips_"+key+" as trip LEFT OUTER JOIN core."+regionName+"_cars as cars ON (trip.car_type=cars.car_id AND cars.car_key='"+fleet_key+"') WHERE trip.mode =2 ORDER BY hh_id, p_id, start_time_min LIMIT "+chunk+" OFFSET "+(step*chunk);				
				ResultSet rs = dbCon.executeQuery(query, this);
				step++;
				while(rs.next()){
					recordsFound=true;
	...
	public void printResults(){
		...
		for(Entry<EcoMoveCar, Double> entry: this.segmentDistances.entrySet()){
			...
			System.out.printf("%s %s %s\t%.3f\t%d\t%d\n", entry.getKey().size.toString(), entry.getKey().fuelType.toString(), entry.getKey().ecoCar?"ecoCar":"NonEco", entry.getValue()/1000.0, count,countCars);
		}
