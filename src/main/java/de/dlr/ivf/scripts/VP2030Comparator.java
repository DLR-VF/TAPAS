...
public class VP2030Comparator extends TPS_BasicConnectionClass {
	...
	public  List<VP2030Data>  loadTrafficVolumes(String table, boolean useOnlyDestination, boolean useOnlySource){
		...
		try{
			ResultSet rs;
			query = "SELECT quelle, ziel,"
					+ "bahn_ber, bahn_ausb, bahn_eink, bahn_gesch, bahn_url,"
					...
					+ "miv_all, luft_all, oespv_all, rad_all, fuss_all, filter_, rnb_raum_first "
					+ "FROM "+table;
			...
			while(rs.next()){
				...
				if(useOnlyDestination && !this.vp2taz.containsKey(e.ziel))
					continue;
				if(useOnlySource && !this.vp2taz.containsKey(e.quelle))
					continue;
	...
	public static void main(String[] args) {
		if(args.length !=4){
			System.out.println("Parameters: <region> <tablename> <boolean: use only destination> <boolean: use only source>");
			return;
		}
		...
		List<VP2030Data> cells = worker.loadTrafficVolumes(args[1], Boolean.parseBoolean(args[2]), Boolean.parseBoolean(args[3]));
