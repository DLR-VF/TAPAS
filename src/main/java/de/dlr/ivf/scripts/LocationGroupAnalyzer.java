...
public class LocationGroupAnalyzer extends TPS_BasicConnectionClass {
	...
	public void analyzeTrips(String key){
		...
		try{
			...
			do{
				recordsFound=false;
				query = "SELECT p_id, hh_id, start_time_min, mode, loc_id_start, loc_id_end FROM "+key+" ORDER BY hh_id, p_id, start_time_min LIMIT "+chunk+" OFFSET "+(step*chunk);				
				ResultSet rs = dbCon.executeQuery(query, this);
				step++;				
				while(rs.next()){
					recordsFound=true;
					...
					if(	pID==actPID &&
						hhID==actHHID &&
						locStart!=homeId){
						if(locations.get(locStart).equals(locations.get(locEnd)) && locations.get(locStart)!=null){ // is this the same location group?
							if(!enteringGroup){ //first enter
								...
								if(actMode!=betweenMode){
									System.out.println("Mode change: "+actMode+" expected: "+betweenMode+ " Loc start: "+locStart+ " Loc end: "+locEnd);
								}
								else{
									groupCount++;
								}
							...
							if(enteringGroup){ // did we enter a group?
								egrMode= actMode;
								String keyVal = accMode +"-"+ betweenMode +"-"+
                                        egrMode;
								if(groupStatistics.containsKey(keyVal))
									groupCount+=groupStatistics.get(keyVal);
			...
			System.err.println(this.getClass().getCanonicalName()+" analyzeTrips: SQL-Error during statement: "+query);
			e.printStackTrace();
		}	
	}
	...
	public void printStatistics(){
		...
		for(Entry<String, Integer> entries : this.groupStatistics.entrySet()){
			...
			if(entries.getKey().charAt(2)!='0')
				strangeModeSets +=entries.getValue();
			if(	(entries.getKey().charAt(0)=='1' && entries.getKey().charAt(4)!='1') ||
				(entries.getKey().charAt(4)=='1' && entries.getKey().charAt(0)!='1'))	
				illegalBikes += entries.getValue();
			if(	(entries.getKey().charAt(0)=='2' && entries.getKey().charAt(4)!='2') ||
				(entries.getKey().charAt(4)=='2' && entries.getKey().charAt(0)!='2'))
				illegalCars += entries.getValue();
		}
