...
public class SrVCalculator {
	...
	public SrVCalculator(TPS_DB_Connector connection) {
 	 ...
 	 */
	private HashMap<Integer, Integer> tazToStatisticArea = new HashMap<>();
	
	private HashMap<Integer, List<Integer>> statisticAreaToTAZ = new HashMap<>();
	
	private List<Integer> statisticArea = new ArrayList<>();
	...
	public void aggregateAndWriteCSVFile(String fileName){
		try {
			...
			String line;
			//the headder
			writer.append("von;nach;rw;rw_pt;beeline;cost_miv;cost_small;cost_medium;cost_large;cost_variable;cost_pt;rz_miv;rz_pt;rz_bike;rz_walk;pt_interchange;pt_init_waiting;pt_sum_waiting;pt_access;pt_egress;tazConnections\n");
			int i,j,anz;
			...
			for( i= 0; i<odArray[0].length; ++i){ //from
				...
				for( j= 0; j<odArray[0][i].length; ++j){ //to
					...
					int tazConnectors= this.statisticAreaToTAZ.get(from).size()*this.statisticAreaToTAZ.get(to).size();
...
//					}
	...
	public boolean loadTAZToArea(String tazTable){
		...
		String query = "SELECT taz_id, taz_statistical_area, st_X(taz_coordinate) as lat, st_Y(taz_coordinate) as lon FROM "+tazTable;
		try{
			...
			while (rs.next()) {
				...
				if(!this.statisticArea.contains(area))
					this.statisticArea.add(area);
			}
			rs.close();
			...
			query = "SELECT taz_id, taz_statistical_area, st_X(taz_coordinate) as lat, st_Y(taz_coordinate) as lon FROM "+tazTable;
	...
	public void readPTMatrix(String filename){
		...
		try {
			...
			int lineNr=1;
			input.readLine(); lineNr++;//header
			input.readLine(); lineNr++;//header
			int from, to;
			while (cleanedLine != null){
				cleanedLine = input.readLine();lineNr++;
				if(cleanedLine!=null){
	...
	public void readMatrixFromDB(String tableentry, double[][] matrix, double factor, String matrixTable){
		String querry="";
		try {
			querry = "SELECT matrix_values FROM "+matrixTable
					+ " WHERE matrix_name='" + tableentry + "'";
			ResultSet rs = this.dbConnection.executeQuery(querry, this);
			if (rs.next()) {
				int[] iArray;
					iArray = TPS_DB_IO.extractIntArray(rs, "matrix_values");
	...
	public boolean readTrips(String key){
		...
		try{
			...
			do{
				recordsFound=false;
				query = "SELECT p_id, taz_id_start, taz_id_end, loc_id_start, loc_id_end, mode, travel_time_sec, distance_real_m FROM public."+key+" ORDER BY hh_id, p_id, start_time_min LIMIT "+chunk+" OFFSET "+(step*chunk);				
				rs = this.dbConnection.executeQuery(query, this);
				step++;
				while(rs.next()){
					recordsFound=true;
					...
					counter++;
					if(counter %10000==0)
						System.out.println("Processed "+counter+" work trips");
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		if(args.length !=4){
			System.out.println("Usage: SrVCalculator <miv-travel time matrix> <dist matrix> <pt matrix prefix> <output>");
			return;
		}
		...
		if(worker.loadTAZToMapping("Statistisches Gebiet zu TVZ 1223", "core.berlin_taz_1223", "core.berlin_taz_mapping_values")){
			...
			System.out.println("Found "+worker.tazToStatisticArea.size()+" TAZ codes and "+worker.odArray[0].length+" statistical area codes." );
