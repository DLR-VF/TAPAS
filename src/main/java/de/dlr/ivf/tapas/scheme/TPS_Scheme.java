...
public class TPS_Scheme implements Iterable<TPS_SchemePart>, ExtendedWritable {
	...
	private class TourPartIterator implements Iterator<TPS_TourPart>, Iterable<TPS_TourPart> {
		 ...
		 */
		private Iterator<TPS_SchemePart> it;
	 ...
	 */
	private List<TPS_SchemePart> schemeParts;
	...
	public boolean addEpisode(TPS_Episode episode) {
		...
		if (episode.isHomePart && sp.isTourPart()) {
			if (sp.getLastEpisode().isStay()) {
				sp.getLastEpisode().setOriginalDuration(Math.min(this.parameterClass.getIntValue(ParamValue.SEC_TIME_SLOT), sp.getLastEpisode().getOriginalDuration()-this.parameterClass.getIntValue(ParamValue.SEC_TIME_SLOT))); //make the last stay 5 min shorter
				TPS_ActivityConstant activity = TPS_ActivityConstant.getActivityCodeByTypeAndCode(TPS_ActivityCodeType.ZBE, 80); // activity: moving
				TPS_Trip takeMeHome = new TPS_Trip(sp.getId(), activity, (int) sp.getLastEpisode().getOriginalEnd(),
						5, this.parameterClass); // insert a "take me home in 5 minutes"-trip
	...
	public TPS_SchemePart getSchemePart(TPS_Episode episode){
		...
		for(TPS_SchemePart parts: this.schemeParts){
			if(parts.episodes.indexOf(episode)>=0)
				return parts;
		}
