...
public class TPS_Trip extends TPS_Episode {
	 ...
	 *            id of the trip
	 * @param actCode
	 *            activity code of the trip, e.g. 80. Don't confuse with the tour number. The tour number is only stored
	 *            in the scheme.
	 * @param start
	 *            start time of the trip in seconds
	 * @param duration
	 *            original duration of the trip determined by the scheme in seconds
	 */
	public TPS_Trip(int id, TPS_ActivityConstant actCode, int start, int duration, TPS_ParameterClass parameterClass) {
		super(	id, 
				...
				start, 
				duration, 
				start - parameterClass.getDoubleValue(ParamValue.DELTA_START_EARLIER),
