...
public class TPS_Stay extends TPS_Episode implements Comparable<TPS_Stay> {
	 ...
	 *          ID of the stay
	 * @param actCode
	 *          activity code of the stay
	 * @param start
	 *          original start time of the stay in seconds
	 * @param duration
	 *          original duration of the stay in seconds
	 * @param startEarlier
	 * @param startLater
	 * @param durationMinus
	 * @param durationPlus
	...
	public TPS_Stay(int id, TPS_ActivityConstant actCode, int start, int duration, double startEarlier, double startLater,
                    double durationMinus, double durationPlus, TPS_ParameterClass parameterClass) {
		super(id, actCode, start, duration, startEarlier, startLater, durationMinus, durationPlus, parameterClass);
	...
	public int compareTo(TPS_Stay stay) {
		...
		if(deltaPriority==0){
			int deltaDuration = this.getOriginalDuration() - stay.getOriginalDuration() ; // same priority: longer duration -> first in list
			int deltaStart = stay.getOriginalStart() - this.getOriginalStart(); // same priority: lower Start Time -> first in list
			if(deltaDuration==0)
				return deltaStart;
			else
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.dlr.ivf.tapas.scheme.TPS_Episode#toString(java.lang.String)
	 */
	@Override
