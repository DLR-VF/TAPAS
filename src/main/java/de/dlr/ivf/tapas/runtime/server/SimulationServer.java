...
public class SimulationServer extends Thread{
	...
	private class SimulationServerUpdateTask extends TimerTask {
		 ...
		 * 
		 * @throws SQLException
		 *             This exception is thrown if there occurs an error inserting the server into the database
		 */
		public SimulationServerUpdateTask() throws SQLException {
			...
			if (!rs.next()) {
				query = "INSERT INTO " + sTable + " (server_ip, server_online, server_cores, server_name) VALUES("+ this.iAddr
		...
		public void run() {
			try {
				ResultSet rset = dbConnector.executeQuery("SELECT * FROM "+SimulationServer.this.getParameters().getString(ParamString.DB_TABLE_PROCESSES)+" WHERE host = '"+hostname+"' AND end_time IS NULL", this);

				if(rset.next()) 
					SimulationServer.shuttingDown = rset.getBoolean("shutdown");				
				rset.close();
				
				if(!SimulationServer.shuttingDown && !SimulationServer.shutdown){
					String query = "SELECT server_name FROM "+this.sTable+" WHERE server_name = '"+SimulationServer.hostname+"'";
					...
					if(rs!=null) { //sometimes the connection times out. It will be restablished on the next try...
						if(rs.next()) {
						  query = "UPDATE "+this.sTable+" SET server_online = TRUE, server_usage = "+usage.getCPUUsage()+", server_ip = "+this.iAddr+" WHERE server_name = '"+SimulationServer.hostname+"'";
						}else {
						  query = "INSERT INTO "+this.sTable+" (server_ip, server_online, server_cores, server_name) VALUES("+this.iAddr+", TRUE, "+this.procs+", '" + SimulationServer.hostname +"')";
						}
						SimulationServer.this.dbConnector.execute(query, this);
						rs.close();
					}
	 ...
	 * 
	 * @param args
	 *            args[0]: tapas network directory path
	 * 
	 * @throws Exception
	 *             this exception is thrown when the server couldn't be setup
	 * 
	 */
 		...
 		public void run() {
		  if(!SimulationServer.shutdown) {	
			...
			try {
				...
				for(int i = 0; i < 60; i++){ //quick fix to avoid endless looping in case TPS_Main is "hanging"; one could think of implementing a CountDownLatch
				...
				query = "UPDATE "+SimulationServer.this.getParameters().getString(ParamString.DB_TABLE_PROCESSES)+" SET end_time = '"+LocalDateTime.now().toString()+"', tapas_exit_ok = "+tapas_finished+" WHERE host = '"+SimulationServer.hostname+"' AND end_time IS NULL";
	 ...
	 *            IP address of the current network adapter
	 * @param tapasNetworkDirectory
	 *            tapas network directory path
	 * 
	 * @throws SQLException
	 *             This exception is thrown if a connection to the database could not be established.
	 * @throws ClassNotFoundException
	 *             This exception is thrown if the driver for the database was not found.
	 */
	private SimulationServer(InetAddress simulationServerIPAddress, File tapasNetworkDirectory) throws SQLException,
		...
		String query = "SELECT * FROM "+parameterClass.getString(ParamString.DB_TABLE_PROCESSES)+" WHERE host = '"+SimulationServer.hostname+"' AND end_time IS NULL";
		...
		query = "INSERT INTO "+parameterClass.getString(ParamString.DB_TABLE_PROCESSES)+" (identifier, server_ip, p_id, start_time, host, sim_key, sha512) VALUES ('"+processidentifier+"', "+ip+", "+processid+", '"+starttime+"', '"+SimulationServer.hostname+"', 'IDLE', '"+SimulationServer.hashcode+"')";

		this.dbConnector.execute(query, SimulationServer.this);
		
		query = "UPDATE "+parameterClass.getString(ParamString.DB_TABLE_SERVERS)+" SET server_boot_flag = false WHERE server_name = '"+SimulationServer.hostname+"'";
		this.dbConnector.execute(query,SimulationServer.this);
	}
	...
	public void run() {
		...
		try {
			...
			while(rs.next()){
				...
				if(rsGet.next()){
					if(rsGet.getInt(1)>0)
						System.out.println(sysoutPrefix+"Cleaning up crashed simulation "+sim_key+": Reset "+rsGet.getInt(1)+" households");
				}
				rsGet.close();
			}
			rs.close();
			...
			while (keepOn) {
				...
				if (rsGet.next())
					sim_key = rsGet.getString("sim_key");
...
//
					if(file!=null){
						...
						query = "UPDATE server_processes SET sim_key = '"+sim_key+"' WHERE host = '"+SimulationServer.hostname+"'";
						...
						if(main.getPersistenceManager().finish())
							while(SimulationServer.shuttingDown){
					...
					if(printMessage){
						...
						query = "UPDATE server_processes SET sim_key = 'IDLE' WHERE host = '"+SimulationServer.hostname+"'";
						SimulationServer.this.dbConnector.execute(query, this);
						printMessage=false;
					}
					try {
