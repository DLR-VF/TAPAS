...
public class QualityChartDataset extends AbstractXYDataset {
    ...
    public static final EnumSet<Key> DATASERIES = EnumSet.of(Key.DATA_HIGH, Key.DATA_MED, Key.DATA_LOW);
    /**
     *
     */
    private static final long serialVersionUID = 1105064554090557394L;
    private final EnumMap<Key, ArrayList<QualityChartData>> seriesCollection = new EnumMap<>(Key.class);
    private double max;

    public QualityChartDataset(ArrayList<QualityChartData> data, EnumSet<Key> seriesKeys) {
        ...
        for (QualityChartData d : data) {
            ...
            if (null != key) seriesCollection.get(key).add(d);

        }

        seriesCollection.get(Key.OPTIMAL).add(new QualityChartData(0, 0, ""));
        seriesCollection.get(Key.OPTIMAL).add(new QualityChartData(max, max, ""));
    ...
    public int getItemCount(int series) {
        ...
        throw new ArrayIndexOutOfBoundsException("Requested series does not exist.");
    }
    ...
    public String getLabel(int series, int item) {
        ...
        Key key = (Key) getSeriesKey(series);
        if (DATASERIES.contains(key)) return seriesCollection.get(key).get(item).getLabel();
        else return null;
    }
    ...
    /**
     * @param seriesKey must be of type {@link Key}.
     * @throws IllegalArgumentException if <code>seriesKey</code> is not of the right type.
     */
    @SuppressWarnings("rawtypes")
    @Override
    public int indexOf(Comparable seriesKey) {
        if (seriesKey instanceof Key) {
            ...
            return Arrays.binarySearch(a, (Key) seriesKey, new Comparator<Key>() {
                @Override
        ...
        throw new IllegalArgumentException("The key must be of class " + Key.class);
    }
    ...
    private double setConfidenceInterval(ArrayList<QualityChartData> data) {
        ...
        seriesCollection.get(Key.UPPER_CONFIDENCE).add(new QualityChartData(pwa, 0, ""));
        seriesCollection.get(Key.UPPER_CONFIDENCE).add(new QualityChartData(max, max - pwa, ""));

        seriesCollection.get(Key.LOWER_CONFIDENCE).add(new QualityChartData(0, pwa, ""));
        seriesCollection.get(Key.LOWER_CONFIDENCE).add(new QualityChartData(max - pwa, max, ""));
        return pwa;
    }
    ...
    private void setGEH5(ArrayList<QualityChartData> data) {
        for (QualityChartData d : data) {
            ...
            seriesCollection.get(Key.LOWER_GEH5).add(new QualityChartData(g5m, s, null));
            seriesCollection.get(Key.UPPER_GEH5).add(new QualityChartData(g5p, s, null));
        }
    ...
    public enum Key {
         ...
         * @param series
         * @return
         * @throws ArrayIndexOutOfBoundsException if <code>series</code> is not a valid id for
         *                                        <code>Key</code>.
         */
