...
public class SumoMatrixImporter extends TPS_BasicConnectionClass {
	...
	public void writeMatrix( Matrix matrix, String matrixName){
		...
		String query =
				"SELECT * FROM " + this.parameterClass.getString(ParamString.DB_SCHEMA_CORE) +this.parameterClass.getString(ParamString.DB_TABLE_MATRICES)+" WHERE \"matrix_name\" = '" + matrixName + "'";
		if(TPS_Logger.isLogging(SeverenceLogLevel.INFO)) {
			TPS_Logger.log(SeverenceLogLevel.INFO, "Preparing data for entry: "+matrixName+ " in table "+this.parameterClass.getString(ParamString.DB_TABLE_MATRICES));
		}
		if(converter.checkMatrixName(matrixName)){
			//update!
			query = "UPDATE " + this.parameterClass.getString(ParamString.DB_SCHEMA_CORE) + this.parameterClass.getString(ParamString.DB_TABLE_MATRICES)+" SET matrix_values = " ;
			query +=TPS_DB_IO.matrixToSQLArray(matrix, 0) + " WHERE \"matrix_name\" = '" + matrixName + "'";
			if(TPS_Logger.isLogging(SeverenceLogLevel.INFO)) {
				TPS_Logger.log(SeverenceLogLevel.INFO, "Updating data for entry: "+matrixName+ " in table "+this.parameterClass.getString(ParamString.DB_TABLE_MATRICES)+".");
			}
		} else {
			query = "INSERT INTO " + this.parameterClass.getString(ParamString.DB_SCHEMA_CORE) + this.parameterClass.getString(ParamString.DB_TABLE_MATRICES)+" (matrix_name, matrix_values) VALUES ('"+matrixName+"', " ;
			query +=TPS_DB_IO.matrixToSQLArray(matrix, 0) + ")";
			if(TPS_Logger.isLogging(SeverenceLogLevel.INFO)) {
				TPS_Logger.log(SeverenceLogLevel.INFO, "Inserting data for entry: "+matrixName+ " in table "+this.parameterClass.getString(ParamString.DB_TABLE_MATRICES)+".");
			}
		}
	...
	public void readSumoValues(String schema, String table){
		...
		try{
			...
			query = "SELECT taz_num_id from "+this.parameterClass.getString(ParamString.DB_SCHEMA_CORE)+this.parameterClass.getString(ParamString.DB_TABLE_TAZ)+" order by taz_num_id asc";
			...
			System.err.println(this.getClass().getCanonicalName()+" readSumoValues: SQL-Error during statement: "+query);
			e.printStackTrace();
		}		
	...
	public static void main(String[] args) {
		...
		SumoMatrixImporter worker = new SumoMatrixImporter("T:\\Simulationen\\runtime_perseus.csv", "T:\\Simulationen\\Berlin\\DB_Test\\full_test_perseus\\db_Run_full_new.csv");
