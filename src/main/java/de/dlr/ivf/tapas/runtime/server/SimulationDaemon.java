...
public final class SimulationDaemon{
	...
	private static List<Process> processes = new ArrayList<>(); //for later use when the Daemon will be able to start multiple servers
	private Timer timer;
	 ...
	 */
	private static TPS_ArgumentType<?>[] PARAMETERS = { new TPS_ArgumentType<>(
			"tapas network directory path", File.class) };
	...
	private SimulationDaemon(String[] args){
		...
		try {
			...
			con.executeUpdate("UPDATE "+parameterClass.getString(ParamString.DB_TABLE_SERVERS)+" SET remote_boot = true WHERE server_name = '"+hostname+"'", this);
	...
	public static void main(String[] args){				
		...
		try {
			...
			try{
				File f = new File("SimulationDaemonException_"+ SDF.format(new Date(System.currentTimeMillis()) +
						".log"));
				if (!f.exists())
					f.createNewFile();				
		...
		if(daemon == null)
			daemon = new SimulationDaemon(args);
	}
	...
	public void startServer() {
		...
		if(!p.isAlive()){
			...
			System.out.println(sysoutPrefix+"Previous Process is still running...");
	...
	private class ShutDownProcedure extends Thread {
		...
		public void run(){
			...
			try {
				con.executeUpdate("UPDATE "+con.getParameters().getString(ParamString.DB_TABLE_SERVERS)+" SET remote_boot = false WHERE server_name = '"+hostname+"'", this); //shutting down the daemon means no remote server start up
				if(p.isAlive()) {
					System.out.println(sysoutPrefix+"Waiting for Server to shut down...");
					con.executeUpdate("UPDATE "+con.getParameters().getString(ParamString.DB_TABLE_PROCESSES)+" SET shutdown = true WHERE host = '"+hostname+"' AND end_time IS NULL", this);   //set the shutdown flag on the server process
				}
				while(p.isAlive())
					Thread.sleep(1000);
	...
	private class ServerProcessChecker extends TimerTask{
		...
		public void run() {
			...
			try(ResultSet rs = con.executeQuery("SELECT * FROM "+con.getParameters().getString(ParamString.DB_TABLE_SERVERS)+" WHERE server_name = '"+hostname+"'", this)){
				if(rs.next()) {					
