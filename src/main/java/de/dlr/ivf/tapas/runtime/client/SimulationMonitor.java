...
public class SimulationMonitor implements TableModelListener, ITumInterface {
    ...
    public SimulationMonitor(SimulationControl control) {
        ...
        SimulationData.TPS_SimulationState.FINISHED.setAction(MultilanguageSupport.getString("state.finished"));
        SimulationData.TPS_SimulationState.INSERTED.setAction(MultilanguageSupport.getString("state.inserted"));
        SimulationData.TPS_SimulationState.STOPPED.setAction(MultilanguageSupport.getString("state.stopped"));
        SimulationData.TPS_SimulationState.STARTED.setAction(MultilanguageSupport.getString("state.started"));
            ...
            public void keyReleased(KeyEvent e) {
                if (e.getKeyCode() == 127) {
                    ...
                    for (String key : sim_key_to_remove) {
                        SimulationMonitor.this.control.removeSimulation(key, true);
                    }
                }
            }
        });
                ...
                public void actionPerformed(ActionEvent e) {
                    new Thread(() -> {
                        ...
                        if (row >= 0) {
                            ...
                            addWorkingKey(key);
                            SimulationMonitor.this.control.changeSimulationDataState(key);
                            stopCellEditing();
