...
public class SimulationMonitor implements TableModelListener, ITumInterface {
	...
	private JPanel tuminfo = new JPanel(new BorderLayout());
	...
	private final class ExportThread implements Runnable {
		...
		private ExportThread(TPS_DB_Connector connection, String params,
				String key) {
		...
		public void run() {
			try {
				ResultSet rs = connection.executeQuery(
						"SELECT core.simple_export_trip_table('" + params
								+ "_trips_" + key + "')",
	...
	public void setBackGroundColorForWorking(String simKey,
			Component component, boolean selected) {

		if (workingSimKeys.contains(simKey))
	...
	private void removeWorkingKey(String... keys) {
		if (keys.length == 1)
			workingSimKeys.remove(keys[0]);
		else if (keys.length > 1)
			workingSimKeys.removeAll(Arrays.asList(keys));
	...
	private void addWorkingKey(String... keys) {
		if (keys.length == 1)
			workingSimKeys.add(keys[0]);
		else if (keys.length > 1)
			workingSimKeys.addAll(Arrays.asList(keys));
	...
	private class BorderedRenderer extends DefaultTableCellRenderer implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			if(this.component == null)
				this.component = (JComponent) super.getTableCellRendererComponent(table, value,
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasfocus, int row, int column) {
			...
			if(value instanceof HashStatus) {
				...
				switch ((HashStatus)value) {
				  case UNKNOWN: comp.setBackground(transparent);
				  				comp.setToolTipText(MultilanguageSupport.getString("HASH_MESSAGE_UNKNOWN"));
				  				break;
				  case INVALID: comp.setBackground(Color.RED);
				  				comp.setToolTipText(MultilanguageSupport.getString("HASH_MESSAGE_ERROR"));
				  				break;
				  case NON_DOMINANT: comp.setBackground(Color.GRAY);
				  					 comp.setToolTipText("<html><strong>"+table.getValueAt(row, SERVER_INDEX_NAME)+
				  							             "</strong> "+MultilanguageSupport.getString("HASH_MESSAGE_INFIX")+
				  							             "<br />- "+control.getServerHash((String)table.getValueAt(row, SERVER_INDEX_NAME))+"</html>");
				  					 break;
				  case DOMINANT: comp.setBackground(Color.GREEN);
				  				 comp.setToolTipText("<html><strong>"+table.getValueAt(row, SERVER_INDEX_NAME)+
				  						 			 "</strong> "+MultilanguageSupport.getString("HASH_MESSAGE_INFIX")+
				  				 "<br />- "+control.getServerHash((String)table.getValueAt(row, SERVER_INDEX_NAME))+"</html>");
				  				 break;
				  case DEBUG: comp.setBackground(Color.ORANGE);
				  			  comp.setToolTipText(table.getValueAt(row, SERVER_INDEX_NAME)+" "+MultilanguageSupport.getString("HASH_MESSAGE_DEBUG"));
				  			  break;
				}				
			}
			else
	...
	private class ServerColorRenderer extends BorderedRenderer {
		...
		ServerColorRenderer(final JTable table, TableCellRenderer delegate){
            ...
            Timer blinkingTimer = new Timer(250, e -> {	blinkingState = !blinkingState;
                										table.repaint(); });
            ...
            boolean hasFocus, int row, int column)
        {
            ...
            if (blinkingCells.contains(cell))
                component.setBackground(blinkingState ? Color.RED : Color.GREEN);            
            else
                component.setBackground((Boolean) table.getModel().getValueAt(row, SERVER_INDEX_ONLINE) ? Color.GREEN : Color.RED);
	 ...
	 */
	private JPanel serverPanelRight;
	 ...
	 */
	private ConfigurableJTable serverTable;
	...
	private Set<String> workingSimKeys = Collections
	 ...
	 *            Reference to the control instance of this client
	 */
	public SimulationMonitor(SimulationControl control) {
		
		this.control = control;
		this.frame = new JFrame();
		this.frame.setTitle("TAPAS Simulation Control - Version: "+control.getVersion()+" | Build: "+control.getBuildnumber()+" | Build-date: "+control.getBuilddate());
		
		this.frame.setIconImage(new ImageIcon(getClass().getClassLoader().getResource("icons/TAPAS-Logo.gif")).getImage());
		// setup menu bar
		JMenu languageMenu = MultilanguageSupport.init(SimulationMonitor.class);
		...
		SimulationData.TPS_SimulationState.FINISHED
				.setAction(MultilanguageSupport.getString("state.finished"));
		SimulationData.TPS_SimulationState.INSERTED
				.setAction(MultilanguageSupport.getString("state.inserted"));
		SimulationData.TPS_SimulationState.STOPPED
				.setAction(MultilanguageSupport.getString("state.stopped"));
		SimulationData.TPS_SimulationState.STARTED
				.setAction(MultilanguageSupport.getString("state.started"));

		// setup server table panel

		this.serverTable = new ConfigurableJTable(new ClassifiedTableModel(
				new Object[0][0], new Object[] { "", "", "", "", "", "", "" })) {
			/**
					 * 
					 */
		...
		serverTable.getModel().addTableModelListener(e -> {
															if(e.getColumn() == SERVER_INDEX_SHUTDOWN && serverTable.getModel().getValueAt(e.getFirstRow(), SERVER_INDEX_SHUTDOWN) == ServerControlState.STOP)
																blinkCellRenderer.addBlinkingCell(e.getFirstRow(), SERVER_INDEX_ONLINE);																	
															else 
																blinkCellRenderer.removeBlinkingCell(e.getFirstRow(), SERVER_INDEX_ONLINE);
														  });
		...
		JButton chooseButton = new JButton(
				MultilanguageSupport.getString("CHOOSE_SIMULATION_FILE_BUTTON"));
		chooseButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				File[] files = SimulationMonitor.this.control.chooseRunFile(frame);
				if (files != null) {
					String fileList = "";
		...
		this.simTable = new ConfigurableJTable(new ClassifiedTableModel(
				new Object[0][0], new String[SIM_INDEX.values().length]) {
			/**
			 * serial UID
			 */
			 ...
			 * (non-Javadoc)
			 * 
			 * @see de.dlr.ivf.tapas.runtime.client.SimulationMonitor.
			 * TPS_ItemTableModel#isCellEditable(int, int)
			 */
			@Override
			public boolean isCellEditable(int row, int column) {
				return column == SIM_INDEX.ACTION.ordinal()
						|| column == SIM_INDEX.FILE.ordinal();
			}
			...
			public void setValueAt(Object aValue, int row, int column) {
				...
				if (column == SIM_INDEX.FILE.ordinal()) {
					String simkey = (String) getValueAt(row,
							SIM_INDEX.KEY.ordinal());
			...
			public void keyReleased(KeyEvent e) {
				if (e.getKeyCode() == 127) {
					...
					for (Integer row : simTable.getSelectedRows()) {
						sim_key_to_remove.add(simTable.getValueAt(row,
								SIM_INDEX.KEY.ordinal()).toString());
					}
					for (String key : sim_key_to_remove) {
						SimulationMonitor.this.control.removeSimulation(key,
								true);
					}
				}
			}
		...
		this.simTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getClickCount() == 2 && selectedRow != -1) {
					...
					final String simKeyOld = simTable.getModel()
							.getValueAt(selectedRow, SIM_INDEX.KEY.ordinal())
							.toString();
					new Thread(() -> {
						String optionalNewSimKey = new TPS_ParameterFileConverterGUI(control.getParameters())
		...
		simPanel.setBorder(BorderFactory
				.createTitledBorder(MultilanguageSupport
						.getString("SIM_PANEL_TITLE")));
		...
		bStart = new JButton(
				MultilanguageSupport.getString("SIM_CONTROL_START"));
		...
		bPause = new JButton(
				MultilanguageSupport.getString("SIM_CONTROL_PAUSE"));
		...
		bDelete = new JButton(
				MultilanguageSupport.getString("SIM_CONTROL_DELETE"));
		...
		bExport = new JButton(
				MultilanguageSupport.getString("SIM_CONTROL_EXPORT"));
		...
		bParameters = new JButton(
				MultilanguageSupport.getString("SIM_CONTROL_PARAMETER"));
		...
		bAnalyzer = new JButton(
				MultilanguageSupport.getString("SIM_CONTROL_ANALYZER"));
		...
		bCompare.addActionListener(e -> {
			...
			if(selected_sim_indices.length != 2) {
				...
				if(simkey1 != null && simkey2 != null && !simkey1.equals("") && !simkey2.equals("")) {
					Platform.runLater(() -> {				
						try {
							FXMLLoader loader = new FXMLLoader(getClass().getClassLoader().getResource("ParamComparator.fxml"));
							SimParamComparatorController controller = new SimParamComparatorController(simkey1,simkey2,this.control.getConnection());
							loader.setController(controller);					
							Parent root;
							root = loader.load();
							Scene scene = new Scene(root);
							scene.getStylesheets().add(getClass().getClassLoader().getResource("ParamComparator.css").toExternalForm());
		...
		bTUM.addActionListener(e-> {
									...
									if(approved == JFileChooser.APPROVE_OPTION){
										...
										IntegratedTUM itum = new StandardTUM();											
																			   //SwingWorker is done	
										itum.addPropertyChangeListener(evt -> {if("state".equals(evt.getPropertyName()) && SwingWorker.StateValue.DONE.equals(evt.getNewValue())){
																			   ...
																			   if("intermediate".equals(evt.getPropertyName()))
																				   resetProgressBars();																	
																     		  }
																	  );
										if(itum.setInput(this)){
											...
											JOptionPane.showMessageDialog(null, "Couldn't set the input for the TUM-export!\n\nPlease retry...", "Error passing input",
                                                    JOptionPane.ERROR_MESSAGE);
									}	
							   }
		);

		List <JButton> mainbtns = Arrays.asList(bCompare,bTUM,bAnalyzer,bParameters,bStart,bPause,bDelete,bExport);		
		...
		bCancel.addActionListener(e -> { if(!this.itum.isDone()){
											int approve = JOptionPane.showConfirmDialog(null, "Are you sure about cancelling the currently running TUM-Export?", "CANCEL", JOptionPane.YES_NO_OPTION);
											if(approve == JOptionPane.YES_OPTION)
												this.itum.cancel(true);
										 }
									   }
								 );
		...
		this.frame.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				SimulationMonitor.this.control.close();
			}
		});
		...
		this.frame.setLocation(dim.width / 2 - this.frame.getWidth() / 2,
				dim.height / 2 - this.frame.getHeight() / 2);
		this.frame.setVisible(true);
		this.frame.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				if (SimulationMonitor.this.control.configChanged()) {
					// default icon, custom title
					int n = JOptionPane.showConfirmDialog(frame,
							MultilanguageSupport
									.getString("SAVE_CHANGES_QUESTION"), "",
	...
	protected void exportSelected(Object[][] rows) {
		...
		if (connection != null) {
			...
			Toolkit jTools = Toolkit.getDefaultToolkit();
			Dimension dim = jTools.getScreenSize();
			jDialog.setLocation(dim.width / 2 - jDialog.getWidth() / 2,
					dim.height / 2 - jDialog.getHeight() / 2);
			...
			for (Object[] row : rows) {
				...
				if ((Boolean) row[SIM_INDEX.FINISHED.ordinal()]) {
					export.add(key);
					futures.add(threadPool.submit(new ExportThread(connection,
							params, key)));
				}
			}
			...
			if (futures.size() > 0) {
				...
				jDialog.getContentPane().add(
						new JLabel(
								"<html><b>Folgende Simulationen werden exportiert:</b><br/>"
										+ sbuilder));
	...
	protected void startSelected(Object[][] rows) {
		...
		JOptionPane.showMessageDialog(frame,
				"Folgende Simulationen nicht gestartet:\n\n" + notStarted);
	}
	...
	public void addServerData(SimulationServerData serverData) {
		synchronized (this.serverTable) {
			...
			if (index == -1) {
				...
				data[SERVER_INDEX_TIMESTAMP] = serverData
	...
	public void addSimulation(SimulationData simulation) {
			...
			data[SIM_INDEX.PARAMS.ordinal()] = simulation
					.getSimParams();
	...
	/**
	 * @param serverData
	 *            simulation server data to search
	...
	private int containsServerData(SimulationServerData serverData) {
		synchronized (this.serverTable) {
			for (int i = 0; i < this.serverTable.getRowCount(); i++) {
				if (serverTable.getModel().getValueAt(i, SERVER_INDEX_IP)
						.equals(serverData.getIp().getHostAddress())) {
					return i;
				}
			}
		}
	...
	private int containsSimulation(SimulationData simulation) {
		synchronized (this.simTable) {
			for (int i = 0; i < this.simTable.getRowCount(); i++) {
				if (simTable.getModel().getValueAt(i, SIM_INDEX.KEY.ordinal())
						.equals(simulation.getKey())) {
					return i;
				}
			}
		}
	...
	private void initTables() {
		String[] serverHeader = {
				MultilanguageSupport.getString("SERVER_HEADER_NAME"),
				MultilanguageSupport.getString("SERVER_HEADER_IP"),
				MultilanguageSupport.getString("SERVER_HEADER_HASH"),
				MultilanguageSupport.getString("SERVER_HEADER_ONLINE"),
				MultilanguageSupport.getString("SERVER_HEADER_CPU"),
				MultilanguageSupport.getString("SERVER_HEADER_CORES"),
				MultilanguageSupport.getString("SERVER_HEADER_TIMESTAMP"),
				"Shutting down"};
		((DefaultTableModel) this.serverTable.getModel())
				.setColumnIdentifiers(serverHeader);
		...
		// MultilanguageSupport
		((DefaultTableModel) this.simTable.getModel())
				.setColumnIdentifiers(simHeader);

		this.serverTable.initTableColumn(SERVER_INDEX_CORES, 50,
				new AlignmentRenderer(JLabel.CENTER, false), null);
		this.serverTable.initTableColumn(SERVER_INDEX_NAME, 100,
				new AlignmentRenderer(JLabel.LEFT, false), null);
		this.serverTable.initTableColumn(SERVER_INDEX_TIMESTAMP, 120,
				new AlignmentRenderer(JLabel.RIGHT, false), null);
		this.serverTable.initTableColumn(SERVER_INDEX_HASH, 50,
				new HashRenderer(true), null);
		this.serverTable.initTableColumn(SERVER_INDEX_ONLINE, 50,
				blinkCellRenderer , null);
		this.serverTable.initTableColumn(SERVER_INDEX_CPU, 0,
				new ProgressRenderer(false), null);
		this.serverTable.initTableColumn(SERVER_INDEX_IP, 111,
				new AlignmentRenderer(JLabel.LEFT, false), null);
		...
		this.simTable.initTableColumn(SIM_INDEX.FILE.ordinal(), 0,
				new LongTextRenderer(), new TextPopupEditor());
		...
		this.simTable.setTransferHandler(new TransferHandler("simkey") {
			...
			protected Transferable createTransferable(JComponent c) {
				...
				String s = (String) table.getValueAt(table.getSelectedRow(),
						SIM_INDEX.KEY.ordinal());
	...
	private void insertAction() {
		...
		try {
			StringTokenizer st = new StringTokenizer(
					runFilenameField.getText(), ";");
			int msOffset = 0;
			while (st.hasMoreTokens()) {
				sim_key = c.get(Calendar.YEAR) + "y_"
						+ f00.format(c.get(Calendar.MONTH) + 1) + "m_"
						+ f00.format(c.get(Calendar.DAY_OF_MONTH)) + "d_"
						+ f00.format(c.get(Calendar.HOUR_OF_DAY)) + "h_"
						+ f00.format(c.get(Calendar.MINUTE)) + "m_"
						+ f00.format(c.get(Calendar.SECOND)) + "s_"
						+ f000.format(c.get(Calendar.MILLISECOND) + msOffset)
						+ "ms";
				SimulationMonitor.this.control.addSimulation(sim_key, st.nextToken());
				msOffset++;
			}
		} catch (Exception ex) {
			JOptionPane.showMessageDialog(null, ex.getMessage(),
					"Start Simulation Exception", JOptionPane.ERROR_MESSAGE);
			ex.printStackTrace();
		}
	}
	 ...
	 * 
	 * @param serverData
	 *            simulation server data to remove
	 * @throws SQLException 
	 */
	 ...
	 * 
	 * @param serverData
	 *            simulation server data to update
	 */
	public void updateServerData(SimulationServerData serverData) {
		...
		synchronized (this.serverTable) {
			...
			if (index == -1) {
				...
				this.serverTable.setValueAt(serverData.getName(), index,
						SERVER_INDEX_NAME);
				this.serverTable.setValueAt(
						serverData.getIp().getHostAddress(), index,
						SERVER_INDEX_IP);
				this.serverTable.setValueAt(serverData.getHashStatus(), index, SERVER_INDEX_HASH);
				this.serverTable.setValueAt(serverData.isOnline(),
						index, SERVER_INDEX_ONLINE);
				this.serverTable.setValueAt(serverData.getCpu(), index,
						SERVER_INDEX_CPU);
				this.serverTable.setValueAt(serverData.getCores(), index,
						SERVER_INDEX_CORES);
				this.serverTable.setValueAt(serverData.getFormattedTimestamp(),
						index, SERVER_INDEX_TIMESTAMP);
	...
	public void updateSimulationData(SimulationData simulation) {
			...
			if (row == -1) {
				...
				simTable.setValueAt((double) simulation.getProgress()
						/ simulation.getTotal(), row,
						SIM_INDEX.PROGRESS.ordinal());
				simTable.setValueAt(
						simulation.getProgress() + "/" + simulation.getTotal(),
						row, SIM_INDEX.COUNT.ordinal());
				simTable.setValueAt(
						simulation.minimumState(TPS_SimulationState.STOPPED),
						row, SIM_INDEX.STOPPED.ordinal());
				simTable.setValueAt(
						simulation.minimumState(TPS_SimulationState.STARTED),
						row, SIM_INDEX.STARTED.ordinal());
				simTable.setValueAt(
						simulation.minimumState(TPS_SimulationState.FINISHED),
						row, SIM_INDEX.FINISHED.ordinal());
				simTable.setValueAt(simulation.getTimestampStarted(), row,
						SIM_INDEX.DATE_STARTED.ordinal());
				simTable.setValueAt(simulation.getTimestampFinished(), row,
						SIM_INDEX.DATE_FINISHED.ordinal());
				simTable.setValueAt(simulation.getElapsedTime(control
						.getCurrentDatabaseTimestamp()), row, SIM_INDEX.ELAPSED
						.ordinal());
				simTable.setValueAt(simulation.getEstimatedTime(control
						.getCurrentDatabaseTimestamp()), row,
						SIM_INDEX.ESTIMATED.ordinal());
				simTable.setValueAt(simulation.getState().getAction(), row,
						SIM_INDEX.ACTION.ordinal());
				simTable.setValueAt(simulation.getSimParams(), row,
						SIM_INDEX.PARAMS.ordinal());
			}
	...
	private void updateSimulationDescription(String description, String simkey) {
		TPS_DB_Connector connection = control.getConnection();
		String query = "UPDATE simulations SET sim_description = '"
				+ description + "' WHERE sim_key = '" + simkey + "'";
		connection.executeUpdate(query, control);
	}


	private void startAnalyzer(int selectedRow) {
		final String simkey = (String) simTable.getModel().getValueAt(
				selectedRow, SIM_INDEX.KEY.ordinal());
...
//		}
	...
	private JFileChooser tumFileChooser(int simcount){
		...
		JFileChooser tumfc = new JFileChooser(){
			...
			private ArrayList<File> filestopass = new ArrayList<>();
				...
				if(!existingfiles.isEmpty()){
					...
					for(File f : existingfiles){
						if(!f.canWrite()){
							...
							failfiles.append("- ").append(f.getName()).append("<br />");
					}	
					...
					if(protectedfiles.size()> 0){
						JOptionPane.showConfirmDialog(this, "<html>The following files already exist and are <strong>protected to overwrite</strong>:\n"+protfiles+"\nPlease choose a different filename.", "Protected Files",
                                JOptionPane.DEFAULT_OPTION, JOptionPane.ERROR_MESSAGE);
						result = JOptionPane.NO_OPTION;
					}
					else{
						result = JOptionPane.showConfirmDialog(this,"The following files already exist:\n"+failfiles+"\nWould you like to overwrite them?", "Existing files",
	 ...
	 * 
	 * @param cntsim
	 * 			number of selected simulations in the simulation panel
	 * 
	 */
	private void handleSimulationButtons(int cntsim){
		...
		List<JComponent> multiselecbtns = Arrays.asList(bCompare,bStart,bPause,bDelete,bExport,bAnalyzer,bParameters);
	 ...
	 * @param serverDataMap
	 * 			a map containing hostname as key and {@link SimulationServerData} as value
	 */
	public void updateServerControl(Map<String,SimulationServerData> serverDataMap){
		...
		if(count == 1){
			...
			String host = (String) this.serverTable.getModel().getValueAt(this.serverTable.getSelectedRow(), SERVER_INDEX_NAME);
			...
			enableComponents(this.serverPanelRight, false);	
	}
