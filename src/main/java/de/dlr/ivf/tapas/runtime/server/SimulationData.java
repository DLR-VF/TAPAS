...
public class SimulationData {
	...
	public enum TPS_SimulationState {
		...
		/**
		 * @param simulation
		 *            simulation to get the state of
		 * 
		...
		public static TPS_SimulationState getState(SimulationData simulation) {
			if (simulation.sim_finished) {
				if (simulation.getProgress() == simulation.getTotal())
					return TPS_SimulationState.FINISHED;
	...
	private class ProgressCalculator{
		...
		public void update(){
			...
			if(this.simData.isStarted() != this.lastStateStarted){
				...
				if(this.lastStateStarted && this.simData.sim_progress!=this.processedHouseholds){
					...
					mSecPerHH/=elements;
					//calc estimate
					this.estimatedTime = (long) ((double)(this.simData.sim_total-this.simData.sim_progress)*mSecPerHH);
				}
			}
		}
		...
		public long getEstimatedTime(){
			if(this.lastStateStarted)
				return Math.max(0, this.estimatedTime-System.currentTimeMillis()+this.lastTimeUpdate);
			else
				return 0;
		}
	}
	 ...
	 */
	private ProgressCalculator 				progressCalc;
	 ...
	 * 
	 * @throws SQLException
	 *             This exception is thrown if anything with the ResultSet is wrong, e.g. closed or too few values
	 */
	public SimulationData(ResultSet rs) throws SQLException {
		this(rs.getString("sim_key"), rs.getString("sim_file"), rs.getString("sim_description"), rs.getLong("sim_progress"), rs.getLong("sim_total"), rs.getBoolean("sim_ready"), rs.getBoolean("sim_started"), rs
				.getBoolean("sim_finished"), (String[]) rs.getArray("sim_par").getArray());
		this.update(rs);
	}
	 ...
	 *            run properties file
	 * @param sim_progress
	 *            amount of simulated households
	 * @param sim_total
	 ...
	 *            finished flag
	 * @param sim_params
	 *            array of the simulation parameters: random seed, sample size etc.
	 */
	...
	/**
	 * @param path
	 *            absolute path where the run properties file can be found.
	 * 
	...
	public String getTimestampFinished() {
		if (timestamp_finished != null)
			return SDF.format(timestamp_finished);
		return "";
	}
	...
	public String getTimestampStarted() {
		if (timestamp_started != null)
			return SDF.format(timestamp_started);
		return "";
	}
	...
	/**
	 * @param state
	 *            state to compare the current state to
	 * 
	...
	public String toString() {
		return this.getClass().getSimpleName() + " [id=" + getKey() + ", state=" + getState().name() + ", ready=" + sim_ready + " started=" + sim_started + " finished=" + sim_finished + "]";
	}
	 ...
	 * 
	 * @param rs
	 *            ResultSet from a database select
	 * @throws SQLException
	 *             This exception is thrown if anything with the ResultSet is wrong, e.g. closed or too few values
	 */
