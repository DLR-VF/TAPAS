...
public class TPS_DB_IOManager implements TPS_PersistenceManager {
	...
	public enum Behaviour {
		...
		FAT,
		/// Medium mix for the calculations.
		MEDIUM,
		/// All possible calculations are done on the database server.
		THIN
	}
	...
	private Queue<Integer> insertedHouseHolds = new LinkedList<>();
	
	private Queue<TPS_Plan> plansToStore = new LinkedList<>();
	...
	public static String buildQuery(TPS_ParameterClass parameterClass, String function, Object... parameters) {
		StringBuilder sb = new StringBuilder("SELECT " + parameterClass.getString(ParamString.DB_SCHEMA_CORE) + function + "(");
		if (parameters != null) {
	...
	public void createTemporaryAndOutputTables() {
		...
		functionExecute("create_hh_sample", ParamString.DB_TABLE_HOUSEHOLD, ParamString.DB_TABLE_HOUSEHOLD_TMP,
				...
				this.getParameters().getLongValue(ParamValue.RANDOM_SEED_NUMBER), this.getParameters().getDoubleValue(ParamValue.DB_HH_SAMPLE_SIZE));

		if(TPS_Logger.isLogging(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO)) {
			TPS_Logger.log(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO, "Create output trip table");
		}
		String query = "CREATE TABLE "
				+ this.getParameters().getString(ParamString.DB_TABLE_TRIPS)
	...
	public void init() {
		try{
			...
			if(TPS_Logger.isLogging(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO)) {
				TPS_Logger.log(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO, "Clearing constants (all codes: age," +
						" distance, person, activity, location, ...)");
			}
			this.dbIO.clearConstants();

			// reading initial information from database
			if(TPS_Logger.isLogging(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO)) {
				TPS_Logger.log(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO, "Reading constants (all codes: age," +
						" distance, person, activity, location, ...)");
			}
			this.dbIO.readConstants();
			
			if(TPS_Logger.isLogging(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO)) {
				TPS_Logger.log(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO, "Reading parameters for the utility function");
			}
			this.dbIO.readUtilityFunction();

			// read scheme set
			if(TPS_Logger.isLogging(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO)) {
				TPS_Logger.log(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO, "Reading scheme set (includes scheme classes, schemes and episodes)");
			}
			this.schemeSet = this.dbIO.readSchemeSet();
	
			if(TPS_Logger.isLogging(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO)) {
				TPS_Logger.log(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO, "Reading region with all blocks and locations");
			}
			this.region = this.dbIO.readRegion();
	
			if(TPS_Logger.isLogging(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO)) {
				TPS_Logger.log(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO, "Reading mode set with mode choice tree");
			}
			this.modeSet = new TPS_ModeSet(this.dbIO.readModeChoiceTree(), this.dbIO.readExpertKnowledgeTree(), this.getParameters());
	
			if(TPS_Logger.isLogging(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO)) {
				TPS_Logger.log(HierarchyLogLevel.CLIENT, SeverenceLogLevel.INFO, "Reading matrices (distances, travel times, access and egress times)");
			}
	...
	private class LocWithOccupancy implements Comparable<LocWithOccupancy>{
		...
		public int compareTo(LocWithOccupancy arg0) {
			...
			if(this.occupancy!=arg0.occupancy )
				return this.occupancy-arg0.occupancy;
			else
	...
	private void storeLocationOccupanyInDB() {
		...
		try {
			...
			if(TPS_Logger.isLogging(SeverenceLogLevel.INFO)) {
				TPS_Logger.log(SeverenceLogLevel.INFO,"Storing " +this.plansToStore.size()+" plan elements into the database.");
			}
			...
			Collections.sort(tmpIds); // Sorting prevents deadlocks, because no process can wait for a process with a smaller loc_id
			...
			pS = con.prepareStatement("UPDATE "+this.getParameters().getString(ParamString.DB_TABLE_LOCATION_TMP)+" SET loc_occupancy = loc_occupancy + ? WHERE loc_id = ANY (?::int[])");
			statement = "";
			...
			statement = "LOCK TABLE " + this.getParameters().getString(ParamString.DB_TABLE_LOCATION_TMP) + " IN EXCLUSIVE MODE";
			...
			while (!this.plansToStore.isEmpty()) {
				...
				for (TPS_TourPart tp : plan.getScheme().getTourPartIterator()) {
					for (TPS_Trip t : tp.getTripIterator()) {
						...
						if (Double.isNaN(plan.getAcceptanceProbability())|| Double.isInfinite(plan.getAcceptanceProbability())) {
							TPS_Logger.log(SeverenceLogLevel.FATAL, "NaN detected in getAcceptanceProbability for person " + p.getId());
						}
						pS.setDouble(index++, plan.getAcceptanceProbability());
						if (Double.isNaN(plan.getBudgetAcceptanceProbability())|| Double.isInfinite(plan.getBudgetAcceptanceProbability())) {
							TPS_Logger.log(SeverenceLogLevel.FATAL, "NaN detected in getBudgetcceptanceProbability for person " + p.getId());
						}
						pS.setDouble(index++, plan.getBudgetAcceptanceProbability());
						if (Double.isNaN(plan.getTimeAcceptanceProbability())|| Double.isInfinite(plan.getTimeAcceptanceProbability())) {
							TPS_Logger.log(SeverenceLogLevel.FATAL, "NaN detected in getTimeAcceptanceProbability for person " + p.getId());
						}
						...
						pS.setBoolean(index++, prevLoc.getTrafficAnalysisZone().hasToll(this.getParameters().getSimulationType()));
						...
						pS.setBoolean(index++, nextLoc.getTrafficAnalysisZone().hasToll(this.getParameters().getSimulationType()));
						...
						if (Double.isNaN(pt.getDuration())|| Double.isInfinite(pt.getDuration())) {
							TPS_Logger.log(SeverenceLogLevel.FATAL, "NaN detected in getDuration for person " + p.getId());
						}
						...
						if (Double.isNaN(pt.getDistanceBeeline()) || Double.isInfinite(pt.getDistanceBeeline())) {
							TPS_Logger.log(SeverenceLogLevel.FATAL, "NaN detected in getDistanceBeeline for person " + p.getId());
						}
						pS.setDouble(index++, pt.getDistanceBeeline());
						if (Double.isNaN(pt.getDistance()) || Double.isInfinite(pt.getDistance())) {
							TPS_Logger.log(SeverenceLogLevel.FATAL, "NaN detected in getDistance for person " + p.getId());
						}
						...
						pS.setInt(index++, (int) ((nextStayLocated.getStart() * 1.66666666e-2)+0.5)); // secs to min incl round
						...
						pS.setInt(index++, prevLoc.getTrafficAnalysisZone().getBbrType().getCode(TPS_SettlementSystemType.FORDCP));
						pS.setInt(index++, hh.getLocation().getTrafficAnalysisZone().getBbrType().getCode(TPS_SettlementSystemType.FORDCP));
	...
	private void storeFinishedHouseholdIDsToDB() {
		...
		try {
			...
			if(this.insertedHouseHolds.size()>0){
				if(TPS_Logger.isLogging(SeverenceLogLevel.INFO)) {
					TPS_Logger.log(SeverenceLogLevel.INFO,"Finishing " +this.insertedHouseHolds.size()+" households.");
				}
				...
				statement = "UPDATE " + this.getParameters().getString(ParamString.DB_TABLE_HOUSEHOLD_TMP) + " SET hh_started = true, hh_finished = true WHERE hh_id = ?";
				pS = con.prepareStatement(statement);
				for(Integer i: tmpIds){
					...
					if(numberOfHouseholds>0 && numberOfHouseholds%chunkSize ==0) {
						//lock table
						statement = "LOCK TABLE " + this.getParameters().getString(ParamString.DB_TABLE_HOUSEHOLD_TMP) + " IN EXCLUSIVE MODE";
						this.dbConnector.execute(statement, this);
						batchResultCounter = pS.executeBatch();
						//commit 
						con.commit();
			...
			if(numberOfHouseholds+numberOfPlans>0){
				if(householdStoreSuccesful && planStoreSuccessful ) {
					...
					if(TPS_Logger.isLogging(SeverenceLogLevel.ERROR)) {
						TPS_Logger.log(SeverenceLogLevel.ERROR,"ERROR: Error during commit to DB! Plans successful:"+planStoreSuccessful+" households successful: "+householdStoreSuccesful);
					}
				}
			}
			con.setAutoCommit(true);
	...
	public void resetUnfinishedHouseholds() {
		try {
			...
			// create additional temporary tables in database
			ResultSet rs = this.executeQuery(TPS_DB_IOManager.buildQuery(this.getParameters(),"reset_unfinished_households", this.getParameters().getString(ParamString.RUN_IDENTIFIER),IPInfo.getEthernetInetAddress().getHostAddress()));
			if(rs.next()){
				int count = rs.getInt(1);
				if(count >0) {
					if(TPS_Logger.isLogging(SeverenceLogLevel.WARN)) {
						TPS_Logger.log(SeverenceLogLevel.WARN, "Resetted "+ count + " households for new calculation!");
					}
				}
			}
			rs.close();
