...
public abstract class TPS_FileConsumer {
	 ...
	 *            flag if the file has a header
	 * @param recordLength
	 *            length of the record, values smaller than zero determines that the length is not specified
	 */
	 ...
	 * 
	 * @param record
	 *            the current line (record) of the file
	 * @param lineCount
	 ...
	 * 
	 * @param recordLength
	 *            record length, values smaller than zero determines that the length is not specified
	 */
	...
	private void consumeFile(boolean hasHeader) throws Exception {
		...
		if (hasHeader)
			this.init(this.reader.getHeaders());
		else
			this.init(null);
		...
		while ((record = reader.getRecord()) != null) {
			if (record.length == 0 || record[0].length() == 0) {
				if(TPS_Logger.isLogging(SeverenceLogLevel.WARN)) {
					TPS_Logger.log(SeverenceLogLevel.WARN, "Skipped empty line in file " + this.getShortFileName() + " in line " + (lineCount + 1) + ": " + Arrays.toString(record));
				}
			} else {
				try {
					if (this.getRecordLength() > 0) {
						if (record.length < this.getRecordLength()) {
							throw new IOException("Record  in file " + this.getShortFileName() + " in line "
									+ (lineCount + 1) + " " + Arrays.toString(record) + " is too short: "
							...
							if(TPS_Logger.isLogging(SeverenceLogLevel.FINE)) {
								TPS_Logger.log(SeverenceLogLevel.FINE, "Record  in file " + this.getShortFileName()
										+ " in line " + (lineCount + 1) + " " + Arrays.toString(record) + " is too long: "
										+ this.getRecordLength() + "(requested) < " + record.length);
							}
						}
					}
		...
		if(TPS_Logger.isLogging(SeverenceLogLevel.INFO)) {
			TPS_Logger.log(SeverenceLogLevel.INFO, "Read " + lineCount + " entries of file: " + new File(this.getFileName()).getName());
		}
	}
}
