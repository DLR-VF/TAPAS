...
public class TPS_IndividualTransportMode extends TPS_Mode {
	...
	public double getDistance(Locatable start, Locatable end, SimulationType simType, TPS_Car car) {
		...
		if (idStart == idDest) {
			...
			factor = beelineDistanceLoc / this.getParameters().paramMatrixClass.getValue(ParamMatrix.DISTANCES_BL,idStart, idDest);
	...
	public double getTravelTime(Locatable start, Locatable end, int time,
		...
		if (idStart == idDest) {
			...
			if (this.getParameters().isTrue(ParamFlag.FLAG_INTRA_INFOS_MATRIX)) {
				...
				// this value is used.
				tt = this.getParameters().paramMatrixMapClass.getValue(ParamMatrixMap.TRAVEL_TIME_MIT,idStart, idDest, simType, time);
				if (tt < 0) { // no connection via MIT!
					tt = beelineDistanceLoc * this.getParameters().getDoubleValue(ModeType.WALK.getBeelineFactor()) / this.getParameters().getDoubleValue(get(ModeType.WALK).getVelocity());
			...
			if (idStart != idDest) {
				tt *= beelineDistanceLoc / this.getParameters().paramMatrixClass.getValue(ParamMatrix.DISTANCES_BL,idStart, idDest);
			}
		}

		if(car!=null && car.getAutomation()>=this.getParameters().getIntValue(ParamValue.AUTOMATIC_VEHICLE_LEVEL) && SimulationType.SCENARIO.equals(simType)){
			...
			if(tt>rampUp){
				double distanceNet = this.getParameters().paramMatrixClass.getValue(ParamMatrix.DISTANCES_STREET,idStart, idDest);
				double timeMod = distanceNet>this.getParameters().getIntValue(ParamValue.AUTOMATIC_VEHICLE_TIME_MOD_THRESHOLD)?
									this.getParameters().getDoubleValue(ParamValue.AUTOMATIC_VEHICLE_TIME_MOD_FAR):
									this.getParameters().getDoubleValue(ParamValue.AUTOMATIC_VEHICLE_TIME_MOD_NEAR);
		...
		if (	this.getParameters().isDefined(ParamString.DB_NAME_MATRIX_ACCESS_MIT) && simType.equals(SimulationType.SCENARIO) ||
				this.getParameters().isDefined(ParamString.DB_NAME_MATRIX_ACCESS_MIT_BASE) && simType.equals(SimulationType.BASE))
			acc = this.getParameters().paramMatrixMapClass.getValue(ParamMatrixMap.ARRIVAL_MIT,idStart, idDest, simType, time);
		if (	this.getParameters().isDefined(ParamString.DB_NAME_MATRIX_EGRESS_MIT) && simType.equals(SimulationType.SCENARIO) ||
				this.getParameters().isDefined(ParamString.DB_NAME_MATRIX_EGRESS_MIT_BASE) && simType.equals(SimulationType.BASE))
			egr = this.getParameters().paramMatrixMapClass.getValue(ParamMatrixMap.EGRESS_MIT,idStart, idDest, simType, time);

		if(car!=null && car.getAutomation()>=this.getParameters().getIntValue(ParamValue.AUTOMATIC_VALET_PARKING)){
