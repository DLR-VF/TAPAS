...
public class TPS_ModeSet implements ExtendedWritable {
	 ...
	 * Mode Choice Tree for this mode set
	 */
	private TPS_ExpertKnowledgeTree expertKnowledgeTree;
	...
	public TPS_DiscreteDistribution<TPS_Mode> getModeDistribution(TPS_Plan plan, double distanceNet, TPS_ModeChoiceContext mcc){
		...
		TPS_DiscreteDistribution<TPS_Mode> srcDis = TPS_Mode.getUtilityFunction(this.parameterClass).getDistributionSet(this, plan, distanceNet, mcc);//locComingFrom, currentStayLocation, stay.getOriginalStart(), durationStay, car, fBike,stay);
	...
	private TPS_ExtMode selectMode0(TPS_Plan plan, double distanceNet, TPS_ModeChoiceContext mcc) {
		...
		if (dstDis == null || dstDis.size()==0) {
			if(TPS_Logger.isLogging(HierarchyLogLevel.EPISODE, SeverenceLogLevel.SEVERE)) {
				TPS_Logger.log(HierarchyLogLevel.EPISODE, SeverenceLogLevel.SEVERE, "Distribution is empty. Using 'WALK' as default.");
			}
		...
		if(primary.isType(ModeType.MIT) && !plan.mustPayToll) {
			...
			if (mcc.carForThisPlan != null && mcc.carForThisPlan.hasPaidToll)
				carMustPayToll = false;
			...
			if (this.getParameters().isTrue(ParamFlag.FLAG_USE_EXIT_MAUT) && !goingToTVZ.hasToll(SimulationType.SCENARIO)
					&& comingFromTVZ.hasToll(SimulationType.SCENARIO) && carMustPayToll) {
...
//		}
	 ...
	 * @param prevStay
	 * @param locatedStay
	 * 			the stay the modes should be selected for
	 * @param nextStay
	 * @param pc
	 */
	public void selectMode(TPS_Plan plan, TPS_Stay prevStay, TPS_LocatedStay locatedStay, TPS_Stay nextStay, TPS_PlanningContext pc) {
		...
		if(currentArrivalMode==null || currentDepartureMode ==null){
			...
			double distanceNet = Math.max(this.parameterClass.getDoubleValue(ParamValue.MIN_DIST), TPS_Mode.get(ModeType.WALK).getDistance(previousStayLocation, currentStayLocation, SimulationType.SCENARIO, null)-obscureDistanceCorrectionNumber);
			...
			if (nextStayArrivalMode == null) { //does the next stay allow unrestricted mode choice
				...
				if(currentArrivalMode==null) {
					TPS_Logger.log(HierarchyLogLevel.EPISODE, SeverenceLogLevel.SEVERE,	"Error: no possible mode found!");
				}
				...
				if(currentArrivalMode==null) { // is the arrival mode set?
					if (previousStayDepartureMode == null) { //does the previous stay allow unrestricted mode choice
						if (currentDepartureMode.isFix()) { // do we have to stick to the mode
							...
							mcc.duration = mcc.toStay.getOriginalDuration();
							mcc.startTime = mcc.toStay.getOriginalStart();
							mcc.isBikeAvailable = tourpart.isBikeUsed(); // pc.isBikeAvailable; !!! why this - it should be same as before (pc.isBikeAvailable)
							mcc.carForThisPlan = tourpart.getCar(); // pc.carForThisPlan; !!! why this - it should be same as before (pc.carForThisPlan)
							currentArrivalMode = selectMode0(plan, distanceNet, mcc);
						}						
					} else {// adopt arrival mode according to departure mode	
						currentArrivalMode = previousStayDepartureMode;
