...
public class TPS_Worker implements Callable<Exception> {
	...
	private void createPersonPlans(TPS_PlanEnvironment pe, TPS_Car carDummy) {
		...
		if(this.getParameters().isDefined(ParamString.HOUSEHOLD_MEMBERSORTING)){
			sortAlgo = TPS_Household.Sorting.valueOf(this.getParameters().getString(ParamString.HOUSEHOLD_MEMBERSORTING));
		}
		...
		for(int schemeIt=0; !finished&&schemeIt<maxTriesScheme; ++schemeIt) {
			...
			while(!finishedPlanSearch){
				...
				modedPlan.selectLocationsAndModesAndTravelTimes(pc); // mode choice for all trips and location choice for not fixed locations
		...
		if (TPS_Logger.isLogging(this.getClass(), SeverenceLogLevel.DEBUG)) {
			...
			if (!accepted) {
				TPS_Logger.log(SeverenceLogLevel.DEBUG, "No acceptable plan was found in " + pe.getNumberOfRejectedPlans() + " rounds ( feasible Plans: "+ pe.getNumberOfFeasiblePlans() +" )in " + time + "ms");
			} else {
				TPS_Logger.log(SeverenceLogLevel.DEBUG, "Acceptable plan was found in round: " + pe.getNumberOfRejectedPlans() + "( feasible Plans: "+ pe.getNumberOfFeasiblePlans() +" ) in " + time + "ms");
			}
		}
	}	
    ...
    public static List<Integer[]> getAllLists(List<Integer> elements, int lengthOfVector) {
    	...
    	if(lengthOfVector <0){ //no permutations
    		System.err.println("Du willst eine Permutation mit einer Länge von "+lengthOfVector+"! Das geht nicht!");
    		return allList;
    	}
    	if(lengthOfVector ==0){ //no permutations
    		return allList;
	...
	private void processHousehold(TPS_Household hh) {
		...
		if(!sortAlgo.equals(TPS_Household.Sorting.COST_OPTIMUM)){
			for (TPS_Person person : hh.getMembers(sortAlgo)) {
				...
				if (person.isChild()) {
					if(TPS_Logger.isLogging(HierarchyLogLevel.PERSON, SeverenceLogLevel.DEBUG)) {
						TPS_Logger.log(HierarchyLogLevel.PERSON, SeverenceLogLevel.DEBUG, "Person is skipped because it is a child");
					}
					continue;
				}
				...
				if (this.getParameters().isTrue(ParamFlag.FLAG_REJUVENATE_RETIREE)) {
					if (person.getAge() >= this.getParameters().getIntValue(ParamValue.REJUVENATE_AGE)) {
						if(TPS_Logger.isLogging(HierarchyLogLevel.PERSON, SeverenceLogLevel.DEBUG)) {
							TPS_Logger.log(HierarchyLogLevel.PERSON, SeverenceLogLevel.DEBUG, "Person's age gets adapted");
						}
						person.setAgeAdaption(true, this.getParameters().getIntValue(ParamValue.REJUVENATE_BY_NB_YEARS));
					}
				}
		...
		try {
			while (TPS_Main.STATE.isRunning() && (hh = PM.getNextHousehold()) != null) {
				if(TPS_Logger.isLogging(HierarchyLogLevel.HOUSEHOLD, SeverenceLogLevel.DEBUG)) {
					TPS_Logger.log(HierarchyLogLevel.HOUSEHOLD, SeverenceLogLevel.DEBUG, "Working on household: " + hh.toString());
				}
				processHousehold(hh);
