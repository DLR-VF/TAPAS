...
public class TPS_Person implements ExtendedWritable {
	...
	private ShoppingPreferenceAccessibility selectPreferenceAccessibility(ShoppingClass s) {
		...
		Map<ShoppingPreferenceAccessibility,PreferenceSet> accessForShoppingClass = preferenceValues.preferencesAccess.get(s);
		...
		for(ShoppingPreferenceAccessibility e : ShoppingPreferenceAccessibility.valueArray) {
			...
			if(prefSet.isUsed) {
				utilities[index] = 	TPS_FastMath.exp(prefSet.utility + //utility
									Randomizer.randomGaussianDistribution(()-> Randomizer.randomGaussian(),0,prefSet.sigma) //uncertainty
	...
	private ShoppingPreferenceSupply selectPreferenceSupply(ShoppingClass s) {
		...
		double sum =0;
		boolean oneIsSet = false;
		Map<TPS_PreferenceParameters.ShoppingPreferenceSupply,PreferenceSet> supplyForShoppingClass = preferenceValues.preferencesSupply.get(s);
		PreferenceSet prefSet;
	...
	public double primaryDriver(){
		if(driverScore <-0.1){
			...
			for( TPS_Person p :this.getHousehold().getMembers(TPS_Household.Sorting.NONE) ){
				members++;
				if(p.working>0)
					numberOfWorkingMembers++;
				if(p.hasDrivingLicense())
					numberOfMembersWithDrivingLicense++;
				if(p.age>=18){
					//numberOfAdhults++;
	 ...
	 */
	public TPS_Person(int id, TPS_Sex sex, TPS_PersonGroup personGroup, int age, boolean abo, boolean hasBike, double budget,
			double isWorking, boolean isTeleworker, int workLocationID, int eduactionLevel, boolean use_shopping_motives) {
		...
		if(use_shopping_motives) {
			this.errorTerm = Randomizer.randomGumbelDistribution(() -> Randomizer.random(),0, 1); // constant for one person
		}
	...
	public boolean mayDriveACar() {
		return (hasDrivingLicenseInformation() && hasDrivingLicense()) || (!hasDrivingLicenseInformation() && getAge()>=18); 
	}
	...
	public String toString(String prefix) {
		return prefix + this.getClass().getSimpleName() + " [id=" + id + ", persGroup=" + persGroup.getCode() + ", age="
				+ age + " in class=" + this.getAgeClass().getCode(TPS_AgeCodeType.STBA) + ", sex=" + sex.name() + "]";
	}
