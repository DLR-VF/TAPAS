...
public class Matrix {
    ...
    public double getAverageValue(boolean skipDiagonal, boolean skipNegative) {
        ...
        for (i = 0; i < this.n; i++) {
            for (j = 0; j < this.m; j++) {
                if (i == j && skipDiagonal) continue;
                if (this.vals[i][j] < 0 && skipNegative) continue;
                num++;
    ...
    public String toString(MatrixPrint print) {
        ...
        for (int i = this.sIndex; i < m + sIndex; i++) {
            if (i == sIndex) sb.append(begin[0]);
            else sb.append(begin[1]);
            ...
            if (i == m + sIndex - 1) sb.append(end[1]);
            else sb.append(end[0]);
        }
        return sb.toString();
    }
