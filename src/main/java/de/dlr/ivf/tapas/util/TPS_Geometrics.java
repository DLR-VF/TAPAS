...
public class TPS_Geometrics {
	...
	private static double earthRadius = 6371000.785;  
	...
	public static double getDistance(Locatable l0, Locatable l1, double min_dist) {
		...
		//}
		return TPS_Geometrics.getDistance(l0.getCoordinate(), l1.getCoordinate(), min_dist);
	}	
	...
	public static double getDistance(Locatable startLoc, Locatable endLoc, TPS_Mode mode, SimulationType simType) {
		...
		if (startLoc != null && endLoc != null) {
			...
			if (bezAId == bezBId  && mode.getParameters().isTrue(ParamFlag.FLAG_INTRA_INFOS_MATRIX) && mode.getParameters().isFalse(ParamFlag.FLAG_USE_BLOCK_LEVEL)) {
				...
				double beelineTAZ = mode.getParameters().paramMatrixClass.getValue(ParamMatrix.DISTANCES_BL, bezAId, bezBId);
				double beelineLoc = TPS_Geometrics.getDistance(startLoc.getCoordinate(), endLoc.getCoordinate(), mode.getParameters().getDoubleValue(ParamValue.MIN_DIST));
				...
				dist = TPS_Geometrics.getDistance(startLoc.getCoordinate(), endLoc.getCoordinate(), mode.getParameters().getDoubleValue(ParamValue.MIN_DIST));
			}
		}
	...
	public static double getDistance(double lat1, double lon1, double lat2, double lon2) {
		...
		x=Math.max(-1.0,Math.min(1.0,x)); //if c0 == c1 values slightly bigger than 1 occur. FIX: chop to valid acos-range 
	...
	public static double getDistance(TPS_Coordinate g0, TPS_Coordinate g1, TPS_Coordinate point, double min_distance) {
		if(isSameLocation(g0,g1)) { // give point dist
			...
			z = ((p[0] - t[0]) *r[0] + (p[1] - t[1]) * r[1] + (p[2] - t[2]) * r[2])/(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
	 ...
	 * @param distToEnd
	 * 		radius around the end
	 * @param point
	 * 		point, which distance should be measured
