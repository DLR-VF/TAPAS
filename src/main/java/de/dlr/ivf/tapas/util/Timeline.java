...
public class Timeline {
	...
	public boolean add(int start, int end) {
		if (start > end) {
			if(TPS_Logger.isLogging(SeverenceLogLevel.WARN)) {
				TPS_Logger.log(SeverenceLogLevel.WARN, "Start is greater or equal to end (start=" + start + ", end=" + end + ")");
			}
			return false;
		}
	...
	private Entry[] getSurroundingEntries(int start, int end) {
		...
		for (e = entrance; e !=null &&(this.entries[0] ==null || this.entries[1] == null); e = e.next){
			if(this.entries[0] ==null && e.end<=start && e.next.start>=end)
				this.entries[0] = e;
			if(this.entries[1] ==null && e.start>=end && e.previous.end<=start)
				this.entries[1] = e;
		}
