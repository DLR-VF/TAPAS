...
public class TPS_VariableMap {
	...
	private Map<Thread, Map<TPS_Attribute, Integer>> attributeMap;
	 ...
	 */
	private List<TPS_Attribute> attributes;

	/**
	 * Debug string which stores the last request
	 */
	private Map<Thread, String> lastRequest;
	 ...
	 * 
	 * @param codes
	 *            list of codes corresponding to the attributes list
	 * @param value
	 *            the leaf value
	 */
	...
	public void addValue(List<Integer> codes, double value) {
		if (codes.size() != this.attributes.size()) {
			throw new IllegalArgumentException("Sizes of attributes and codes do not fit! [" + this.attributes.size() + "!="
					+ codes.size() + "]");
		}
...
//	}
	...
	private String toString(Map<Integer, Map<Integer, ?>> mappedMap, String prefix, int depth) {
		...
		if (depth == this.attributes.size() - 2) {
			...
			for (Integer key : mappedMap.keySet()) {
				sb.append(this.toString((Map<Integer, Map<Integer, ?>>) mappedMap.get(key), prefix + ", "
						+ key, depth + 1));
			}
		}
		return sb.toString();
