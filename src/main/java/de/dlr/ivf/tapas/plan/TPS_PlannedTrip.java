...
public class TPS_PlannedTrip extends TPS_AdaptedEpisode implements ExtendedWritable {
	 ...
	 * 
	 * @param currency
	 *            currency to return in the costs
	 * 
	...
	boolean setTravelTime(TPS_LocatedStay pComingFrom, TPS_LocatedStay pGoingTo) {
		...
		if (!pComingFrom.isLocated()) {
			throw new RuntimeException("tpsTrip.setTravelTime: missing location of the preceding stay: " + pComingFrom.getStay().getActCode());
		}

		if (!pGoingTo.isLocated()) {
			throw new RuntimeException("tpsTrip.setTravelTime: missing location of the following stay: " + pGoingTo.getStay().getActCode());
		}

		TPS_Location pLocComingFrom = pComingFrom.getLocation();
		TPS_Location pLocGoingTo = pGoingTo.getLocation();
		this.setDistanceBeeline(TPS_Geometrics.getDistance(pLocComingFrom, pLocGoingTo, plan.getParameters().getDoubleValue(ParamValue.MIN_DIST)));
		...
		this.setDistanceEmptyNet(TPS_Mode.get(ModeType.WALK).getDistance(pLocComingFrom, pLocGoingTo, SimulationType.SCENARIO,null));
		...
		if (primaryMode.isType(ModeType.PT)){
			tt = primaryMode.getTravelTime(pLocComingFrom, pLocGoingTo, this.getStart(), 
					SimulationType.SCENARIO, pComingFrom.getStay().getActCode(), pGoingTo.getStay()
					.getActCode(), plan.getPerson(),null);
		...
		} else if(primaryMode.isType(ModeType.TRAIN) && this.plan.getParameters().isDefined(ParamFlag.FLAG_USE_CARSHARING) && this.plan.getParameters().isTrue(ParamFlag.FLAG_USE_CARSHARING)){ // carsharing faker!
			...
			tt= primaryMode.getTravelTime(pLocComingFrom, pLocGoingTo, this.getStart(), 
					SimulationType.SCENARIO, pComingFrom.getStay().getActCode(), pGoingTo.getStay()
					.getActCode(), plan.getPerson(),carForTrip);
		}
		...
		if(TPS_Mode.noConnection(tt)){ //you can always walk!
			...
			tt = primaryMode.getTravelTime(pLocComingFrom, pLocGoingTo, this.getStart(), SimulationType.SCENARIO, pComingFrom.getStay().getActCode(), pGoingTo.getStay()
					.getActCode(), plan.getPerson(),null);
		}
		...
		if(!primaryMode.isType(ModeType.MIT)) {
			...
			if (this.plan.getPerson().isWorking() && plan.getPerson().getHousehold().getIncome() >= this.plan.getParameters().getIntValue(ParamValue.MIT_INCOME_CLASS_COMMUTE)) {
			...
			if (this.plan.getParameters().isTrue(ParamFlag.FLAG_USE_EXIT_MAUT)) {
				if (!goingToTVZ.hasToll(SimulationType.SCENARIO) && comingFromTVZ.hasToll(SimulationType.SCENARIO)) {
					...
					if(TPS_Logger.isLogging(SeverenceLogLevel.FINE)) {
						TPS_Logger.log(SeverenceLogLevel.FINE, "MIV - trip has maut: " + comingFromTVZ.getTollFee(SimulationType.SCENARIO) + " costSum: " + costStay);
					}
				}
			}
			...
			if (goingToTVZ.hasParkingFee(SimulationType.SCENARIO)) {
				...
				if(TPS_Logger.isLogging(SeverenceLogLevel.FINE)) {
					TPS_Logger.log(SeverenceLogLevel.FINE, "GoingTo has parkingfee of" + goingToTVZ.getParkingFee(SimulationType.SCENARIO) + "; duration: " + this.getDuration() + "; costSum: " + costSzen);
				}
			}
		}
