...
public class TPS_Plan implements ExtendedWritable, Comparable<TPS_Plan> {
	...
	private Map<TPS_Stay, TPS_LocatedStay> locatedStays;

	/// The environment for this plan
	public TPS_PlanEnvironment pe;

	/// A map of planned trips so far
	private Map<TPS_Trip, TPS_PlannedTrip> plannedTrips;
	...
	public TPS_Plan(TPS_Person person, TPS_PlanEnvironment pe, TPS_Scheme schemeIn, TPS_PersistenceManager pm) {
		...
		myAttributes.put(TPS_Attribute.HOUSEHOLD_INCOME_CLASS_CODE, TPS_Income.getCode(person.getHousehold().getIncome()));
		myAttributes.put(TPS_Attribute.PERSON_AGE, person.getAge());
		myAttributes.put(TPS_Attribute.PERSON_AGE_CLASS_CODE_PERSON_GROUP, person.getPersGroup().getCode(TPS_PersonGroupType.TAPAS));
		...
		myAttributes.put(TPS_Attribute.CURRENT_TAZ_SETTLEMENT_CODE_TAPAS, person.getHousehold().getLocation().getTrafficAnalysisZone().getBbrType().getCode(TPS_SettlementSystemType.TAPAS));
		myAttributes.put(TPS_Attribute.PERSON_AGE_CLASS_CODE_STBA, person.getAgeClass().getCode(TPS_AgeCodeType.STBA));
		myAttributes.put(TPS_Attribute.PERSON_HAS_BIKE, person.hasBike()?1:0);
		myAttributes.put(TPS_Attribute.HOUSEHOLD_CARS, person.getHousehold().getCarNumber()); // TODO: note that this is set once again in selectLocationsAndModesAndTravelTimes
	...
	public int balanceStarts() {
		...
		for (TPS_TourPart tp : this.scheme.getTourPartIterator()) {
			for (TPS_Trip t : tp.getTripIterator()) {
				...
				start= pt.getStart();
				if(start <= lastStart) {
					TPS_Logger.log(SeverenceLogLevel.WARN, "Same or earlier start of episode detected! last:" +lastStart+" act:" +start);
					start =lastStart+1;
					pt.setStart(start);
				}
				lastStart = start;
	...
	public List<List<String>> createOutput() {
		...
		for (TPS_TourPart tour : this.scheme.getTourPartIterator()) {
			...
			for (TPS_Trip trip : tour.getTripIterator()) {
				...
				// TAZ departure
				out.add(Integer.toString(this.getLocatedStay(prevStay).getLocation().getTrafficAnalysisZone().getTAZId()));

				// TAZ arrival
				out.add(Integer.toString(this.getLocatedStay(nextStay).getLocation().getTrafficAnalysisZone().getTAZId()));
				...
				// coming Has Toll
				out.add(Boolean.toString(this.getLocatedStay(prevStay).getLocation().getTrafficAnalysisZone().hasToll(
						SimulationType.SCENARIO)));

				// going Has Toll
				out.add(Boolean.toString(this.getLocatedStay(nextStay).getLocation().getTrafficAnalysisZone().hasToll(
						SimulationType.SCENARIO)));
	...
	public TPS_LocatedStay getPreviousLocatedStay(TPS_AdaptedEpisode adaptedEpisode) {
		return this.getLocatedStay(adaptedEpisode.getEpisode().getSchemePart().getPreviousStay(
				adaptedEpisode.getEpisode()));
	}
	...
	public void selectLocationsAndModesAndTravelTimes(TPS_PlanningContext pc) {		
		if(TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
			TPS_Logger.log(SeverenceLogLevel.DEBUG, "Start select locations procedure for plan (number=" + pe.getNumberOfRejectedPlans() + ") with scheme (id=" + this.scheme.getId() + ")");
		}
		...
		for (TPS_SchemePart schemePart : this.scheme) {
			...
			TPS_TourPart tourpart = (TPS_TourPart) schemePart;
			if(TPS_Logger.isLogging(SeverenceLogLevel.FINE)) {
				TPS_Logger.log(SeverenceLogLevel.FINE, "Start select location for each stay in tour part (id=" + tourpart.getId() + ")");
			}
			...
			for (TPS_Stay stay : tourpart.getPriorisedStayIterable()) {
				myAttributes.put(TPS_Attribute.CURRENT_EPISODE_ACTIVITY_CODE_TAPAS, stay.getActCode().getCode(TPS_ActivityCodeType.TAPAS));			
				
				pc.pe.getPerson().estimateAccessibilityPreference(stay, this.PM.getParameters().isTrue(ParamFlag.FLAG_USE_SHOPPING_MOTIVES));
				...
				if(!currentLocatedStay.isLocated()){
					...
					pc.fixLocationAtBase = currentActCode.isFix() && this.PM.getParameters().isTrue(ParamFlag.FLAG_USE_FIXED_LOCS_ON_BASE)
							&& !this.fixLocations.containsKey(currentActCode);
					...
					if (currentActCode.isFix() && this.fixLocations.containsKey(currentActCode)) {
						...
						if(	pc.carForThisPlan != null && // we have a car 
							...
							this.fixLocations.get(currentActCode).getTrafficAnalysisZone().isRestricted()) // we have a restricted car wanting to go to a restricted area! -> BAD!
							{
						...
						if(TPS_Logger.isLogging(HierarchyLogLevel.EPISODE, SeverenceLogLevel.FINE)) {
							TPS_Logger.log(HierarchyLogLevel.EPISODE, SeverenceLogLevel.FINE, "Set location from fix locations");
						}
					} else {
						currentLocatedStay.selectLocation(this, pc);
						if (currentActCode.isFix()) {
					...
					if (DEBUG) {
						if(TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
							String s = "gewählte Location zu Stay: " + currentLocatedStay.getEpisode().getId()+ ": "+ currentLocatedStay.getLocation().getId()+ " in TAZ:"+ currentLocatedStay.getLocation().getTrafficAnalysisZone().getTAZId()+ " in block: "+ (currentLocatedStay.getLocation().hasBlock() ? currentLocatedStay.getLocation().getBlock().getId() : -1) + " via" + currentLocatedStay.getModeArr().getName()+ "/" + currentLocatedStay.getModeDep().getName();
							TPS_Logger.log(SeverenceLogLevel.DEBUG, s);
						}
					}
					if (TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
						TPS_Logger.log(SeverenceLogLevel.DEBUG, "Selected location (id="+ currentLocatedStay.getLocation().getId()+ ") for stay (id="+ currentLocatedStay.getEpisode().getId()+ " in TAZ (id="+ currentLocatedStay.getLocation().getTrafficAnalysisZone().getTAZId()+ ") in block (id= "+ (currentLocatedStay.getLocation().hasBlock() ? currentLocatedStay.getLocation().getBlock().getId() : -1) + ") via modes "+ currentLocatedStay.getModeArr().getName() + "/"+ currentLocatedStay.getModeDep().getName());
					}
				}
				...
				if(currentLocatedStay.getModeArr()==null || currentLocatedStay.getModeDep()==null){
					if(TPS_Logger.isLogging(SeverenceLogLevel.FINE)) {
						TPS_Logger.log(SeverenceLogLevel.FINE, "Start select mode for each stay in tour part (id=" + tourpart.getId() + ")");
					}
				...
				if (DEBUG) {
					if(TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
						String s = "gewählter Modus zu Stay: "
								+ currentLocatedStay.getEpisode().getId()
								+ ": " + currentLocatedStay.getModeArr()==null?"NULL":currentLocatedStay.getModeArr().getName()
								+ " in TAZ:" + currentLocatedStay.getLocation().getTrafficAnalysisZone().getTAZId()
								+ " in block: " + (currentLocatedStay.getLocation().hasBlock() ? currentLocatedStay.getLocation().getBlock().getId() : -1) + " via" + currentLocatedStay.getModeArr().getName()
								+ "/" + currentLocatedStay.getModeDep().getName();
						TPS_Logger.log(SeverenceLogLevel.DEBUG, s);
					}
				}
				if (TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
					TPS_Logger.log(SeverenceLogLevel.DEBUG, "Selected mode (id="
							+ currentLocatedStay.getModeArr()==null?"NULL":currentLocatedStay.getModeArr().getName()
							+ ") for stay (id="
							+ currentLocatedStay.getEpisode().getId()
							+ " in TAZ (id="
				...
				if (!tourpart.isFirst(stay)) {
					this.getPlannedTrip(tourpart.getPreviousTrip(stay)).setTravelTime(this.getLocatedStay(prevStay), this.getLocatedStay(stay));
				} 
				//set travel time for the departure mode
				if (!tourpart.isLast(stay)) {
					this.getPlannedTrip(tourpart.getNextTrip(stay)).setTravelTime(this.getLocatedStay(stay), this.getLocatedStay(goingTo));
				}
				//update the travel durations for this plan
				tourpart.updateActualTravelDurations(this);
				myAttributes.put(TPS_Attribute.CURRENT_TAZ_SETTLEMENT_CODE_TAPAS, currentLocatedStay.getLocation()
						.getTrafficAnalysisZone().getBbrType().getCode(TPS_SettlementSystemType.TAPAS));
			}//end for tourpart
		}
		...
		TPS_Logger.log(SeverenceLogLevel.DEBUG, "Selected all locations in " + (System.currentTimeMillis() - start) + "ms");
	}	
	...
	public void calcPlanFeasiblity() {
		...
		for (TPS_Episode e : sortedEpisodes) {
			...
			if (es.getDuration() < this.PM.getParameters().getIntValue(ParamValue.SEC_TIME_SLOT)) 
				es.setDuration(this.PM.getParameters().getIntValue(ParamValue.SEC_TIME_SLOT));
			start = es.getStart();
		...
		if( bookCar && //disable car booking for COST_OPTIMUM sorting
				...
				this.PM.getParameters().getString(ParamString.HOUSEHOLD_MEMBERSORTING).equalsIgnoreCase(TPS_Household.Sorting.COST_OPTIMUM.name())){
			bookCar =false;
		}
	...
	public void writeOutputStream(Writer out) throws IOException {
		...
		for (TPS_TourPart tour : this.scheme.getTourPartIterator()) {
			...
			for (TPS_Trip trip : tour.getTripIterator()) {
				...
				// TAZ departure
				out.write(Integer.toString(this.getLocatedStay(prevStay).getLocation().getTrafficAnalysisZone().getTAZId()));
				out.write(", ");

				// TAZ arrival
				out.write(Integer.toString(this.getLocatedStay(nextStay).getLocation().getTrafficAnalysisZone().getTAZId()));
				out.write(", ");
