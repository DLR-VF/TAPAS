...
public class TPS_PlanEVA1Acceptance implements TPS_PlanAcceptance {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.dlr.ivf.tapas.plan.acceptance.TPS_PlanAcceptance#isPlanAccepted(de.dlr.ivf.tapas.plan.TPS_Plan)
	 */
	...
	private boolean isPlanAcceptedByAdditionalConstraints(TPS_Plan plan) {
		...
		double 	acceptanceByTime = this.calculateAcceptanceProbTimeConstraints(plan),
				acceptanceByBudget = this.calculateAcceptanceProbBudgetConstraints(plan);
	...
	private double calculateAcceptanceProbBudgetConstraints(TPS_Plan plan) {
		...
		double myFinancialBudgetVariable = plan.getPerson().getBudget() / 30.5; //30.5 days per month in average: 365d/12m
		...
		if (myFinancialBudgetVariable == 2* 99999 / 30) { // TODO integer compared to float?
			...
			if(TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
				TPS_Logger.log(SeverenceLogLevel.DEBUG, "Plan with financial costs=" + plan.getTravelCosts()
						+ " and person's budget=" + myFinancialBudgetVariable + " leads to [ratio=" + ratio
						+ ", acceptance=" + budgetAcceptanceProbability + "]");
			}
		}
	...
	private double calculateAcceptanceProbTimeConstraints(TPS_Plan plan) {
		...
		if(TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
			TPS_Logger.log(SeverenceLogLevel.DEBUG, "Plan with time costs=" + plan.getTravelDuration()
					+ " and person's budget=" + 1.0 / myTimeBudget + " leads to [ratio=" + ratio + ", acceptance="
					+ timeAcceptanceProbability + "]");
		}
	 ...
	 * 
	 * @param plan
	 *            the constructed day plan to be checked for acceptance
	...
	private boolean isPlanAcceptedByOverallTravelTime(TPS_Plan plan) {
		...
		if (Double.compare(0.0, originalDuration)== 0 || Double.compare(0.0, realDuration) == 0 ) {
			...
			ratio = Math.abs(ratio - 1.0);  //even shorter plans should be rejected due to the magic "84min travel time"-postulation
...
//			}
		...
		if(TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
			TPS_Logger.log(SeverenceLogLevel.DEBUG, "Plan with travel times [orig=" + originalDuration + ", real="
					+ realDuration + "] leads to [ratio=" + ratio + ", acceptance=" + acceptance + "] is accepted => "
					+ accepted);
		}
