 ...
 */
	...
	public TPS_DiscreteDistribution<TPS_Mode> getDistributionSet(TPS_ModeSet modeSet, TPS_Plan plan, double distanceNet, TPS_ModeChoiceContext mcc/*TPS_Location locComingFrom, TPS_Location locGoingTo, double startTime, double durationStay, TPS_Car car, boolean fBike, TPS_Stay stay*/) {
		...
		do {
			if(plan.getPerson().getHousehold().getLocation().getTrafficAnalysisZone().getBbrType().getCode(TPS_SettlementSystemType.TAPAS)==2){				
				...
				if(	plan.getPM().getParameters().isTrue(ParamFlag.FLAG_USE_CARSHARING) || plan.getPM().getParameters().isTrue(ParamFlag.FLAG_USE_ROBOTAXI))
					dist.setValueByKey(TPS_Mode.get(ModeType.TRAIN), origDist.getValueByKey(TPS_Mode.get(ModeType.TRAIN)));
			}
