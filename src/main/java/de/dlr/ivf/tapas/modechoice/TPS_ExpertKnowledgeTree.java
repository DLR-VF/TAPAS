    ...
    public static boolean applyExpertKnowledge(TPS_ModeSet modeSet, TPS_Plan plan, double distanceNet, TPS_ModeChoiceContext mcc, boolean checkForbiddenModes, TPS_DiscreteDistribution<TPS_Mode> dist) {
        ...
        if(checkForbiddenModes){
            ...
            for(TPS_Mode.ModeType m : TPS_Mode.MODE_TYPE_ARRAY){
                ...
                double tt = mode.getTravelTime(mcc.fromStayLocation, mcc.toStayLocation, mcc.startTime, SimulationType.SCENARIO, TPS_ActivityConstant.DUMMY, TPS_ActivityConstant.DUMMY, plan.getPerson(), mcc.carForThisPlan);
                if(	TPS_Mode.noConnection(tt)
                    || (!mcc.isBikeAvailable && mode.isType(TPS_Mode.ModeType.BIKE))
                    || (mcc.carForThisPlan==null && mode.isType(TPS_Mode.ModeType.MIT))
                    || (mode.equals(TPS_Mode.ModeType.TAXI) && mode.getParameters().isFalse(ParamFlag.FLAG_USE_TAXI)) //disable TAXI
            ...
            if(changed || numEmpty == TPS_Mode.MODE_TYPE_ARRAY.length){
                if(numEmpty == TPS_Mode.MODE_TYPE_ARRAY.length || !dist.normalize()){
                    if(TPS_Logger.isLogging(HierarchyLogLevel.EPISODE, TPS_LoggingInterface.SeverenceLogLevel.SEVERE)) {
                        TPS_Logger.log(HierarchyLogLevel.EPISODE, TPS_LoggingInterface.SeverenceLogLevel.SEVERE,	"\"Check forbidden modes\" erased all possible modes!");
                    }
                    return false;
                }
            }
        ...
        TPS_DiscreteDistribution<TPS_Mode> mod = modeSet.getExpertKnowledgeTree().getDistributionSet(plan).modifyDistribution(dist);
