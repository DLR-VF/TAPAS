...
public class TPS_ExpertKnowledgeNode extends TPS_Node {
	 ...
	 * mode distribution
	 */
	private TPS_DiscreteDistribution<TPS_Mode> modificationFactor;
	 ...
	 * @param summand
	 *            the summands of this distribution modification
	 * @param factor
	 *            the factors of this distribution modification
	 * @param parent
	 * 			  a link to the parent node
	 */
	public TPS_ExpertKnowledgeNode(int id, TPS_Attribute splitVariable, Collection<Integer> attributeValues,
								   TPS_DiscreteDistribution<TPS_Mode> summand, TPS_DiscreteDistribution<TPS_Mode> factor, TPS_Node parent) {
	...
	public TPS_DiscreteDistribution<TPS_Mode> modifyDistribution(TPS_DiscreteDistribution<TPS_Mode> input){
		TPS_DiscreteDistribution<TPS_Mode> modification = new TPS_DiscreteDistribution<>(
				TPS_Mode.getConstants());
		for(TPS_Mode mode: TPS_Mode.getConstants()) {
			modification.setValueByKey(mode,
					input.getValueByKey(mode)*this.modificationFactor.getValueByKey(mode)
