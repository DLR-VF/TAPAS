...
public class ActionProvider {
    ...
    private final BiFunction<TPS_Episode,Integer, Integer> guard_adaption_function;
    private final TPS_ModeValidator mode_validator;
    private final TazBasedCarSharingDelegator car_sharing_delegator;

    public ActionProvider(TPS_DB_IOManager pm, TPS_TripWriter writer, BiFunction<TPS_Episode,Integer,
    ...
    public List<TPS_PlanStateAction> generateAndGetActivityToTripActions(PlanContext plan_context, TourContext tour_context, Guard trip_to_activity_guard){
        ...
        List<TPS_PlanStateAction> transition_actions = List.of(
                new SetupAvailableModesAction(tour_context, plan_context.getHouseholdCarProvider(), person, pc),
                new SelectLocationAction(tour_context, location_context, plan_context),
                new SelectModeAction(tour_context, plan_context, mode_set),
                new ValidateModeAction(mode_validator, tour_context, pc),
                new UpdateDepartureAndArrivalModeAction(current_located_stay,next_located_stay, mode_context.getNextMode()),
                new CalculateTravelTimeAction(current_located_stay, next_located_stay, next_planned_trip),
                new UpdateTimeDeviationAction(next_trip, next_planned_trip, plan_context),
                new TripPersistenceAction(this.writer,plan_context,tour_context),
                new AdaptGuardAction(trip_to_activity_guard, guard_adaption_function.apply(next_trip, plan_context.getTimeDeviation()))
        );
        ...
        if(!current_stay.isAtHome()) { //we are not at home an leave a location
            //increment capacity
            transition_actions.add(new UpdateCapacityAction(location_context.getCurrentLocation(), 1));
        }else{
            if(next_mode.primary == TPS_Mode.get(TPS_Mode.ModeType.MIT)) //we are leaving home and use a car
                transition_actions.add(new CheckOutHouseholdVehicle(plan_context.getHouseholdCarProvider(), pc.getHouseHoldCar()));
            else
                if(pc.getHouseHoldCar() != null) //a household car has been requested but will not be used
                    transition_actions.add(new ReleaseCarAndResetAction(plan_context.getHouseholdCarProvider(), pc));
        }

        //checkout a potential car-sharing car that has been requested
        if(next_mode.primary == TPS_Mode.get(TPS_Mode.ModeType.CAR_SHARING))
            transition_actions.add(new CheckOutSharingVehicle<>(car_sharing_delegator, location_context.getCurrentLocation().getTAZId(), pc.getCarSharingCar()));
    ...
    public List<TPS_PlanStateAction> generateAndGetTripToActivityActions(PlanContext plan_context, TourContext tour_context, Guard trip_to_activity_guard){
        ...
        if(mode_context.getCurrentMode().primary == TPS_Mode.get(TPS_Mode.ModeType.CAR_SHARING))
            transition_actions.add(new CheckInSharingVehicle<>(car_sharing_delegator, location_context.getCurrentLocation().getTAZId(), pc.getCarSharingCar()));
