...
class TPS_Log4jLogger implements TPS_LoggingInterface {
	...
	private Map<Thread, Map<Class<?>, Logger>> loggerMap;
	...
	private synchronized Logger getLogger(Class<?> callerClass) {
		...
		if (map == null) {
			...
			try{
				String filename ="";
				if(this.parameterClass.isDefined(ParamString.FILE_WORKING_DIRECTORY) && this.parameterClass.isDefined(ParamString.RUN_IDENTIFIER)){
					...
					filename = filename + this.parameterClass.LOG_DIR + this.parameterClass.getString(ParamString.RUN_IDENTIFIER)+System.getProperty("file.separator");
					//mkdir makes the directory only if it NOT exists!
					new File(filename).mkdirs();
					
					
					filename = filename + this.parameterClass.getString(ParamString.RUN_IDENTIFIER) + "-" + host //+ "-" + t.getName()
								+ ".log";
