...
public class RegionPOJO implements Comparable<RegionPOJO>
{
	...
	public static class ModalSplitForDistanceCategory {
		...
		public static class ModalSplitForDistanceCategoryElement {
			...
			public List<DistanceCategory> getDistanceCategories() {
				if (distanceCategories == null)
					distanceCategories = new ArrayList<>();
	...
	public static class ModalSplitForDistanceCategoryAndTripIntention {
		...
		public static class ModalSplitForDistanceCategoryAndTripIntentionElement {
			private List<DistanceCategory>		distanceCategories;
			private Map<TripIntention, Double>	modalSplitTripIntentionSeparated	= new HashMap<>();
			private Map<TripIntention, Long>	modalSplitTripIntentionSeparatedAbs	= new HashMap<>();
			...
			public double getModalSplitTripIntentionSeparated(TripIntention tripIntention) {
				return modalSplitTripIntentionSeparated.containsKey(tripIntention) ? modalSplitTripIntentionSeparated.get(tripIntention) : -1;
			}
			
			public long getModalSplitTripIntentionSeparatedAbs(TripIntention tripIntention) {
				return modalSplitTripIntentionSeparatedAbs.containsKey(tripIntention) ? modalSplitTripIntentionSeparatedAbs.get(tripIntention) : -1;
			}
	 ...
	 * @return
	 */
	public Map<Mode, ModalSplitForDistanceCategoryAndTripIntention> getModalSplitForDistanceCategoryAndTripIntention(boolean[] modalSplitForDistanceCatAndTripIntentionFilterDistCat,
			boolean[] modalSplitForDistanceCatAndTripIntentionFilterTripInt) {
		...
		for (Mode mode : Mode.values()) {
			...
			for (DistanceCategory cat : DistanceCategory.values()) {
				...
				if (element == null)
					element = new ModalSplitForDistanceCategoryAndTripIntentionElement();
				...
				if (!modalSplitForDistanceCatAndTripIntentionFilterDistCat[cat.getId()]) {
					...
					for (TripIntention ti : TripIntention.values()) {
						...
						if (modalSplitForDistanceCatAndTripIntentionFilterTripInt[ti.getId()]) {
							long modalSplitAbsOld = element.containsModalSplitTripIntentionSeparatedAbs(ti) ? element.getModalSplitTripIntentionSeparatedAbs(ti) : 0;
							element.putModalSplitTripIntentionSeparatedAbs(ti, modalSplitAbsOld + modalSplitAbs);
							
							if (!tripsAllTripIntentionSeparated.containsKey(ti))
								tripsAllTripIntentionSeparated.put(ti, 0L);
							for (Mode m : Mode.values()) {
								tripsAllTripIntentionSeparated.put(ti, tripsAllTripIntentionSeparated.get(ti) + distModeTapas[m.getId()][ti.getId()][cat.getId()]);
							}
						} else {
					...
					for (TripIntention ti : TripIntention.values()) {
						...
						if (modalSplitForDistanceCatAndTripIntentionFilterTripInt[ti.getId()]) {
							...
							element.putModalSplitTripIntentionSeparated(ti, saveDiv(modalSplitAbsOld + modalSplitAbs * 100, tripsAllTripIntentionSeparated.get(ti)));
						} else {
							// andernfalls Zähle die Werte für "sonstiges" hoch
	...
	public Map<Mode, ModalSplitForDistanceCategory> getModalSplitForDistanceCategory(boolean[] modalSplitForDistanceCatFilter) {
		...
		for (Mode mode : Mode.values()) {
			...
			for (DistanceCategory cat : DistanceCategory.values()) {
				...
				if (element == null)
					element = new ModalSplitForDistanceCategoryElement();
				element.getDistanceCategories().add(cat);
	...
	private double getModalSplitForDistanceCategory(Mode mode, DistanceCategory cat) {
		...
		if (trips == 0 || tripsAll == 0)
			return 0;// 66.39270042487985+24.968474148802017+12.29344159185774
	...
	public double getModalSplitForTripIntentionAndPersonGroupViseva(Mode m, TripIntention ti, PersonGroup pg) {
		return saveDiv(getModalSplitForTripIntentionAndPersonGroupVisevaAbs(m, ti, pg),getCountOfTripsForTripIntentionAndPersonGroupViseva(ti, pg)) * 100.0;
	...
	public double getModalSplitForTripIntentionAndPersonGroupTapas(Mode m, TripIntention ti, Job pg) {
		...
		return saveDiv(getModalSplitForTripIntentionAndPersonGroupTapasAbs(m, ti, pg),getCountOfTripsForTripIntentionAndPersonGroupTapas(ti, pg)) * 100.0;
