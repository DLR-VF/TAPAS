...
public class CalibrationResultsReader {
	 ...
	 */
	public CalibrationResultsReader(String simkey) throws IOException,
	 ...
	 * 
	 * @param categories
	 *            may be a combination of elements of {@link PersonGroup},
	 ...
	 *             if at least one of the <code>categories</code> are not valid
	...
	public HashMap<Mode, Double> getModalSplit(Enum... categories) {
		...
		// build full query
		String q = "SELECT trip_mode,SUM(cnt_trips) AS sum"
				+ " FROM calibration_results WHERE sim_key = '" + simkey + "' "
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public HashMap<CategoryCombination, Double> getAbsoluteSplit(
		...
		String q = "SELECT "
				+ gc
				+ "SUM(cnt_trips) AS sum FROM calibration_results WHERE sim_key = '"
				+ simkey + "'GROUP BY " + gc.substring(0, gc.length() - 1);
		...
		try {
			...
			while (rs.next()) {
				...
				for (Categories cat : categories) {
					catList.add(cat.getElementById(rs.getInt(cat
							.getCalibrationColumn())));
				}
	...
	public long getCntTrips(Enum... categories) {
		...
		String q = "SELECT SUM(cnt_trips) AS sum"
				+ " FROM calibration_results WHERE sim_key = '" + simkey + "' "
				+ wc;
	...
	public long getCntTrips() {
		String q = "SELECT SUM(cnt_trips) AS sum FROM calibration_results WHERE sim_key = '"
				+ simkey + "'";
	...
	private String buildGroupClause(Categories... categories) {
		...
		for (Categories cat : categories) {
			...
			if (null == columnName) {
				throw new IllegalArgumentException("The category " + cat
	...
	public static void main(String[] args) throws ClassNotFoundException,
			IOException {
		CalibrationResultsReader cr = new CalibrationResultsReader(
				"2013y_03m_07d_16h_43m_41s_859ms");

		System.out.println(cr.getModalSplit());
		System.out.println(cr.getModalSplit(DistanceCategoryDefault.CAT_3));

		System.out.println(cr.getAbsoluteSplit(Categories.Mode,
