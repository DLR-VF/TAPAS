...
public class RegionAnalyzer extends AbstractCoreProcess {
	...
	private EnumMap<Categories, AnalyzerBase> baseAnalyzers;
	private HashMap<EnumSet<Categories>, Analyzer> analyzers;
	private boolean differentiateRegion;

	private EnumMap<RegionCode, GlobalAnalysis> globalAnalysis = new EnumMap<>(RegionCode.class);
	public ArrayList<EnumSet<Categories>> allCategoryCombinations;

	/**
	 * 
	 * @param distanceCategoryFilter
	 *            must have the same length as {@link DistanceCategory}.
	 *            <code>true</code> means, that the category is handled
	 *            separately.
	 * @param differentiateRegion
	 *            <code>true</code> if all occurring regions should be treated
	 *            separately.
	 */
	@SuppressWarnings("unchecked")
	public RegionAnalyzer(boolean[] distanceCategoryFilter,
			boolean differentiateRegion) {
		...
		baseAnalyzers.put(
				...
				new DistanceCategoryAnalyzer(DistanceCategory
						.createFilteredDistances(distanceCategoryFilter)));

		baseAnalyzers
				.put(Categories.TripIntention, new TripIntentionAnalyzer());
		...
		for (EnumSet<Categories> es : allCategoryCombinations) {
			...
			Analyzer tmpAnalyzer = new Analyzer(
					locAnalyzers.toArray(new AnalyzerBase[0]));

			analyzers.put(es, tmpAnalyzer);
		}

		globalAnalysis
				.put(RegionCode.REGION_0,
						new GlobalAnalysis(RegionCode.REGION_0, baseAnalyzers
								.get(Categories.PersonGroup)));
	...
	private ArrayList<EnumSet<Categories>> createAllCombinations() {
		...
		al.add(EnumSet
				.of(Categories.DistanceCategory, Categories.TripIntention));
	...
	/**
	 * @param console
	 *            my be <code>null</code>. Then, only error messages will be
	...
	public boolean finish() throws BadLocationException {
		...
		try {
			...
			if (dbWriter.writeSummary()) {
				if (null != console)
					console.insertString(console.getLength(),"Database export successful.\n",null);
			} else {
				if (null != console)
					console.insertString(console.getLength(),"Database export failed.\n",null);
				else
	...
	private void updateGlobalStatistics(TapasTrip tt) {
		...
		RegionCode region = (RegionCode) baseAnalyzers.get(
				Categories.RegionCode).assignTrip(tt);
		...
		if (globalAnalysis.containsKey(region)) {
			...
			globalAnalysis.put(
					region,
					new GlobalAnalysis(region, baseAnalyzers
							.get(Categories.PersonGroup)));
		}
	 ...
	 * 
	 * @param cat
	 *            e.g. [Categories.RegionCode, Categories.TripIntention]
	 * @param c
	 *            e.g. [RegionCode.REGION_0, TripIntention.TRIP_31]
	 ...
	 * @param category
	 * @param instance
	 *            must be from the category given as parameter
	 * @throws IllegalArgumentException
	 *             if there is no Analyzer considering <code>cat</code>.
	 * @return
	 */
	 ...
	 * @param categories
	 * @param instances
	 *            must be the same length as <code>categories</code> and every
	 *            element must be from the according category.
	 * @throws IllegalArgumentException
	 *             if the combination of <code>categories</code> is not found.
	 */
	...
	public String getDistanceCategoryDescription(DistanceCategory dc) {
		return ((DistanceCategoryAnalyzer) baseAnalyzers
				.get(Categories.DistanceCategory)).getCategoryName(dc);
	}
	...
	public ArrayList<DistanceCategory> getDistanceCategories() {
		return ((DistanceCategoryAnalyzer) baseAnalyzers
				.get(Categories.DistanceCategory)).getDistanceCategories();
	}
