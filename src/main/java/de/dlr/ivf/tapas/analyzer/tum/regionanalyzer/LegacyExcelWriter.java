...
public class LegacyExcelWriter {
	 ...
	 * @throws IOException
	 *             if the Excel workbook could not be initialized. Handle retry!
	 */
	public LegacyExcelWriter(String filename, RegionAnalyzer regionAnalyzer)
			throws IOException {
		this.regionAnalyzer = regionAnalyzer;

		workbook = Workbook.createWorkbook(new File(filename
				+ File.separatorChar + "LegacyTuM.xls"));
		...
		try {
			for (RegionCode region : RegionCode.values()) {
				if (regionAnalyzer.getCntTrips(Categories.RegionCode, region) > 0) {
					...
					WritableSheet wegelaengenSheet = workbook.createSheet(
							region.getName() + " Wegelängen", 0);
					initWegelaengenSheet(wegelaengenSheet);
					WritableSheet modalSplitSheet = workbook.createSheet(
							region.getName() + " Modalsplit", 1);
					initModalsplitSheet(modalSplitSheet);
					WritableSheet persongroupSheet = workbook.createSheet(
							region.getName() + " Personengruppen", 2);
					initPersongroupSheet(persongroupSheet);
					WritableSheet furtherValuesSheet = workbook.createSheet(
							region.getName() + " Weitere Werte", 3);
	...
	private void writeFurtherValuesData(RegionCode region,
			WritableSheet furtherValuesSheet) throws WriteException {
		int x = 4;
		int y = 27;

		WritableCellFormat dc2cell = new WritableCellFormat(NumberFormats.FLOAT);

		furtherValuesSheet.addCell(new Number(3 + xOffset, 5 + yOffset,
				regionAnalyzer.getCntPersons()));
		furtherValuesSheet.addCell(new Number(3 + xOffset, 6 + yOffset,
				regionAnalyzer.getCntTrips(Categories.RegionCode, region)));
		// TODO write region differentiation
		furtherValuesSheet
				.addCell(new Label(3 + xOffset, 7 + yOffset, "" + ""));
		...
		for (TripIntention ti : TripIntention.values()) {
			insRcTi[1] = ti;
			furtherValuesSheet
					.addCell(new Number(x + xOffset, y + yOffset,
							regionAnalyzer.getAvgDistByTrip(catRcTi, insRcTi),
							dc2cell));
			furtherValuesSheet.addCell(new Number(x + 1 + xOffset, y + yOffset,
					regionAnalyzer.getCntTrips(catRcTi, insRcTi)));

			y++;
		}
		//
		furtherValuesSheet.addCell(new Number(x + xOffset, y + yOffset,
				regionAnalyzer.getAvgDistByTrip(Categories.RegionCode, region),
				dc2cell));
		furtherValuesSheet.addCell(new Number(x + 1 + xOffset, y + yOffset,
				regionAnalyzer.getCntTrips(Categories.RegionCode, region)));
		...
		Enum[] insRcDcTi = { region, DistanceCategory.CAT_1,
		...
		for (TripIntention ti : TripIntention.values()) {
			...
			for (DistanceCategory cat : DistanceCategory.values()) {
				...
				furtherValuesSheet.addCell(new Number(x + xOffset, y + yOffset,
						saveDiv(cntDcTi, cntTi) * 100.0, dc2cell));
				furtherValuesSheet.addCell(new Number(x + 1 + xOffset, y
						+ yOffset, cntDcTi));
				y++;
			}
			y++;
		...
		for (DistanceCategory cat : DistanceCategory.values()) {
			...
			furtherValuesSheet.addCell(new Number(x + xOffset, y + yOffset,
					saveDiv(cntRcDc, cntRc) * 100.0, dc2cell));
			furtherValuesSheet.addCell(new Number(x + xOffset, y + yOffset,
					cntRcDc));
			y++;
	...
	private void writePersongroupData(RegionCode region,
			WritableSheet persongroupSheet) throws WriteException {
		int x = 2;
		int y = 5;
		WritableCellFormat dc2cell = new WritableCellFormat(NumberFormats.FLOAT);
		...
		for (PersonGroup pg : PersonGroup.values()) {
			...
			persongroupSheet.addCell(new Number(x, y, saveDiv(cntRcPg, nbP),
					dc2cell));
			persongroupSheet.addCell(new Number(x, y + 1, saveDiv(distRcPg,
					cntRcPg), dc2cell));
	...
	private void writeModalsplitData(RegionCode region,
			WritableSheet modalsplitSheet) throws WriteException {
		int x = 1;
		int y = 6;
		WritableCellFormat dc2cell = new WritableCellFormat(NumberFormats.FLOAT);
		...
		for (Mode m : Mode.values()) {
			...
			modalsplitSheet.addCell(new Number(x + xOffset, y + yOffset,
					saveDiv(cntRcMo, cntRc) * 100, dc2cell));
			y++;
		}
		y += 2;
		...
		Categories[] catRcMoDc = { Categories.RegionCode, Categories.Mode,
				Categories.DistanceCategory };
		...
		for (Mode mo : Mode.values()) {
			...
			for (DistanceCategory dc : DistanceCategory.values()) {
				instRcMoDc[2] = dc;
				long cntRcMoDc = regionAnalyzer.getCntTrips(catRcMoDc,
						instRcMoDc);
				modalsplitSheet.addCell(new Number(x + xOffset, y + yOffset,
						saveDiv(cntRcMoDc, cntRcMo) * 100, dc2cell));
				y++;
			}
			y++;
		}
		...
		Categories[] catRcMoDcTi = { Categories.RegionCode, Categories.Mode,
				Categories.DistanceCategory, Categories.TripIntention };
		Enum[] instRcMoDcTi = { region, Mode.BIKE, DistanceCategory.CAT_1,
				TripIntention.TRIP_31 };
		...
		for (Mode mo : Mode.values()) {
			for (DistanceCategory dc : DistanceCategory.values()) {
				...
				for (TripIntention ti : TripIntention.values()) {
					instRcMoDcTi[3] = ti;
					long cntRcMoDcTi = regionAnalyzer.getCntTrips(catRcMoDcTi,
							instRcMoDcTi);
					modalsplitSheet.addCell(new Number(x + xOffset,
							y + yOffset, saveDiv(cntRcMoDcTi, cntRcMoDc) * 100,
							dc2cell));
					y++;
				}
				y++;
	...
	private void writeWegelaengenData(RegionCode region,
			WritableSheet wegelaengenSheet) throws WriteException {
		int x = 3;
		int y = 27;
		WritableCellFormat dc2cell = new WritableCellFormat(NumberFormats.FLOAT);

		wegelaengenSheet.addCell(new Number(x + xOffset, 5 + yOffset,
				regionAnalyzer.getCntPersons()));
		wegelaengenSheet.addCell(new Number(x + xOffset, 6 + yOffset,
				regionAnalyzer.getCntTrips(Categories.RegionCode, region)));
		wegelaengenSheet
				.addCell(new Label(x + xOffset, 7 + yOffset, "" + ""/* regionalDifferentiation */));
		...
		wegelaengenSheet.addCell(new Number(x + xOffset, y + yOffset,
				regionAnalyzer.getAvgDistByTrip(Categories.RegionCode, region),
				dc2cell));
	...
	private void initFurtherValuesSheet(WritableSheet writableSheet)
		...
		writableSheet.addCell(new Label(x, y++,
				"Wegelängendifferenzierung verwendet"));
		...
		writableSheet.addCell(new Label(x, y++,
				"Durchschnittliche Weglängen in Metern"));
		for (TripIntention ti : TripIntention.values()) {
		...
		writableSheet.addCell(new Label(x + 4, y, "Anzahl Trips"));
		writableSheet.addCell(new Label(x, y++,
				"Ergebnisse nach Wegezwecken in Prozent"));
	...
	private void initPersongroupSheet(WritableSheet writableSheet)
			throws WriteException {
		int x = 0 + xOffset;
		int y = 1 + yOffset;
		writableSheet.addCell(new Label(x, y++, "Region"));
		...
		for (PersonGroup pg : PersonGroup.values()) {
			writableSheet.addCell(new Label(x, y++, pg.name() + ": "
					+ pg.getName()));
	...
	private void initModalsplitSheet(WritableSheet writableSheet)
			throws WriteException {
		int x = 0 + xOffset;
		int y = 1 + yOffset;
		writableSheet.addCell(new Label(x, y++, "Region"));
		...
		for (Mode m : Mode.values()) {
			for (DistanceCategory cat : DistanceCategory.values()) {
				writableSheet.addCell(new Label(x, y++, m.getDescription()
						+ " - "
						+ regionAnalyzer.getDistanceCategoryDescription(cat)));
			}
			y++;
		}
		y++;
		writableSheet.addCell(new Label(x, y++,
				"Modal Split pro Wegelänge und Wegezweck"));
		for (Mode m : Mode.values()) {
			for (DistanceCategory cat : DistanceCategory.values()) {
				String catName = regionAnalyzer
						.getDistanceCategoryDescription(cat);
				for (TripIntention ti : TripIntention.values()) {
					writableSheet.addCell(new Label(x, y++, m.getDescription()
							+ " - " + catName + " - " + ti.getCaption()));
				}
				// writableSheet.addCell(new Label(x, y++, m.getDescription()
	...
	private void initWegelaengenSheet(WritableSheet writableSheet)
			throws WriteException {
		int x = 0 + xOffset;
		int y = 1 + yOffset;
		writableSheet.addCell(new Label(x + 2, y++, "SOLL"));
	...
	private void addCNF4Config(int x, int y, WritableSheet writableSheet)
			throws WriteException {
		x++;
	...
	private void addDistanceClasses(int x, int y, WritableSheet sheet)
			throws WriteException {
