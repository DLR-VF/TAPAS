...
public class CategoryCombination {
	 ...
	 * 
	 * @param categories
	 *            combination of different enum entries. The order is important
	...
	public CategoryCombination(Enum... categories) {
		...
		for (Enum cat : categories) {
			if (!Categories.isValid(cat)) {
				throw new IllegalArgumentException(cat.getClass().getName()
						+ " is no valid category.");
			}
	...
	public boolean contains(Categories category) {
		for (Enum cat : categories) {
			if (Categories.getByClass(cat.getClass()) == category)
				return true;
		}
		return false;
	}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof CategoryCombination))
			return false;
		CategoryCombination cc = (CategoryCombination) obj;
		if (cc.categories.length != this.categories.length)
			return false;
	 ...
	 * @param categories
	 */
	public static ArrayList<CategoryCombination> listAllCombinations(
		...
		while (idx[nCat - 1] < catLength[nCat - 1]) {
			...
			if (idx[nCat - 1] < catLength[nCat - 1]) {
				// fill array
				for (int i = 0; i < nCat; i++) {
					tmpCats[i] = (Enum) categories[i].getEnumeration()
							.getEnumConstants()[idx[i]];
				}
