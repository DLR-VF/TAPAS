...
public class Analyzer {
	...
	private HashMap<CategoryCombination, TripStats> statsMap = new HashMap<>();
	...
	public void addTrip(TapasTrip trip) {
		if(trip != null){	//avoids {@link NullPointerException} from being thrown when 
			...
			if (statsMap.containsKey(cc))
				statsMap.get(cc).add(trip);
			else
				statsMap.put(cc, new TripStats(trip));
		}
	}
	 ...
	 * 
	 * @param c
	 *            the categories for all but the last category.
	 */
	public ArrayList<TASplitData> getSplit(Enum... c) {

		boolean empty = true;
		ArrayList<CategoryCombination> allCat = CategoryCombination
				.listAllCombinations(getCategories());
		...
		for (TripStats ts : stats) {
			if (ts.getCntTrips() == 0) {
				...
				result.add(new TASplitData((double) ts.getCntTrips()
						/ total.getCntTrips(), ts.getCntTrips(), ts.getDist()
						/ ts.getCntTrips()));
				empty = false;
			}
		}
	...
	public int getCntPersons(Enum[] e) {
		...
		return ((PersonGroupAnalyzer) analyzers[idxPersonGroup])
				.getCntPersons(cc);
