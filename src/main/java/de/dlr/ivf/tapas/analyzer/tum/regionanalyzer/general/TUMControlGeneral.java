...
public class TUMControlGeneral implements ControlInputInterface {
	...
	private static class AnalyzerTransferable implements Transferable {
		public static final DataFlavor ANALYZER_DATA_FLAVOR = new DataFlavor(
				AnalyzerBase.class, "Analyzer");
	...
	private class NodeListTransferHandler extends TransferHandler {
		...
		public boolean canImport(TransferSupport support) {
			return support
					.isDataFlavorSupported(AnalyzerTransferable.ANALYZER_DATA_FLAVOR);
		}
		...
		public boolean importData(TransferSupport support) {
			if (canImport(support)) {
				...
				try {
					AnalyzerBase analyzer = (AnalyzerBase) transferable
							.getTransferData(AnalyzerTransferable.ANALYZER_DATA_FLAVOR);
		...
		protected void exportDone(JComponent source, Transferable data,
			...
			DefaultListModel<AnalyzerBase> model = (DefaultListModel<AnalyzerBase>) activeAnalyzerList
					.getModel();
		...
		public boolean importData(TransferSupport support) {
			if (canImport(support)) {
				...
				try {
					...
					DefaultTreeModel model = (DefaultTreeModel) ((JTree) support
							.getComponent()).getModel();

					node = (DefaultMutableTreeNode) path.getPathComponent(path
							.getPathCount() - 1);

					if (childIndex < 0) {
						AnalyzerCollection col = (AnalyzerCollection) node
								.getUserObject();
						col.addAnalyzer(analyzer);
						model.nodeChanged(node);
					} else {
						model.insertNodeInto(new DefaultMutableTreeNode(
								new AnalyzerCollection(analyzer)), node,
								childIndex);
					}
				} catch (UnsupportedFlavorException | IOException e) {
					e.printStackTrace();
	...
	public JComponent getComponent() {
		...
		moduleMatrix.setBorder(new TitledBorder(new LineBorder(new Color(46,
				90, 214), 2, true), "TUM", TitledBorder.LEADING,
	...
	private JPanel getControlListPanel() {
		...
		JButton btnDelete = makeConstantWidthButton("Delete",
				new AbstractAction() {
					/**
					 * 
					 */
		...
		JButton btnUpdate = makeConstantWidthButton("Update",
				new AbstractAction() {
					/**
					 * 
					 */
	...
	private JPanel getControlTreePanel() {
		...
		JButton btnLoadTree = makeConstantWidthButton("Load",
				new AbstractAction() {
					/**
					 * 
					 */
		...
		btnLoadTree
				.setToolTipText("Läd einen Analyzer-Baum aus einer .xml Datei.");
		panel.add(btnLoadTree);

		JButton btnSaveTree = makeConstantWidthButton("Save",
				new AbstractAction() {
					/**
					 * 
					 */
		...
		btnSaveTree
				.setToolTipText("Speichert den Analyzer-Baum in eine .xml Datei.");
	...
	private void initBaseList() {
		...
		DefaultListModel<AnalyzerBase> model = (DefaultListModel<AnalyzerBase>) baseAnalyzerList
				.getModel();
	...
	private void initActiveList() {
		...
		activeAnalyzerList
				.setToolTipText("<html>Es wird nach der letzten Kategorie<br>"
						+ "gesplittet in der gegebenen Reihenfolge.");
		String del = "DEL";
		activeAnalyzerList.getActionMap().put(del, new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (idx > -1) {
					((DefaultListModel) activeAnalyzerList.getModel())
							.remove(idx);
				}
			}
		});
		activeAnalyzerList.getInputMap().put(
	...
	private JButton makeConstantWidthButton(String text, Action a) {
		...
		button.setAction(a);
		button.setText(text);
		button.setPreferredSize(new Dimension(BUTTON_WIDTH, button
	...
	private JPanel createGUI() {
		...
		gbc = new GridBagConstraints(2, 1, 1, 1, 0.5, 0,
				GridBagConstraints.FIRST_LINE_START,
		...
		gbc = new GridBagConstraints(1, 4, 2, 1, 0, 0,
				GridBagConstraints.FIRST_LINE_START, GridBagConstraints.NONE,
				zeroInset, 2, 0);
	...
	private AnalyzerBase getAnalyzer(Categories c) {
		...
		Enumeration<AnalyzerBase> en = ((DefaultListModel<AnalyzerBase>) baseAnalyzerList
				.getModel()).elements();

		while (en.hasMoreElements()) {
			AnalyzerBase a = en.nextElement();
			if (a.getCategories() == c)
				return a;
		}
	...
	private void updateNode(boolean add) {
		...
		DefaultListModel<AnalyzerBase> analyzers = (DefaultListModel<AnalyzerBase>) activeAnalyzerList
				.getModel();
	...
	private void loadSelectedNodeInfo() {
		...
		((DefaultListModel<AnalyzerBase>) activeAnalyzerList.getModel())
				.clear();
	...
	private void deleteSelectedTreeNode() {
		...
		treeModel.removeNodeFromParent((DefaultMutableTreeNode) path
				.getLastPathComponent());
	...
	private void initTree() {
		...
		tree = new JTree(treeModel) {
			...
			public String convertValueToText(Object value, boolean selected,
				...
				return getLevelAddition((DefaultMutableTreeNode) value)
						+ super.convertValueToText(value, selected, expanded,
								leaf, row, hasFocus);
	...
	private void addDefaultAnalyzers() {
		...
		DefaultMutableTreeNode tab = new DefaultMutableTreeNode(
				new AnalyzerCollection("Wegelängen",
						getAnalyzer(Categories.RegionCode)));
		file.add(tab);
		...
		tab = new DefaultMutableTreeNode(new AnalyzerCollection(
				"Personengruppen", getAnalyzer(Categories.RegionCode)));
		file.add(tab);
		analyzers = new DefaultMutableTreeNode(new AnalyzerCollection(
				getAnalyzer(Categories.PersonGroup)));
		tab.add(analyzers);
	...
	public void loadTree() {
		...
		if (JFileChooser.APPROVE_OPTION == fc.showOpenDialog(null)) {
			...
			try {
				...
				DefaultMutableTreeNode newRoot = GeneralAnalyzerTreeExporter
						.readTree(file.getAbsolutePath());
				root.removeAllChildren();
				for (int i = 0; i < newRoot.getChildCount(); i++) {
					treeModel.insertNodeInto(
							(DefaultMutableTreeNode) newRoot.getChildAt(i),
							root, i);
				}
			} catch (IOException e) {
				e.printStackTrace();
				System.err.println("Could not load tree from "
	...
	public void saveTree() {
		...
		if (JFileChooser.APPROVE_OPTION == fc.showSaveDialog(null)) {
			File file = fc.getSelectedFile();
			try {
				if (GeneralAnalyzerTreeExporter.writeTree(root,
						file.getAbsolutePath())) {
					System.out.println("Tree successfully saved to "
							+ file.getAbsolutePath());
				} else {
					System.err.println("Could not save tree to "
							+ file.getAbsolutePath());
				}
