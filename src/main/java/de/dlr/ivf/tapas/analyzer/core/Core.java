...
public class Core extends Thread {
	...
	private Collection<CoreProcessInterface> modules = new HashSet<>();
	...
	// information outputs
	private JTextPane cons = new JTextPane();
	...
	public static String getRuntime() {
		return "("
				+ (System.nanoTime() - systemTime) / 1000000
				+ "ms)";
	}
	...
	public void run(){
		...
		try{
		console.insertString(console.getLength(),"Core [" + Core.currentThread().getId() + "]"
				+ getRuntime() + ": Gestartet \n",null);
		boolean first = true;
		...
		if (modules.size() > 0 && tripReader.getIterator().hasNext()) {
			...
			if (!isCancelled) {
				console.insertString(console.getLength(),"Core [" + Core.currentThread().getId() + "]"
						+ getRuntime() + ": Starte it. Auswertungsmodule \n",null);
			...
			if (!isCancelled) {
				console.insertString(console.getLength(),"Core [" + Core.currentThread().getId() + "]"
						+ getRuntime() + ": Starte fin. Auswertungsmodule \n",null);
				for (CoreProcessInterface cii : modules) {
					Thread tii = new Thread(cii,
							"Thread for ControlInputInterface implementation: "
									+ cii.getClass().getSimpleName());
					tii.start();
				}
			}
		} else {
			...
			if (!tripReader.getIterator().hasNext())
				infoString = infoString + "Kein Tripfile angegeben.\n";
			if (modules.size() == 0)
				infoString = infoString + "Keine Module angegeben.\n";
			JOptionPane.showMessageDialog(new JPanel(), infoString, "Info",
