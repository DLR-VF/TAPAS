...
public class DatabaseSummaryExport {
    ...
    private void fillBatch(PreparedStatement ps) {
        if (regionAnalyzer.isRegionDifferentiated()) return;
        String simkey = regionAnalyzer.getSource();

        Categories[] cats = {Categories.Mode, Categories.PersonGroup, Categories.TripIntention, Categories.DistanceCategory};
        Enum[] instances = {RegionCode.REGION_0, Mode.BIKE, PersonGroup.PG_1, TripIntention.TRIP_31, DistanceCategory.CAT_1};

        ArrayList<CategoryCombination> combinations = CategoryCombination.listAllCombinations(cats);

        Categories[] catsR = {Categories.RegionCode, Categories.Mode, Categories.PersonGroup, Categories.TripIntention, Categories.DistanceCategory};
        ...
        for (CategoryCombination cc : combinations) {
            ...
            int cntPersons = regionAnalyzer.getCntPersons((PersonGroup) instances[2]);
