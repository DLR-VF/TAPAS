...
public class TPS_ParameterFileConverterGUI {
	...
	public static void main(String[] args) {
		try {
			...
			System.err
					.println("Unable to set specific look and feel: com.sun.java.swing.plaf.windows.WindowsLookAndFeel");
			e.printStackTrace();
		}
	...
	private SortedMap<String, String> configMap = new TreeMap<>();

	private SortedMap<String, String> defaultMap = new TreeMap<>();

	private JFrame frame;

	private String[] header = new String[] { "Key", "Default Value", "", "Config Value", "" };
	...
	public TPS_ParameterFileConverterGUI(TPS_ParameterClass parameterClass) {
		...
		this.table = new ConfigurationTable(new DefaultTableModel(
				new String[0][header.length], header));

		readClientProperties();

		this.init();

		this.frame.setSize(1116, (int) Toolkit.getDefaultToolkit()
				.getScreenSize().getHeight() - 30);
		this.frame.setResizable(false);
	}

	private void readClientProperties() {
		mClientControlProperties = new ClientControlProperties(new File(
				"converter.properties"));
		if (System.getProperty("os.name").toLowerCase().contains("windows")) {
			tapasNetworkDirectory = new File(
					mClientControlProperties
							.get(ClientControlPropKey.TAPAS_DIR_WIN));
		} else {
			tapasNetworkDirectory = new File(
					mClientControlProperties
							.get(ClientControlPropKey.TAPAS_DIR_LINUX));
		}
		...
		if (!parameterClass.isDefined(ParamString.DB_USER)
				|| !parameterClass.isDefined(ParamString.DB_PASSWORD)) {
			String loginFile = "runtime.csv";

			if (!this.mClientControlProperties.get(
					ClientControlPropKey.LOGIN_CONFIG).isEmpty()) {
				loginFile = this.mClientControlProperties
						.get(ClientControlPropKey.LOGIN_CONFIG);
			} else {
				this.mClientControlProperties.set(
						ClientControlPropKey.LOGIN_CONFIG, loginFile);
			}

			File runtimeFile = new File(new File(tapasNetworkDirectory,
					this.parameterClass.SIM_DIR), loginFile);
			try {
				this.parameterClass.loadRuntimeParameters(runtimeFile);

			} catch (Exception e) {
				System.err.println("Exception thrown during reading "
						+ loginFile);
				e.printStackTrace();
			}
		}
	...
	private void loadConfigFromDatabase() {
		...
		try {
			...
			if (simKey != null) {
				...
				resultSet = statement.executeQuery("SELECT * FROM "+this.parameterClass.getString(ParamString.DB_TABLE_SIMULATION_PARAMETERS)+" WHERE sim_key = '"+mSimKey+"' order by param_key");
				configMap.clear();
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(frame, e.getMessage(), "Loading config file failed!", JOptionPane.ERROR_MESSAGE);
		} finally {
			if (con != null)
				try {
					con.close();
	...
	private SortedMap<String, String> getDefaultParams() {
		...
		for (ParamString paramString : ParamString.values()) {
			if (this.parameterClass.paramStringClass.getPreset(paramString)!=null && !this.configMap.containsKey(paramString.name()))
				params.put(paramString.name(), this.parameterClass.paramStringClass.getPreset(paramString));
		}
		for (ParamValue paramValue : ParamValue.values()) {
			if (this.parameterClass.paramValueClass.getPreset(paramValue)!=null && !this.configMap.containsKey(paramValue.name()))
				params.put(paramValue.name(), this.parameterClass.paramValueClass.getPreset(paramValue).toString());
		}
		for (ParamFlag paramFlag : ParamFlag.values()) {
			if (this.parameterClass.paramFlagClass.getPreset(paramFlag)!=null && !this.configMap.containsKey(paramFlag.name()))
				params.put(paramFlag.name(), this.parameterClass.paramFlagClass.getPreset(paramFlag).toString());
		}
		return params;
	}
	...
	private void saveConfigInDatabase() {
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(frame, e.getMessage(), "Saving config file failed!", JOptionPane.ERROR_MESSAGE);
		} finally {
			if (con != null)
				try {
					con.close();
	...
	private String getNewSimKey() {
		...
		int msOffset = 0;
		return c.get(Calendar.YEAR) + "y_" + f00.format(c.get(Calendar.MONTH) + 1) + "m_" + f00.format(c.get(Calendar.DAY_OF_MONTH)) + "d_" + f00.format(c.get(Calendar.HOUR_OF_DAY)) + "h_"
				+ f00.format(c.get(Calendar.MINUTE)) + "m_" + f00.format(c.get(Calendar.SECOND)) + "s_" + f000.format(c.get(Calendar.MILLISECOND) + msOffset) + "ms";
	}
	private void updateConfigParamsInEnum() {
		...
		for (int i = 0; i < table.getRowCount(); i++) {
			...
			if (value != null && value.toString().length() > 0)
				this.configMap.put(key, value.toString());
		}
	}
	...
	private boolean isSimulationStartedAlready(Connection con) throws SQLException{
		...
		ResultSet resultSet =
				statement.executeQuery("SELECT sim_started, sim_finished FROM "+this.parameterClass.getString(ParamString.DB_TABLE_SIMULATIONS)+" WHERE sim_key = '"+mSimKey+"'");
		while (resultSet.next()) {
	...
	private String chooseSimulation(Connection con) throws SQLException {
		...
		try (Statement statement = con.createStatement()) {
			...
			return (String) JOptionPane.showInputDialog(frame, "Wähle die Simulation aus:\n",
					"Wähle Simulation", JOptionPane.PLAIN_MESSAGE, null/* icon */, simKeys.toArray(), simKeys.get(0));
		}
...
//				parameterClass.clear();
				...
				if (fieldPath.getText().length() > 0 && fieldConfig.getText().length() > 0) {
					File folder = new File(fieldPath.getText(), fieldConfig.getText().replace(".xls", "").replace(".csv", ""));
					if (!folder.exists())
						folder.mkdir();
					file = new File(folder, fieldConfig.getText());
				} else {
					file = TPS_ParameterFileConverter.chooseFileToSave(new File(fieldPath.getText(), fieldConfig.getText()), "Save config file", JFileChooser.FILES_ONLY, new ExtensionFilter(
							"CSV-File", ".csv"), new ExtensionFilter("Excel-File", ".xls"));
				}
				...
				if (file != null) {
					...
					if (file.getAbsolutePath().endsWith(".xls")) {
						...
						converter.setFile(file);
						converter.addAdditionalParameter(ParamString.FILE_PARENT_PROPERTIES.name(), parent, "link to default file");
...
//				parameterClass.clear();
				...
				for (int i = 0; i < table.getRowCount(); i++) {
					value = table.getValueAt(i, 3);
					if (value != null && value.toString().length() > 0)
						parameterClass.setValue(table.getValueAt(i, 0).toString(), value.toString());
					else{
				...
				if (fieldPath.getText().length() > 0 && fieldConfig.getText().length() > 0) {
					...
					// Wenn eines der beiden Textfelder leer ist (normalerweise sind dann beide leer), wurde die Config aus der Datenbank geladen
					// Der Nutzer muss nun einen Speicherort wählen
					file = TPS_ParameterFileConverter.chooseFileToSave(new File("T:/TAPAS"), "Save config file", JFileChooser.FILES_ONLY, new ExtensionFilter("CSV-File", ".csv"), new ExtensionFilter(
							"Excel-File", ".xls"));
					if (file != null) {
						fieldPath.setText(file.getParent());
						fieldConfig.setText(file.getName());
	...
	private int insert(String key, ParamType type, int row) {
		if (type.equals(ParamType.EXEC) || type.equals(ParamType.TMP))
			return row;
