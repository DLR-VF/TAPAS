...
public class UniqueStreetIdentifier extends TPS_BasicConnectionClass {
	...
	private static HashMap<HashSet<String>, HashSet<String>> combineHsn(
			HashMap<String, HashSet<String>> hsn,
	...
	private static ArrayList<ArrayList<String>> combineStreets(
			HashMap<PlzPair, Double> input) {
	...
	public static void main(String[] args) {
		...
		try {
			PreparedStatement getStreetParts = ui.dbCon
					.getConnection(ui)
					.prepareStatement(
							"WITH strassen AS ( "
									+ " SELECT strasse,plz,ARRAY_AGG(DISTINCT hausnummer) AS hsn, "
									+ " ST_CONVEXHULL(ST_COLLECT(the_geom)) AS the_geom "
									+ " FROM core.berlin_buildings "
									+ " WHERE bundesland = '11' AND strasse = ? "
									+ " GROUP BY strasse,plz) "
									+ " SELECT s1.strasse, s1.plz AS plz1, s2.plz AS plz2, s1.hsn AS hsn1, s2.hsn AS hsn2,  "
									+ " ST_DISTANCE(s1.the_geom,s2.the_geom, false) AS dist "
									+ " FROM strassen s1 "
									+ " JOIN strassen s2 ON s1.plz < s2.plz");

			ResultSet rs = ui.dbCon.executeQuery(
					"SELECT strasse FROM core.berlin_buildings "
							+ " WHERE bundesland = '11' "
							+ " GROUP BY strasse "
...
//					System.out.println(str);
				...
				while (rs2.next()) {
					...
					if (!plzHsn.containsKey(plz1))
						plzHsn.put(plz1, hn1);
					if (!plzHsn.containsKey(plz2))
						plzHsn.put(plz2, hn2);
