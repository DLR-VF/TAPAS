...
public class TPS_SpeedAndDistanceValidator extends TPS_BasicConnectionClass {
	...
	class TravelData{
		...
		double xVal=0, //usually this is distance 
			   beeLine=0, 
			   yVal=0; //usually this is time
		
		public String toString(){
			return 	"from: "+from+" to: "+to+" dist: "+xVal+
					" travel time: "+yVal+" beeline: "+beeLine+
					" speed: "+this.getKMH()+ " ratio: "+(xVal/beeLine);
		}
		...
		for (int i=0; i<this.data.length; ++i){
			for(int j=0; j< this.data[i].length; ++j){
				if(i==j | this.data[i][j].yVal<=0)
					continue;
				if(this.data[i][j].xVal>distMax | this.data[i][j].xVal<distMin)
					continue;
				if(speed)
					val = this.data[i][j].getKMH();					
				else
					val = this.data[i][j].xVal/this.data[i][j].beeLine;
	...
	public Collection<ScatterData> calchScatterPlotData(double speedBin, double distBin, boolean speed){
		...
		for (int i=0; i<this.data.length; ++i){
			for(int j=0; j< this.data[i].length; ++j){
				...
				if(	(
						distCell*distBin ==11560 ||
						distCell*distBin ==11910
					)	
					&& speedCell*speedBin==3850) {
...
//				TPS_BasicConnectionClass.matrixToStringWriterSQL(egt, 0)+" where matrix_name='PT_VISUM_1193_CALCULATED_AUG_2015_ACT_INTERMOBILE'");
		...
		TPS_SpeedAndDistanceValidator worker = new TPS_SpeedAndDistanceValidator();
		worker.loadTAZInfos("core."+region+"_taz_1223", "core."+region+"_matrices", distMatrix, timeMatrix,accessName,egressName);
		...
		System.out.format("Dist:\t%6.2f\t%6.2f\t%6.2f\t%6.2f\n",
							...
							distStat[STATVAL.MAX.ordinal()],
							distStat[STATVAL.AVG.ordinal()],
							distStat[STATVAL.STDDEV.ordinal()]);
		
		System.out.format("Speed:\t%6.2f\t%6.2f\t%6.2f\t%6.2f\n",
				speedStat[STATVAL.MIN.ordinal()],
				speedStat[STATVAL.MAX.ordinal()],
				speedStat[STATVAL.AVG.ordinal()],
