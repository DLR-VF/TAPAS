...
//				"tpsModeDistribution.betaDistanceCostsSquareOEVBudget");
	...
	IndexedColors[] indexedColors = new IndexedColors[] { IndexedColors.LIGHT_CORNFLOWER_BLUE,
			IndexedColors.LIGHT_GREEN, IndexedColors.WHITE };
	...
	public void save(File file, TPS_ParameterClass parameterClass, String... parameters) {
		...
		ParamString[] params = new ParamString[] { 
				ParamString.FILE_DATABASE_PROPERTIES, ParamString.FILE_PARAMETER_PROPERTIES,
				ParamString.FILE_LOGGING_PROPERTIES };
		...
		try {
			...
			for (String line = breader.readLine(); line != null; line = breader.readLine()) {
				if (line.startsWith("#")) {
					...
					if (defined) {
						String text = "Parameter " + e.name() + " is already defined with the default value '"
								+ definition + "'. Would you like to overwrite this parameter with the new value '"
								+ entry[1] + "'?";
	...
	static File chooseFileToSave(File path, String title, int type, ExtensionFilter... fileFilter) {
		...
		if (selectedFile != null)
			if (!selectedFile.toString().endsWith(extension)) {
	...
	private static JFileChooser createDialog(File path, String title, int type, boolean multisel, boolean save, javax.swing.filechooser.FileFilter... fileFilter) {
		...
		fd.setVisible(true);
		if (fileFilter != null)
			for (javax.swing.filechooser.FileFilter fileFilterElement : fileFilter) {
