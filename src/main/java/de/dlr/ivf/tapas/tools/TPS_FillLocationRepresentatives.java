...
public class TPS_FillLocationRepresentatives extends TPS_BasicConnectionClass{
		...
		try{
			...
			for(Integer i : tazIds){				
				query = "WITH adr as (SELECT gid, st_transform(geom,4326) as geom FROM "+addressTable+") "
						+ "SELECT gid, st_X(geom) as x, st_Y(geom) as y FROM adr WHERE st_within(geom, (SELECT the_geom from "+tazTable+" WHERE gid = "+i+")) order by random() LIMIT "+samples;
				ResultSet rs= this.dbCon.executeQuery(query, this);	
				...
				if(representativesFound==0) { // no address in this TAZ
					...
					query = "SELECT st_X(st_transform(st_centroid(the_geom),4326)) as x, st_Y(st_transform(st_centroid(the_geom),4326)) as y FROM "+tazTable+" WHERE gid= "+i;
	...
	public void saveToDB(String representativesTable){
		...
		for(Representative i : representatives){
			query = "INSERT INTO "+representativesTable+" VALUES ("+i.id+","+i.taz+",st_setsrid(st_makepoint("+i.x+","+i.y+"),4326))";
			this.dbCon.execute(query, this);
		}
	}
	...
	public void createRepresentativesTable(String representativesTable) {
		...
		String query =
				"DROP TABLE IF EXISTS "+representativesTable+";"
				+ "CREATE TABLE "+representativesTable
				+ "				("
				+ "						  id integer NOT NULL,"
				+ "						  taz_id integer,"
	...
	public static void main(String[] args) {
		if(args.length!=4){
			System.out.println("Usage: <taz multiline table> <address table> <output: representatives table> <samples per taz>");
			return;
		}
		TPS_FillLocationRepresentatives worker = new TPS_FillLocationRepresentatives("T:\\Simulationen\\runtime_herakles_admin.csv");
