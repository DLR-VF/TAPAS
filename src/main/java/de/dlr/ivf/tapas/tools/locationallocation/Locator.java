...
public class Locator {
	...
	private void updateLocation(ResultSet rs, Location loc) throws SQLException {
		...
		if (Constants.ENABLE_CHANGES_OF_MARCO && loc.isUpdatedByDB()) {
			...
			if (!loc.getValue("Straße").equals(streetValue)) {
				loc.setStatus(LocationProcessor.Location.STATUS_NOT_DISTINCT_IN_DATABASE);
				return;
			}
	...
	private void generateQueriesForAdressToCoordinate(State bundesland, String streetValue, String houseNumber, String plz, String houseNumberAddOn, String additionalConstraints, String additionalColumns, String order, HashMap<Integer,String> queries, int startSqlIndex) {
		...
		if (Constants.ENABLE_CHANGES_OF_MARCO) {			
			...
			if (houseNumberAddOn != null && houseNumberAddOn.length() > 0) {
				...
				if (streetValue != null || plz != null || houseNumberAddOn != null || bundesland != null || houseNumber != null)
					query += "	where ";
				whereAdded = false;
				if (streetValue != null) {
			...
			String region;
			if(bundesland!=null)
				region =" AND bundesland LIKE '" + bundesland.getGemeindeSchluessel() + "' ";
			else
				region ="";
			if (plz != null) {
				if (houseNumber != null) {
					if (houseNumberAddOn != null && houseNumberAddOn.length() > 0) {
						query = "select * from (" + 
								"	select 	id, strasse, hausnummer,hausnummerzusatz,plz, st_X(the_geom) as x, st_Y(the_geom) as y " + additionalColumns +
								"	from core.berlin_buildings " + 
								"	where lower(strasse)='" + streetValue + "' " + 
								"   AND lower(plz)='" + plz + "' " + 
								"   AND lower(hausnummer)='" + houseNumber + "' "+ 
								"   AND lower(hausnummerzusatz)='" + houseNumberAddOn + "'" + 
								region+
								additionalConstraints+
								") as foo "+order;
						queries.put(startSqlIndex+2,query);
	...
	public boolean localizeLocation(Location loc) {
		...
		if (coord_x != null)
			coord_x = coord_x.replaceAll(",", ".");
		if (coord_y != null)
			coord_y = coord_y.replaceAll(",", ".");
		...
		for (int i = 0; i < 1 && resultNotFound; ++i) {
			...
			try {
				if (coord_x != null && coord_y != null) {
					geometricConstruct = "ST_Distance_Sphere(st_setsrid(st_makepoint(" + coord_x + "," + coord_y + "),4326),the_geom)";
				...
				if (streetValue == null) {
					...
					if (coord_x != null && coord_y != null) {// LIKE
						...
						if (plz != null) {
							query = "select * from (" + 
									"	select 	id, strasse, hausnummer,hausnummerzusatz,plz, st_X(the_geom) as x, st_Y(the_geom) as y, " + geometricConstruct+ " as distancetopoint " + 
									"	from core.berlin_buildings " + 
									"	where lower(plz) = '" + plz + "'" +
									geometricCheck+
									regionalCheck+
					...
					this.generateQueriesForAdressToCoordinate(State.getByName(stateName), streetValue, null,
							plz.toString(), null, geometricCheck,", "  + geometricConstruct+ " as distancetopoint ", "order by distancetopoint", queries,8); // generates 13 more queries
					...
					this.generateQueriesForAdressToCoordinate(stateName==null?null:State.getByName(stateName.toUpperCase()), streetValue, houseNumber,
							plz.toString(), houseNumberAddOn, geometricCheck,", "  + geometricConstruct+ " as distancetopoint ", "order by distancetopoint", queries,35); // generates 13 more queries
				...
				for (int j = 0; j < 63 && resultNotFound; ++j) {
					if(queries.containsKey(j)){
						...
						if(numOfResults==1){
							loc.setStatus(LocationProcessor.Location.STATUS_DISTINCT_IN_DATABASE);
							rs = this.dbCon.executeQuery(query, this);
							while (rs.next()) {
								queryIndex= "Query "+ j;
								
								if(loc.getValue("Bemerkungen")!= null)
									queryIndex+=" "+loc.getValue("Bemerkungen");
								...
								if (Constants.ENABLE_CHANGES_OF_MARCO) {
									if (loc.getStatus() == LocationProcessor.Location.STATUS_NOT_DISTINCT_IN_DATABASE) {
										returnValue = false;
										break;
									} else {
										returnValue = true;
		...
		if(loc.getValue("Bemerkungen")!= null)
			topQuery+=loc.getValue("Bemerkungen")+" "+topQuery;
	...
	public boolean isInRegion(Location loc) {
		...
		if (coord_y != null)
			coord_y = coord_y.replaceAll(",", ".");
		String query = "select Within (st_setsrid(st_makepoint(" + coord_x + "," + coord_y + "),4326),st_buffer((select the_geom from core.berlin_outline where id=1)," + searchRadius + "))";
		try {
