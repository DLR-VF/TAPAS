...
public class TPS_ParkAndRideRouter extends TPS_BasicConnectionClass {
	...
	protected Matrix readMatrix(String matrixName, String tableName) {
		Matrix m = null;
		String query= "SELECT matrix_values FROM " + tableName
	...
	public void loadTAZRestriction(String tazInfo, String table){
		...
		try{
			ResultSet rs = this.dbCon.executeQuery(query,this);
			while (rs.next()) {
				this.TAZInfo[(rs.getInt("ft_taz_id")-1)]= rs.getBoolean("is_restricted"); //the columns come ordered from the database!
			}
			rs.close();
		}
		catch(SQLException e){
	...
	public Matrix createPandRMatrix(){
		...
		System.out.println("Zeitdifferenz in Minuten\tAnzahl der Routen\tGesamtanzahl PnR-Routen:\t"+pnrRoutes+"\tGesamtanzahl Routen ohne PnR:\t"+classicRoutes+"\tVerkerszellen:"+this.TAZInfo.length);
	...
	public void storeMatrixInDB(Matrix matrix, String matrixName, String tableName){
		...
		//store into DB
		//delete old entry
		StringBuilder query = new StringBuilder("DELETE FROM " + tableName + " WHERE matrix_name = '" + matrixName +
				"'");
		...
		for (int j = 0; j < matrix.getNumberOfRows(); ++j) {
			...
			for (int k = 0; k < matrix.getNumberOfColums(); ++k) {
				if (k == matrix.getNumberOfColums() - 1 && j == matrix.getNumberOfRows() - 1)
					buffer.append(Math.round(matrix.getValue(j, k)));
				else
					buffer.append(Math.round(matrix.getValue(j, k))).append(",");
			}
			query.append(buffer);
		}
		query.append("}' )");
	...
	public void saveInterchangeMatrixInDB(String matrixName,String tableName){
		...
		for (int j = 0; j < this.interchangeMatrix.length; ++j) {
			...
			for (int k = 0; k < this.interchangeMatrix[j].length; ++k) {
				if (k == this.interchangeMatrix[j].length - 1 && j == this.interchangeMatrix.length - 1)
					buffer.append(Math.round(this.interchangeMatrix[j][k]));
				else
					buffer.append(Math.round(this.interchangeMatrix[j][k])).append(",");
			}
			query.append(buffer);
		}
		query.append("}' )");
	...
	public static void main(String[] args) {
		...
		worker.loadMatrices("CAR_1193TAZ_TT_MORNING", "PT_VISUM_1193_CALCULATED_AUG_2014_SUM_TT", "WALK_1193TAZ_TT", "core.berlin_matrices");
