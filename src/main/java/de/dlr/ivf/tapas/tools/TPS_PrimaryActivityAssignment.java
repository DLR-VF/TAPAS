        ...
        try {
            ...
            for (int counter = 0; counter < chunks; counter++) {
                query = "WITH p AS (SELECT p_id, p_hh_id, p_group, p_sex, p_age, p_age_stba, p_work_id, p_working, " +
                        ...
                        "h AS (SELECT hh_id, ST_X(hh_coordinate) AS lon, ST_Y(hh_coordinate) AS lat, hh_taz_id FROM " + hh_table + " WHERE hh_key = '" + key + "') " +
                ...
                while (rs.next()) {
                    ...
                    person.loc_code =
                            ((person.p_group == 3) || (person.p_group == 4) || (person.p_group == 5) || (person.p_group == 9) || (person.p_group == 10) || (person.p_group == 11) || (person.p_group == 12) || (person.p_group == 17) || (person.p_group == 18) || (person.p_group == 19) || (person.p_group == 20)) ? 1001000000 //Workers
                                    : ((person.p_group == 1) && (person.p_age <= 12)) ? 1002001001 //elementary school
                                    : ((person.p_group == 2) && ((person.p_education == 1) || (person.p_education == 2) || (person.p_education == 3))) || ((person.p_group == 2) && (person.p_working != 0)) ? 1002004001 //Vocational training - stimmt das??
            ...
            System.err.println(this.getClass().getCanonicalName() + " readPersonsTable: SQL-Error during statement: " + query);
            e.printStackTrace();
        }
    }
...
//                }
            ...
            System.err.println(this.getClass().getCanonicalName() + " readlocationsTable: SQL-Error during statement: " + query);
            e.printStackTrace();
        }
    }
    ...
    private int whichClass(int a) {
        return (a / this.distanceBinSize * this.distanceBinSize) + (this.distanceBinSize / 2); //integer effect! division always floors to the next number
    }
    ...
    private void completeWorkplaceMap() {
        ...
        for (Map.Entry<Integer, Map<Integer, List<Integer>>> workplaceMap_entry : workplaceMap.entrySet()) {
            for (int taz : taz_id_list) {
                List<Integer> taz_ids_list = workplaceMap_entry.getValue().get(matrix_with_categories[workplaceMap_entry.getKey() - 1][taz - 1]);
                if (taz_ids_list == null) {
                    taz_ids_list = new ArrayList<>();
                }
                taz_ids_list.add(taz);
                workplaceMap_entry.getValue().put(matrix_with_categories[workplaceMap_entry.getKey() - 1][taz - 1], taz_ids_list);
            }
        }
    }
    ...
    private TPS_DiscreteDistribution<Integer> probDistributionWorkplace() {
        ...
        for (int i = 0; i < distanceCategories.size(); i++) {
            workplaceValuesDist[i] = Math.exp(-beta1 * (workplaceKeys[i] / 1000)) - Math.exp(-beta2 * (workplaceKeys[i] / 1000));
        }
    ...
    public void createTable(String table) {
        ...
        try {
            ...
            query = "CREATE TABLE " + table + " (" +
                    ...
                    "p_has_bike boolean, " +
                    "p_education integer, " +
                    "p_professional integer, " +
                    "p_loc_id integer, " +
                    "CONSTRAINT berlin_persons_fix_location_pkey PRIMARY KEY(p_id,p_hh_id,p_key)); " +
                    "ALTER TABLE " + table + " OWNER TO tapas_admin_group; " +
                    "GRANT ALL ON TABLE " + table + " TO tapas_admin_group; " +
                    "GRANT ALL ON TABLE " + table + " TO tapas_user_group;";
    ...
    public void insertPersonsIntoTable(String table) {
        ...
        try {
            query = "INSERT INTO " + table + " (p_id, p_hh_id, p_group, p_sex, p_age, p_age_stba, p_work_id, p_working, p_abo, p_budget_pt," +
