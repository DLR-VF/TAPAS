...
public class RieselTool {
	...
	private BlockingQueue<AddressPojo> queue = new LinkedBlockingQueue<>();
	private UpdateAddressesWorker updateWorker;
	...
	private class Distributor {
		...
		private SortedMap<Double, LAND_USE> finalFactor;
		private EnumMap<LAND_USE, ArrayList<Long>> addresses;
		private Random randomGenerator;

		/**
		 * @param addresses
		 *            Empty lists may be omitted in the map.
		 */
		public Distributor(EnumMap<LAND_USE, ArrayList<Long>> addresses) {
			...
			for (LAND_USE lu : LAND_USE.values()) {
				if (addresses.get(lu).size() > 0) {
					factor.put(addresses.get(lu).size() * lu.getFactor()
							/ total, lu);
				} else {
					factor.put(0.0, lu);
				}
	 ...
	 * @return
	 * @throws SQLException
	 *             if the query fails which might happen if the database
	 *             connection fails.
	 */
	...
	public int testPerformance() throws SQLException {
		String query = "WITH geom AS ( "
				+ " SELECT ST_UNION(ST_INTERSECTION(ew.the_geom, fot.the_geom)) AS fot_geom "
				+ " FROM fot_landuse AS fot "
				+ " JOIN vg250_ew_f AS ew ON ST_INTERSECTS(ew.the_geom, fot.the_geom) "
				+ " WHERE fot.objart = 2111 AND ew.use=6 AND ew.gf=4 AND ew.rs='110000000000')"
				+ " SELECT id  FROM address_bkg AS ad "
				+ " JOIN geom ON ST_WITHIN(ad.the_geom,geom.fot_geom)";
	 ...
	 * 
	 * @throws SQLException
	 *             if the query is not successful. This might happen if the
	 *             database connection is not active.
	 */
	public EnumMap<LAND_USE, ArrayList<Long>> getBuildingsForArea(String rs_key)
			throws SQLException {
		...
		String query = "WITH geom AS ( SELECT ST_UNION(ST_INTERSECTION(ew.the_geom, fot.the_geom)) AS fot_geom "
				+ " FROM fot_landuse AS fot "
				+ " JOIN vg250_ew_f AS ew ON ST_INTERSECTS(ew.the_geom, fot.the_geom)"
				+ " WHERE fot.objart = _LU_ AND ew.use=6 AND ew.gf=4 AND ew.rs='"
				+ rs_key
				+ "') SELECT id FROM address_bkg AS ad"
				+ " JOIN geom ON ST_WITHIN(ad.the_geom,geom.fot_geom)";
	 ...
	 * 
	 * @param addresses
	 *            database ids of the buildings that are to be updated ordered
	 *            by their {@link LAND_USE}.
	 * @param ewz
	 *            Number of inhabitants to be distributed among the buildings.
	 */
	public void updateAddresses(EnumMap<LAND_USE, ArrayList<Long>> addresses,
			int ewz) {
	...
	public void updateAll() {
		...
		try {
			...
			System.out
					.println("List of all regions is created. Starting to update now.");
			System.out.println("I expect to update " + inhabitants.size()
					+ " regions.");
			...
			for (final EWZPojo ew : inhabitants) {
				...
				if (buildings != null) {
					...
					System.err.println("No buildings found in area " + ew.rs
							+ ". " + ew.ewz + " inhabitants not assigned.");
				}
