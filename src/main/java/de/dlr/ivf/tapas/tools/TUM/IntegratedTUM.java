 ...
 * 	<li>every single trip can be passed as argument to {@link GeneralAnalyzer#prepare(String, TapasTrip, de.dlr.ivf.tapas.util.parameters.TPS_ParameterClass)} for
 * 	evaluation.</li>
 ...
 * <ul>
 * 	<li>{@link PSQLException}: thrown if {@link IntegratedTUM#cancel(boolean)} is invoked during the trip table
 * 	creation statement and sometimes occurs during cancellation in a
 * 	{@link GeneralAnalyzer#prepare(String, TapasTrip, de.dlr.ivf.tapas.util.parameters.TPS_ParameterClass)} call.</li>
 * 	<li>{@link IllegalArgumentException}: <strong>manually</strong> thrown by {@link #getAnalyzer(Categories)} if no {@link AnalyzerBase} object that matched the {@link Categories} argument was found.</li>
 * </ul>
 * 
 * 
 * @author sche_ai
...
public abstract class IntegratedTUM extends SwingWorker<Void, String>{
	...
	/**
	 * 	mandatory information from {@link SimulationMonitor} passed by {@link #setInput(ITumInterface)}
	 */
	...
	protected Void doInBackground() throws Exception {	
		...
		for(int i = 0; i < simcount; i++){
			...
			this.firePropertyChange("intermediate", null, null); //reset progress bars in {@link SimulationMonitor} after one simulation is finished		
			...
			publish(time, MultilanguageSupport.getString("TUM_CONSOLE_PROCESS")+" "+(i+1)+" "+MultilanguageSupport.getString("TUM_CONSOLE_OF")+" "+simcount+": '"+simulations[i]+"'");
			publish(time, MultilanguageSupport.getString("TUM_CONSOLE_TRIPTABLE")+" ");
			
			tripReader = new DBTripReader(simulations[i],null, TPS_SettlementSystemType.FORDCP, null, this.connection);
			
			//handle trip table changes coming from {@link DBTripReader} 
			tripReader.addPropertyChangeListener(evt -> {if("triptable".equals(evt.getPropertyName()))
															...
															if((boolean)evt.getNewValue() && !isCancelled()){
																...
																if(!isCancelled())
																	publish(fail, MultilanguageSupport.getString("TUM_CONSOLE_TRIPTABLE_ERROR"));
			...
			try{
				if(tripReader.getIterator().hasNext() && this.triptablesuccess){
					...
					while (tripReader.getIterator().hasNext() && !isCancelled()){
						...
						if(!ga.prepare(simulations[i], tripReader.getIterator().next(), this.connection.getParameters()))
					...
					if(this.currenttripcnt == this.totaltrips){
						...
						if(!isCancelled()){
							publish(fail, MultilanguageSupport.getString("TUM_CONSOLE_TRIP_ERROR")+this.currenttripcnt+"... "+MultilanguageSupport.getString("TUM_CONSOLE_SKIPEXCE"),summary);
				...
				if(!isCancelled()){
					publish(time,MultilanguageSupport.getString("TUM_CONSOLE_EXCEL")+" "+exportfiles[i].getAbsolutePath()+"... ");

					if(ga.finish(exportfiles[i]))
						publish(success,summary);
					else{
						publish(fail, "Please check whether the Excel File you are trying to overwrite is currently NOT in use.",summary);
						if(i < simcount-1)
							publish(nextsim);
			...
			Thread.sleep(1000);	// due to concurrency it might happen while process() is running asynchronously on the EDT that trip counts are already reset
	...
	protected void process(List<String> chunks){
		...
		for(String s : chunks){
			try {
				switch (s){
				case success:
					this.console.insertString(this.console.getLength(), MultilanguageSupport.getString("TUM_CONSOLE_SUCCESS"),this.console.getStyle("green"));
					break;
				case fail:
					this.console.insertString(this.console.getLength(), MultilanguageSupport.getString("TUM_CONSOLE_FAIL"),this.console.getStyle("redbold"));
					break;
				case abort:
					this.console.insertString(this.console.getLength(), MultilanguageSupport.getString("TUM_CONSOLE_ABORT"),this.console.getStyle("orange"));
					break;
				case time:
				...
				case nextsim:
					...
					break;
				case summary:					
					this.console.insertString(this.console.getLength(), summary(), this.console.getStyle("summary"));
					break;
				case totalsummary:					
					this.console.insertString(this.console.getLength(), totalSummary(), this.console.getStyle("totalsummary"));
					break;
				case header:
					if(this.console.getLength()>0)
				...
				default:
					if(this.console.getText(this.console.getLength()-4, 4).equals("FAIL"))
						this.console.insertString(this.console.getLength(), " ("+s+")",this.console.getStyle("blackitalicsmall"));
					else
						this.console.insertString(this.console.getLength(),s,this.console.getStyle("standard"));
					break;
				}
	 ...
	 * @param c
	 * @exception
	 * 		{@link IllegalArgumentException} manually thrown in case there is no {@link AnalyzerBase} object corresponding to a {@link Categories}
	 * @return
	 * 		{@link AnalyzerBase} object belonging to {@link Categories} 
	 */		
	protected AnalyzerBase getAnalyzer(Categories c) {

		return	baseAnalyzerList.stream()
								.filter(analyzer -> analyzer.getCategories().equals(c))
								.findFirst()
	...
	private String summary(){
		...
		return "\n"+MultilanguageSupport.getString("TUM_CONSOLE_PARTEXECTIME")+" "
						+ String.format("%dm %ds", TimeUnit.NANOSECONDS.toMinutes(elapsedtime),
													 TimeUnit.NANOSECONDS.toSeconds(elapsedtime) - TimeUnit.MINUTES.toSeconds(TimeUnit.NANOSECONDS.toMinutes(elapsedtime)))
						+" | "+MultilanguageSupport.getString("TUM_CONSOLE_TRIPS")+" "
						+ this.currenttripcnt+"/"+ this.totaltrips+"\n";
	...
	private String totalSummary(){
		...
		return "\n"+MultilanguageSupport.getString("TUM_CONSOLE_TOTALEXECTIME")+" "
						+ String.format("%dm %ds", TimeUnit.NANOSECONDS.toMinutes(elapsedtime),
                								   TimeUnit.NANOSECONDS.toSeconds(elapsedtime) - TimeUnit.MINUTES.toSeconds(TimeUnit.NANOSECONDS.toMinutes(elapsedtime)));
