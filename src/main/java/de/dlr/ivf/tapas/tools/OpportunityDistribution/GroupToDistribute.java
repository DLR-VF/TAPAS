...
public class GroupToDistribute {
	...
	class ConversionUnitToCapacity{
		...
		public int getCapacity(LocationInDB loc, boolean workplace){
			if(	loc.getType()!=null && loc.getType().equals(this.type) &&
				loc.getUnit()!=null && loc.getUnit().equals(this.unit)){
	...
	public GroupToDistribute(TPS_DB_Connector dbCon, String region){		
		...
		try{
			...
			while(rs.next()){
				...
				this.workEnterpriseId= Math.max(this.workEnterpriseId, Integer.parseInt(rs.getString("loc_enterprise").substring(1)));
			}
			rs.close();
			//increment 
			...
			System.err.println(this.getClass().getCanonicalName()+" GroupToDistribute: SQL-Error during statement: "+query);
			e.printStackTrace();
		...
		try {
			...
			for(Entry<Integer, LocationInDB> entry: this.opportunityOutput.entrySet()){
				...
				String locationDescription=
						actID +";"+tmp.getCode()+";"+tmp.getTaz_id()+";"+tmp.getBlk_id()+";"+tmp.getCapacity()+";"+tmp.getCoordinate()[0]+";"+tmp.getCoordinate()[1] ;
		...
		try{
			...
			if(volume>0){ //something to do!
				...
				for(LocationInDB loc: this.opportunities){
					++i;
					if(i%100==0)
						System.out.println("Processed "+i+" form "+this.opportunities.size()+" DLM-objects for mapping id "+map_id);
			...
			System.err.println(this.getClass().getCanonicalName()+" fillLocationList: SQL-Error during statement: "+query);
			e.printStackTrace();
	...
	private BlockInDB findBlockAndAdressForDLMObject(int id) {
		try {
			...
			String query =
					"SELECT blk_id, blk_taz_id, st_X(blk_coordinate) AS x, st_Y(blk_coordinate) AS y FROM core."+region+"_blocks WHERE ST_WITHIN(ST_TRANSFORM(blk_coordinate, 31467),(SELECT the_geom FROM core.gis_dlm_fot WHERE gid="+(id-this.locID)+"))";
			ResultSet rs = dbCon.executeQuery(query, this);
			...
			while(adressNotFound && actBlocks.size()>0){
				...
				query = "SELECT st_X(the_geom) AS x, st_Y(the_geom) AS y FROM core."+region+"_buildings WHERE ST_WITHIN( the_geom,(SELECT the_geom FROM core."+region+"_blocks_multiline WHERE blocknr="+blkID+")) LIMIT 1";
				rs = dbCon.executeQuery(query, this);
				if(rs.next()){
	...
	public void processOpportunities(String opportunityDistribution){
		String query ="";
		try{
			query= "SELECT map_id,volume, loc_codes FROM core."+this.region+opportunityDistributionTableSuffix+" WHERE name='"+opportunityDistribution+"' ORDER BY level";
			ResultSet rs = dbCon.executeQuery(query, this);
			...
			for(Entry<Integer, VolumeDistribution> es:mappingValues.entrySet()){				
				fillLocationList(es.getValue().getMap_id(),es.getValue().getVolume(),es.getValue().getLocationCodes());
			}
			
		}catch(SQLException e){
			System.err.println(this.getClass().getCanonicalName()+" initOpportunities: SQL-Error during statement: "+query);
			e.printStackTrace();
		}		
	}		
	...
	public void adaptCapacities(){
		String query ="";
		try{
			query= "SELECT loc_id, loc_blk_id, loc_taz_id, loc_code, loc_type, loc_unit, loc_capacity, st_X(loc_coordinate) AS x, st_Y(loc_coordinate) AS y FROM core."+this.region+"_locations";
			ResultSet rs = dbCon.executeQuery(query, this);
			while(rs.next()){
