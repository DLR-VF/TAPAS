...
public class TPS_ECommerceEstimator extends TPS_BasicConnectionClass {
	...
	public void loadTAZ(String table){
		...
		for(MinimalLocation tazfrom: tazes){
			for(MinimalLocation tazto: tazes){
				double dist = TPS_Geometrics.getDistance(tazfrom.getCoordinate(), 
						tazto.getCoordinate(), this.getParameters().getDoubleValue(ParamValue.MIN_DIST));
	...
	public double calcDiffTT(ECommerceTrip trip, int mode) {
		return this.getCorrectedMatrixVal(trip.start, trip.end, this.tt[mode])
				-this.getCorrectedMatrixVal(trip.start, trip.middle, this.tt[trip.mode[0]])
				-this.getCorrectedMatrixVal(trip.middle, trip.end, this.tt[trip.mode[1]]);
	}

	public double calcDiffDist(ECommerceTrip trip) {
		return this.getCorrectedMatrixVal(trip.start, trip.end, this.dist)
				-this.getCorrectedMatrixVal(trip.start, trip.middle, this.dist)
				-this.getCorrectedMatrixVal(trip.middle, trip.end, this.dist);
	}
	...
	public void collectEComTrips(String table) {
		...
		try {
			...
			for(int i=0; i<maxChunk; i++) {
				System.out.print(".");
				query = "SELECT p_id, hh_id, start_time_min, loc_id_start, taz_id_start, lon_start, lat_start, loc_id_end, taz_id_end, lon_end, lat_end, mode, activity from "+table+" where hh_id%"+maxChunk+" = "+i+" order by hh_id, p_id, start_time_min ";
				rs = this.dbCon.executeQuery(query, this);
				while(rs.next()){
					p_id = rs.getInt("p_id");
					...
					if(p_id==currentPID && hh_id ==currentHHID) {
						if(lastWasShopping){
							...
							if(this.isECommerceTrip()) {
								trip.newMode = Math.max(trip.mode[0], trip.mode[1]); //fortunately we have a hierarchy here!
	...
	public void appendBZRInfo(String bzrTable, MinimalLocation loc){
		...
		try {
			...
			query = "SELECT bzr_id from "+bzrTable
					+" where st_within(st_transform(st_setsrid("
					+ 	"st_makepoint("+loc.loc.getValue(0)+", "+loc.loc.getValue(1)+"),4326),25833), the_geom)";
			rs = this.dbCon.executeQuery(query, this);
			if(rs.next()){
				...
				query = "SELECT bzr_id from "+bzrTable
						+" where st_within(st_transform(st_setsrid("
						+ 	"st_makepoint("+loc.loc.getValue(0)+", "+loc.loc.getValue(1)+"),4326),25833), st_buffer(the_geom,50))";
				rs = this.dbCon.executeQuery(query, this);
				if(rs.next()){
					String val = rs.getString("bzr_id");
					loc.bzrID = Integer.parseInt(val);
...
//		}
	...
	public void writeEComHouseholds(String filename){
		try {
			...
			for(MinimalLocation hh: this.ecomHH.values()){
				writer.append(hh.id +"\t"+ hh.tazID +"\t"+ hh.bzrID +"\t"+ hh.cappa +"\t"+ hh.loc.getValue(0) +"\t"+ hh.loc.getValue(1) +"\n");
			}
	...
	public void writeEComTripsToFile(String filename){
		try {
			...
			for(ECommerceTrip trip: this.eComTrips) {
				writer.append(trip.household.id +"\t"+ trip.household.tazID +"\t"+ trip.household.bzrID +"\t"+ trip.newMode +"\t"+ trip.savedDist +"\t"+ trip.savedTime +"\t"+
	...
	public void writeEComTripsToDB(String tablename){
		...
		try {
			...
			query = "CREATE TABLE "+tablename +
					...
					"  CONSTRAINT "+tableWithoutSchema+"_pkey PRIMARY KEY (gid)"+
					")" + 
					"WITH (" + 
					"  OIDS=FALSE" + 
					");";
			...
			query = "INSERT INTO "+tablename+" (" + 
					"            hh_id, taz, bzr, mode, dist_m, time_s, lon, lat)" + 
					"    VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
		...
		try {
			...
			for(ECommerceTrip trip: this.eComTrips) {
				if(trip.start.id == trip.end.id) { //delete both trips!
					...
					prepStUpdate.setDouble(pos++, this.getCorrectedMatrixVal(trip.start, trip.end, this.tt[trip.newMode])); //travel time sec
					prepStUpdate.setDouble(pos++, this.getCorrectedMatrixVal(trip.start, trip.end, this.dist)); //dist real
					prepStUpdate.setDouble(pos++, TPS_Geometrics.getDistance(trip.start, trip.end, this.getParameters().getDoubleValue(ParamValue.MIN_DIST))); //dist bl
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<sims.length; ++i) {
			...
			for(int j=0; j<8;++j) {
				System.out.format("Mode %d:  (num, dist in km , tt in h): \t%12d\t%12d\t%12d\n",j,worker.numShoping[j], (int)(worker.distShopping[j]/1000.0), (int)(worker.timeShopping[j]/3600));			
			}
			
			for(int j=0; j<8;++j) {
				System.out.format("Mode %d:  (num single, num multiple): \t%12d\t%12d\n",j,worker.numSingleTripShoping[j], worker.numMultiTripShoping[j]);			
			}
			...
			System.out.println("\nECom-trip savings");
			for(int j=0; j<8;++j) {
				System.out.format("Mode %d:  (num, dist in km , tt in h): \t%12d\t%12d\t%12d\n",j,numECom[j], (int)(distECom[j]/1000.0), (int)(timeECom[j]/3600));			
			}
		}
	}
}
