...
public class StreetnameProcessor {
	 ...
	 */
	private List<StreetTagEntry> streetTagList = new ArrayList<>();
	...
	private String replaceStreetTag(String input, StreetTagEntry tag){
		...
		if(tag!=null){
			...
			if(inputString.indexOf(tag.getOriginalTag())!=0){ //keine "Straße des 17. Juni" oder "Allee der Kosmonauten"
				//alles bis incl. tag ist Staßenname
				result=((inputString.substring(0, inputString.indexOf(tag.getOriginalTag()))+tag.getReplaceTag()).trim());
			}
		}
		return result; 
	}
	...
	public boolean processStreet(String input){
		...
		if(tag!=null){
			...
			if(inputString.indexOf(tag.getOriginalTag())==0) //oh eine "Straße des 17. Juni" oder "Allee der Kosmonauten"
				return false;
			//alles bis incl. tag ist Staßenname
			...
			if(inputString.indexOf(tag.getOriginalTag())+tag.getOriginalTag().length()+1<inputString.length() ){
				//this is the rest
				inputString = inputString.substring(inputString.indexOf(tag.getOriginalTag())+tag.getOriginalTag().length()+1).trim();
	...
	private String capStupidAmendments(String number){
		...
		if(number!=null && number.length()>0){
			...
			if(pos+1==number.length()|| pos==number.length())
				returnString= number;
			else
				returnString= number.substring(0,pos).trim();
		}
		return returnString;
	}
