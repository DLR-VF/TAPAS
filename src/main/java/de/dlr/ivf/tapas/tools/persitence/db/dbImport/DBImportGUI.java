...
public class DBImportGUI {
	...
	private void createAndShowGui() {
		...
		this.control.setIconImage(new ImageIcon(getClass().getClassLoader().getResource("icons/TAPAS-Logo.gif")).getImage());
		...
		btnStart.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (DEBUG)
						print("Successfully imported all tables");
				} catch (Exception ex) {
...
//					parameterClass.clear();
	...
	public void handleException(String job, Exception ex) {
		JOptionPane
				.showMessageDialog(this.control, "Catched " + ex.getClass().getSimpleName() + " during job:" + job
						+ " with message: " + ex.getMessage(), "Catched " + ex.getClass().getSimpleName(),
			...
			for (ParamString dbKey : DB_2_FILE_MAP.keySet()) {
				...
				for (ParamString fileKey : fileKeyList) {
					...
					if (this.parameterClass.isDefined(fileKey)) {
						...
						data[COL_F_NAME] = s;
						if (DBImportController.DB_IDENTIFIABLE_TABLES.contains(dbKey))
							data[COL_ID] = s.substring(s.lastIndexOf('/') + 1, s.lastIndexOf('.'));
						else
							data[COL_ID] = "";
