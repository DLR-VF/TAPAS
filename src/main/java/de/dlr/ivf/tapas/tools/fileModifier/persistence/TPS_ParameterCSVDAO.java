	...
	public void writeParameter(TPS_ParameterClass parameterClass) {
		...
		try {
			...
			for (ParamFlag pf : ParamFlag.values()) {
				if (parameterClass.isDefined(pf))
					pw.println(pf.name() + "," + parameterClass.isTrue(pf) + ",");
			}
			for (ParamString ps : ParamString.values()) {
				if (parameterClass.isDefined(ps))
					pw.println(ps.name() + "," + parameterClass.getString(ps) + ",");
			}
			for (ParamValue pv : ParamValue.values()) {
				if (parameterClass.isDefined(pv))
					pw.println(pv.name() + "," + parameterClass.getDoubleValue(pv) + ",");
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
