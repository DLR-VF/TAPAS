...
public class TAZCalculator {
	...
	private HashMap<Integer,Integer> getNewTAZ(String region, String table, String idColumn, String coordinateColumn, String sqlCondition){
		...
		try {
			...
			while(rs.next()){
				...
				count =0;
				try{
					query = "SELECT "+ idColumn +" FROM core."+region+table+" WHERE within("+coordinateColumn +",geometry('"+rs.getString(2)+"')) AND "+sqlCondition;
			...
			for(Entry<Integer,Integer> i :TAZRecalculated.entrySet()){
				if (i.getValue()<0){
					query = "SELECT gid FROM core."+region+"_taz_multiline ORDER BY distance_sphere(the_geom, (SELECT "+coordinateColumn+" FROM core."+region+table+" WHERE "+idColumn+" = "+i.getKey()+" AND "+sqlCondition+")) LIMIT 1";
					rs2 = st2.executeQuery(query);
					if(rs2.next()){
						taz = rs2.getInt("gid");
	...
	private boolean updateTAZID(String region, String table, String idColumn, String tazColumn, String sqlCondition, HashMap<Integer,Integer> map){
		...
		int counter;
		try{
			PreparedStatement st = this.manager.getConnection(this).prepareStatement("UPDATE core."+region+table+" SET "+tazColumn+" = ? WHERE "+idColumn+" = ? and "+sqlCondition);
			counter =0;
	...
	private boolean updateTAZ(String region, int bbrCathegory){
		...
		try {
			//update old TAZES
			query = "SELECT gid, text(st_centroid(the_geom)) FROM core."+region+"_taz_multiline WHERE gid IN (SELECT taz_id from core."+region+"_taz) ORDER BY gid";
			ResultSet rs = this.manager.executeQuery(query, this);
			while(rs.next()){
				query = "UPDATE core."+region+"_taz SET taz_coordinate = st_setsrid(geometry('"+rs.getString(2)+"'),4326), taz_bbr_type = "+bbrCathegory+" WHERE taz_id = "+rs.getInt("gid");
			...
			//insert new TAZES
			query = "SELECT gid, text(st_centroid(the_geom)) FROM core."+region+"_taz_multiline WHERE gid NOT IN (SELECT taz_id from core."+region+"_taz) ORDER BY gid";
			rs = this.manager.executeQuery(query, this);
			while(rs.next()){
				query = "INSERT INTO core."+region+"_taz (taz_id, taz_bbr_type, taz_coordinate) VALUES ("+rs.getInt("gid")+","+bbrCathegory+",st_setsrid(geometry('"+rs.getString(2)+"'),4326))";
				this.manager.execute(query, this);
			}
			rs.close();
			
			//delete old TAZES
			query = "DELETE FROM core."+region+"_taz WHERE taz_id NOT IN (SELECT gid FROM core."+region+"_taz_multiline)";
			this.manager.execute(query, this);
