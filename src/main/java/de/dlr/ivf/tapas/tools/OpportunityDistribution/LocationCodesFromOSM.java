	...
	public static void main(String[] args) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (local.length != 3) {
					System.err
							.println("In der folgenden Zeile konnten die Zahlen nicht korrekt geparst werden:");
				...
				if (!map.containsKey(locName))
					map.put(locName, new HashMap<>());
				if (!map.get(locName).containsKey(locTag))
					map.get(locName).put(locTag.replaceAll("\"", ""), new HashSet<>());
		...
		try {
			...
			// planet_osm_point
			String queryTemplate = "SELECT ARRAY['%TABLE%','%FIELD%'] AS source, osm_id, way AS the_geom, %VOL% AS volume,"
			...
			for (Entry<String, HashMap<String, HashSet<String>>> type : map.entrySet()) {
				for (Entry<String, HashSet<String>> field : type.getValue().entrySet()) {
					...
					String currentQuery = queryTemplate
							.replaceAll("%VOL%", "'-1'")
							.replaceAll("%TABLE%", "planet_osm_point")
							.replaceAll("%FIELD%", field.getKey())
							.replaceAll("%TYPE%", type.getKey())
							.concat(whereClause + " \nUNION\n")
							.concat(queryTemplate)
							.replaceAll("%VOL%", volume)
							.replaceAll("%TABLE%", "planet_osm_polygon")
							.replaceAll("%FIELD%", field.getKey())
							.replaceAll("%TYPE%", type.getKey())
							.concat(whereClause.toString());
		...
		System.out.println("Successfully written " + cnt + " queries to "
				+ file.getPath());
	}
