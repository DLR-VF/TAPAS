...
//		DB_2_FILE_MAP.put(DB_TABLE_HOUSEHOLD, getList(FILE_HOUSEHOLD_DATA));
        ...
        DB_HIERARCHY.add(getList(ParamString.DB_TABLE_CONSTANT_ACTIVITY, ParamString.DB_TABLE_CONSTANT_AGE,
                ParamString.DB_TABLE_CONSTANT_CARS, ParamString.DB_TABLE_CONSTANT_DISTANCE,
                ParamString.DB_TABLE_CONSTANT_DRIVING_LICENSE_INFORMATION, ParamString.DB_TABLE_CONSTANT_INCOME,
                ParamString.DB_TABLE_CONSTANT_HOUSEHOLD, ParamString.DB_TABLE_CONSTANT_LOCATION,
                ParamString.DB_TABLE_CONSTANT_MODE, ParamString.DB_TABLE_CONSTANT_SETTLEMENT,
                ParamString.DB_TABLE_CONSTANT_SEX, ParamString.DB_TABLE_CONSTANT_PERSON,
                ParamString.DB_TABLE_CONSTANT_ACTIVITY_2_LOCATION, ParamString.DB_TABLE_CFN4,
                ParamString.DB_TABLE_CFN4_IND, ParamString.DB_TABLE_CFNX));
    ...
    private final DBImportGUI control;
        ...
        Callable<Boolean> callable = new Callable<Boolean>() {
            public Boolean call() {
                ...
                try {
                    ...
                    db.execute(TPS_DB_IOManager
                            .buildQuery(control.getParameters(), "create_region_based_tables", control.getRegion(),
                                    control.getSchema()), this);
                    ...
                    for (int i = 0; i < DB_IMPORT_METHOD.size(); i++) {
                        ...
                        for (ParamString dbKey : dbKeyList) {
                            for (int j = 0; j < fileTable.getRowCount(); j++) {
                                ...
                                File tempFile = new File(control.getParameters().getString(ParamString.PATH_ABS_DB),
                                        control.getParameters().INPUT_DIR);
                                ...
                                switch (method) {
                                    case CSV:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_csvfile_to_table", fileName,
                                                        tableName), this);
                                        break;
                                    case EPS:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_episodes", fileName), this);
                                        break;
                                    case VOT:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_values_of_time", fileName,
                                                        name), this);
                                        break;
                                    case SCD:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_scheme_class_distributions",
                                                        fileName, name), this);
                                        break;
                                    case SCT:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_scheme_class_time_usage",
                                                        fileName), this);
                                        break;
                                    // case TAZ:
                                    ...
                                    case TIM:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_taz_intra_mit_infos",
                                                        fileName, name, tableName), this);
                                        break;
                                    case TIP:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_taz_intra_pt_infos",
                                                        fileName, name, tableName), this);
                                        break;
                                    case MAT:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_matrix", fileName, name,
                                                        tableName), this);
                                        break;
                                    case MCT:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_mode_choice_tree", fileName,
                                                        name), this);
                                        break;
                                    // case BLK:
                                    ...
                                    case PER:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_persons", fileName,
                                                        tableName, name), this);
                                        break;
                                    case HHD:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_households", fileName,
                                                        tableName, name), this);
                                        break;
                                    case FAT:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_taz_fees_tolls", fileName,
                                                        name, tableName), this);
                                        break;
                                    case SCO:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_scores", fileName, name,
                                                        tableName), this);
                                        break;
                                    case NPT:
                                        db.execute(TPS_DB_IOManager
                                                .buildQuery(control.getParameters(), "load_next_pt_stop", fileName,
                                                        name, tableName), this);
                                        break;
                                    default:
                                        //nothing
