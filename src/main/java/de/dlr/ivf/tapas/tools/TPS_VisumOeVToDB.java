...
public class TPS_VisumOeVToDB extends TPS_BasicConnectionClass {
	...
	public boolean loadTAZ(String table){
		...
		try{
			...
			query = "SELECT tapas_taz_id, no, st_X(st_centroid(st_transform(the_geom,4326))) as lat, st_Y(st_centroid(st_transform(the_geom,4326))) as lon, vbz_no, no, name FROM "+table+" order by tapas_taz_id";
	...
	public void readValuesOMFormat(String filename, PTNodeField field, int maxValue, int minValue){
		try {
			...
			while((line = fr.readLine())!=null){
				...
				if(	! tokens[indexFrom].matches("[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?") ||
					! tokens[indexTo].matches("[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?") ||	
					! tokens[indexVal].matches("[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?")) //not a number!
					continue;
				//get values
				...
				if(fromTAZ.ignore || toTAZ.ignore)
					continue;
	...
	public void calcBeelines(){
		...
		for(Entry<Integer, TAZ> i : this.TAZes.entrySet()){
			for(Entry<Integer, TAZ> j : this.TAZes.entrySet()){
				if(i.getValue().ignore || j.getValue().ignore)
					continue;
	...
	public void findMissingValues(){
		...
		for(int i= 0; i<this.nodes.length;++i){
			for(int j=0; j<this.nodes[i].length;++j){
				...
				if(this.nodes[i][j].act<0)
					missing = true;
				if(this.nodes[i][j].egt<0)
					missing = true;
				if(this.nodes[i][j].sumTT<0) {
					if(this.nodes[i][j].ivt<0)
						missing = true;
					if(this.nodes[i][j].twt<0)
						missing = true;
					if(this.nodes[i][j].bdh<0)
						missing = true;
				}
				if(this.nodes[i][j].nrt<=0)
					missing = true;
				if(missing)
					totalMissing++;
			}
	...
	public void calcTop3(double factor){
		...
		for(i=0; i<this.nodes.length; ++i){
			...
			for(j=0; j< this.nodes[i].length;++j){
				if(i!=j && this.nodes[i][j].act>=0)
					row.add(this.nodes[i][j].act);
			}
			this.nodes[i][i].act = this.getTop3FromRow(row , factor);
			//egress time
			row.clear();
			for(j=0; j< this.nodes[i].length;++j){
				if(i!=j && this.nodes[i][j].egt>=0)
					row.add(this.nodes[i][j].egt);
			}
			this.nodes[i][i].egt = this.getTop3FromRow(row , factor);
			//waiting time
			row.clear();
			for(j=0; j< this.nodes[i].length;++j){
				if(i!=j && this.nodes[i][j].twt>=0)
					row.add(this.nodes[i][j].twt);
			}
			this.nodes[i][i].twt =  this.getTop3FromRow(row , factor);
			//travel time
			row.clear();
			for(j=0; j< this.nodes[i].length;++j){
				if(i!=j && this.nodes[i][j].ivt>=0)
					row.add(this.nodes[i][j].ivt);
			}
			this.nodes[i][i].ivt =  this.getTop3FromRow(row , factor);
			//travel dist
			row.clear();
			for(j=0; j< this.nodes[i].length;++j){
				if(i!=j && this.nodes[i][j].jrd>=0)
					row.add(this.nodes[i][j].jrd);
			}
			...
			row.clear();
			for(j=0; j< this.nodes[i].length;++j){
				if(i!=j && this.nodes[i][j].sumTT>=0)
					row.add(this.nodes[i][j].sumTT);
			}
...
//		boolean[] fixPJT = {false,false,false,false};
		...
		for(int i=0; i< start.length;i++) {
			...
			if(usePercievedJourneyTime ) {
				...
				if(fixPJT[i])
					importer.removeACTandEGTfromPJT();
