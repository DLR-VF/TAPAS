...
public class TPS_PTStopWeightCalculator extends TPS_BasicConnectionClass {
	...
	public enum PTLineType {Train(0), S(1), U(2), Tram(3), BRT(4), Bus(5), LowPriBus(6), Unknown(7);
		public int code;
		PTLineType(int code) {
			this.code=code;
		}	
	...
	public void loadPTStops(String stopsTable, String tazTable, int bufferSize) {
		...
		try {
			...
			query = "with stops as (select node_number, st_setsrid(the_geom,25833) as the_geom, line_names from "+stopsTable+"),"
			+ "		taz as (select tapas_taz_id, vbz_no, st_buffer(the_geom,"+bufferSize+") as the_geom from "+tazTable+")"
			...
			while(rs.next()){
				...
				if(tmpLines == null)
					continue; // do not add stops with no lines
	...
	public void loadHouseholds(String table, String key) {
		...
		try {
			query = "select hh_id, hh_taz_id, hh_persons, st_x(st_transform(hh_coordinate,4326)) as lon, st_y(st_transform(hh_coordinate,4326)) as lat from "+table+" where hh_key = '"+key+"'";
			ResultSet rs = this.dbCon.executeQuery(query, this);
			while(rs.next()){
	...
	public void printPTStopsPerTAZ() {
		for(Entry<Integer, List<PTStop>> stops : this.taz2StopMap.entrySet()) {
			...
			for(PTStop stop : stops.getValue()) {
				...
				System.out.println("\tStop: "+stop.id+" number of lines: "+stop.lines.size()+" number of stops: "+ stopsCounter+" weight: "+stop.weight);
			}			
		}
	}
	...
	public void printPTStatisticsPerTAZ() {
		...
		for(Entry<Integer, List<PTStop>> stops : this.taz2StopMap.entrySet()) {
			...
			for(PTStop stop : stops.getValue()) {
				for(PTLine e: stop.lines.values()) {
					if(	e.type == PTLineType.S ||
						e.type == PTLineType.U ||
						e.type == PTLineType.Tram ||
						e.type == PTLineType.Train ||
						e.type == PTLineType.BRT
							) {
		...
		System.out.println(this.taz2StopMap.size()+"\t"+tazWithHighCapacityStops.size()+"\t"+tazWithSUTrain.size()+"\t"+tazWithTram.size()+"\t"+tazWithBRT.size());
	}
	...
	public static void main(String[] args) {
		...
		//for(i=0; i<301; i+=25) {
			...
			worker.savePTStopsPerTAZToFile("T:\\runs\\evoBus\\Kenngroessen\\Anbindungen\\anbindung_2_07_09_"+i+".csv", 1.4,10);
