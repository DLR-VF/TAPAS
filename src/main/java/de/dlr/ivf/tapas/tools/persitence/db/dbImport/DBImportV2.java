...
public class DBImportV2 {
	...
	public boolean importLocations(File file, String region, boolean adoptIDs){
		...
		try {
			...
			while(rs.next()){
				if(adoptIDs)
					idMap.put(rs.getInt("taz_num_id"), rs.getInt("taz_id"));
				else
					idMap.put(rs.getInt("taz_id"), rs.getInt("taz_id")); //1:1 mapping
			}			
			st.close();
			...
			//store into db
			query ="INSERT INTO core."+region+"_locations (" +
					"loc_id, loc_blk_id, loc_taz_id, " +
			...
			System.err.println("SQL-Error in importLocations for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (FileNotFoundException e) {
			System.err.println("File-Error in importLocations for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (IOException e) {
			System.err.println("IO-Error in importLocations for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		}
		return false;
	}
	...
	public boolean importTAZ(File file, String region, boolean useID){
		...
		try {
			...
			//store into db
			Connection con = this.dbCon.getConnection(this);
			query = "INSERT INTO core."+region+"_taz (taz_id, taz_coordinate, taz_num_id) VALUES (?, st_setsrid(st_makepoint(?,?), 4326),?)";
	...
	public boolean importTazFee(File file, String region, String name, boolean  useID, boolean updateBBR){
		...
		try {
			...
			pSt.close();
			query = "INSERT INTO core."+region+"_taz_fees_tolls (" +
					"ft_name, ft_taz_id, " +
					"has_toll_base, toll_type_base, " +
					"has_fee_base,fee_type_base, " +
					"has_toll_scen, toll_type_scen," +
					"has_fee_scen, fee_type_scen) " +
					"VALUES ('"+name+"',?,?,?,?,?,?,?,?,?)";
			pSt = con.prepareStatement(query);
	...
	public boolean importMatrix(File file, String region, String name){
		...
		try {
			...
			//store into db
			Connection con = this.dbCon.getConnection(this);
			query =	new StringBuilder("INSERT INTO core." + region + "_matrices (matrix_name, matrix_values) VALUES "
							+ "('"  + name + "', " + "ARRAY[");
	...
	public boolean importHH(File file, String region, String name, boolean adoptIDs){
		...
		try {
			...
			//store into db
			query = "INSERT INTO core."+region+"_households (hh_id, hh_persons, hh_cars, hh_income, hh_taz_id, hh_key, hh_coordinate) VALUES (?,?,?,?,?,'"+name+"',st_setsrid(st_makepoint(?,?), 4326))";
			PreparedStatement pSt = con.prepareStatement(query);
			int chunk=0;
			int chunksize = 128;
	...
	public boolean importPersons(File file, String region, String name) {
		...
		try {
			...
			query = "INSERT INTO core."
					+ region
					+ "_persons (p_key, p_work_id, " +
					"		p_id, p_hh_id, p_group, " +
					"		p_sex, p_age, p_age_stba, " +
					"		p_working, p_abo, p_budget_pt, p_budget_it, " +
					"		p_budget_it_fi, p_driver_license, p_has_bike" +
					") VALUES ('" + name + "', -1, ?, ?, ?, ?, ?,?,?,?,?,?,?,?,?)";
			PreparedStatement pSt = con.prepareStatement(query);
			query = "UPDATE core."+ region + "_households SET hh_type = ? WHERE hh_key = '" + name + "' AND hh_id = ?";
			PreparedStatement pSt2 = con.prepareStatement(query);
			query = "UPDATE core."+ region + "_households SET hh_has_child = TRUE WHERE hh_key = '" + name + "' AND hh_id = ?";
	...
	public boolean importAct2Loc(File file, String region) {
		...
		try {
			...
			System.err.println("File-Error in importAct2Loc for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (IOException e) {
	...
	public boolean importCFN4(File file, String region) {
		...
		try {
			...
			//store into db
			Connection con = this.dbCon.getConnection(this);
			query = "INSERT INTO core."+region+"_cfn4 (\"CURRENT_EPISODE_ACTIVITY_CODE_TAPAS\", \"CURRENT_TAZ_SETTLEMENT_CODE_TAPAS\", value, comment, raumtyp) VALUES (?,?,?,?,?)";
			PreparedStatement st = con.prepareStatement(query);
			for(CFN4 cfn4: cfn4s){
	...
	public boolean importCFN4Ind(File file, String region) {
		...
		try {
			...
			//store into db
			Connection con = this.dbCon.getConnection(this);
			query = "INSERT INTO core."+region+"_cfn4_ind (name, \"CURRENT_TAZ_SETTLEMENT_CODE_TAPAS\", value, comment, raumtyp) VALUES (?,?,?,?,?)";
			PreparedStatement st = con.prepareStatement(query);
			...
			System.err.println("File-Error in importCFN4Ind for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (IOException e) {
		...
		try {
			...
			while((line = in.readLine())!=null){
				...
				if(tok.length==3 || tok.length==4){
					...
					if(tok.length==4)
						tmp.hasIntraInfo=Boolean.parseBoolean(tok[3]);
			...
			//store into db
			query = "INSERT INTO core."+region+"_taz_intra_mit_infos (info_taz_id, beeline_factor_mit,average_speed_mit, info_name, has_intra_traffic_mit) VALUES " +
					"	(?, ?,?, '"+name+"',?)";
			PreparedStatement pSt = con.prepareStatement(query);
			...
			System.err.println("SQL-Error in importIntraMIT for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (FileNotFoundException e) {
			System.err.println("File-Error in importIntraMIT for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (IOException e) {
		...
		try {
			...
			while((line = in.readLine())!=null){
				...
				if(tok.length>=2 && tok.length<=4){
					...
					if(tok.length>=3)
						tmp.hasIntraInfo=Boolean.parseBoolean(tok[2]);
					if(tok.length==4)
						tmp.ptZone =Integer.parseInt(tok[3]);
			...
			//store into db
			query = "INSERT INTO core."+region+"_taz_intra_pt_infos (info_taz_id, average_speed_pt, info_name, has_intra_traffic_pt, pt_zone) VALUES " +
					"	(?, ?,'"+name+"',?,?)";
			PreparedStatement pSt = con.prepareStatement(query);
			...
			System.err.println("File-Error in importIntraPT for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (IOException e) {
	...
	public boolean importScores(File file, String region, String type, String name, boolean adoptIDs) {
		...
		try {
			...
			//store into db
			query = "INSERT INTO core."+region+"_"+type+"_scores VALUES " +
					"	(?, ?,?,'"+name+"')";
			PreparedStatement pSt = con.prepareStatement(query);
	...
	public boolean importBlock(File file, String region, boolean adoptIDs) {
		...
		try {
			...
			//store into db
			query = "INSERT INTO core."+region+"_blocks (blk_id, blk_taz_id, taz_coordinate) VALUES (?, ?, st_setsrid(st_makepoint(?,?), 4326))";
			PreparedStatement pSt = con.prepareStatement(query);
	...
	public boolean importBlockNextPTStop(File file, String region, String name) {
		...
		try {
			...
			//store into db
			Connection con = this.dbCon.getConnection(this);
			query = "INSERT INTO core."+region+"_block_next_pt_stop (next_pt_stop_blk_id, next_pt_stop, next_pt_stop_name) VALUES (?, ?, '"+name+"')";
			PreparedStatement pSt = con.prepareStatement(query);
	...
	public boolean importModeChoice(File file, String name) {
		...
		try {
			...
			for(ModeChoice tmp: mct){
				if(tmp.splitVariable==null){
					query = "INSERT into core.global_mode_choice_trees (name, node_id, parent_node_id, attribute_values, distribution) " +
						"	VALUES(" +
						"'"+name+"',"+tmp.node+","+tmp.parent+","+tmp.attribtes+","+tmp.distribution+")";
				}else{
					query = "INSERT into core.global_mode_choice_trees (name, node_id, parent_node_id, attribute_values, split_variable, distribution) " +
							"	VALUES(" +
							"'"+name+"',"+tmp.node+","+tmp.parent+","+tmp.attribtes+",'"+tmp.splitVariable+"', "+tmp.distribution+")";
				}
			...
			System.err.println("SQL-Error in importModeChoice for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (FileNotFoundException e) {
			System.err.println("File-Error in importModeChoice for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		} catch (IOException e) {
			System.err.println("IO-Error in importModeChoice for file "+file.getAbsolutePath()+" and query "+query);
			e.printStackTrace();
		}
		return false;
	}
