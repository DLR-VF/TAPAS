...
class DBImportTableModel extends DefaultTableModel {
		...
		public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
			...
			if ((ok == JFileChooser.APPROVE_OPTION) && fd.getSelectedFile() != null && fd.getSelectedFile().isFile()
				...
				if (path.startsWith(DBImportGUI.ABS_PATH)) {
					...
					JOptionPane.showMessageDialog(null, "Invalid File: " + path + "\nChoose a file from directory: "
							+ DBImportGUI.ABS_PATH, "Error", JOptionPane.ERROR_MESSAGE);
				}
	...
	private Set<Integer> errorSet = new TreeSet<>();
	...
	public void setValueAt(Object value, int row, int column) {
		...
		if (column == COL_T_KEY || column == COL_T_NAME)
			this.check(row);
		...
		if (row >= separator) {
			if (column == COL_T_KEY) {
				...
				if (this.parameterClass.isDefined(ps))
					this.setValueAt(this.parameterClass.getString(ps), row, COL_T_NAME);
				if (!DBImportController.DB_IDENTIFIABLE_TABLES.contains(ps))
					this.setValueAt("-----", row, COL_ID);
				else 
					this.setValueAt("", row, COL_ID);
				this.setValueAt("-----", row, COL_F_KEY);
			} else if (column == COL_T_NAME && isCellEditable(row, column)) {
				this.parameterClass.setString(ParamString.valueOf(this.getValueAt(row, COL_T_KEY).toString()), value.toString());
