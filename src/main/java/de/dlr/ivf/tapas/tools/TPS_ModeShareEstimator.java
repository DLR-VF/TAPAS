...
public class TPS_ModeShareEstimator extends ApplicationFrame {
    ...
    private void createChart() {
        chartReference = ChartFactory.createPieChart("Reference",  // chart title
                datasetReference,        // data
                false,           // include legend
                true, false);
        chartEstimation = ChartFactory.createPieChart("Estimation",  // chart title
                datasetEstimation,        // data
                false,           // include legend
                true, false);
        chartVolume = ChartFactory.createStackedBarChart("Traffic volume",  // chart title
        ...
        renderer3.setDefaultToolTipGenerator(
                new StandardCategoryToolTipGenerator("Mode {1}, Volume: {2} million km", NumberFormat.getInstance()));
        ...
        //turn on values
        ((PiePlot) chartReference.getPlot()).setLabelGenerator(new StandardPieSectionLabelGenerator("{0}: {2}"));
        ((PiePlot) chartEstimation.getPlot()).setLabelGenerator(new StandardPieSectionLabelGenerator("{0}: {2}"));
        renderer3.setDefaultItemLabelGenerator(new StandardCategoryItemLabelGenerator());
        renderer3.setDefaultPositiveItemLabelPosition(
    ...
    public void updateValue() {
        ...
        for (int i = 0; i < newShare.length; ++i) {
            ...
            } else costD = deltaCost.get(key) - refCost.get(key);

            newShare[i] = (double) datasetReference.getValue(i) * Math.exp(timeD * factT + +costD * factC);
    ...
    public static class PieRenderer {
        ...
        public void setColor(PiePlot plot, DefaultPieDataset dataset) {
            @SuppressWarnings("unchecked") List<Comparable> keys = dataset.getKeys();
            int aInt;
