...
public class TPS_CapacityAdjuster extends TPS_BasicConnectionClass {
	...
	public void calculateGenerationRates(){
		...
		try{
			...
			for(Integer i : schemeClasses){
				...
				query = "select count(*) as freq, act_code_zbe from core.global_episodes where home=FALSE and act_code_zbe!= 80 and scheme_id = any (SELECT scheme_id from core.global_schemes where scheme_class_id ="+i+") group by act_code_zbe";
				rs = dbCon.executeQuery(query, this);
				while(rs.next()){
	...
	public void loadDBEntries(){
		...
		try{
			...
			query = "select loc_code, sum(loc_capacity) as sum_cap from core."+locationTable+" group by loc_code order by loc_code";
			rs = dbCon.executeQuery(query, this);
			while(rs.next()){
				int locCode = rs.getInt("loc_code");
			...
			//person groups
			query = "select p_group, count(p_group) as count_p from core."+region+"_persons where p_key='"+this.keyVal+"' group by p_group order by p_group";
			rs = dbCon.executeQuery(query, this);
			while(rs.next()){
			...
			query = "select person_group, scheme_class_id, probability from core.global_scheme_class_distributions where name='"+scheme_class_distribution+"'";
			rs = dbCon.executeQuery(query, this);
			while(rs.next()){
						...
						if(sumOfLoc>0){
							...
							for(Integer locType : locForThisAct){
								if(this.locTypeCapDemanded.get(locType)!=null){
									double newVal = this.locTypeCapDemanded.get(locType)+ (this.locTypeCapGiven.get(locType)*demand);
	...
	public void printNewDemand(){
		for(Integer key: this.locTypeCapDemanded.keySet()){
			System.out.println("Loc type "+key+" stored cap: "+this.locTypeCapGiven.get(key)+" demanded cap: "+this.locTypeCapDemanded.get(key));
		}

		for(Integer key: this.locTypeCapDemanded.keySet()){
			if(this.locTypeCapGiven.get(key)>0){
				System.out.printf("Loc type %2d load factor: %5.3f\n", key, (this.locTypeCapDemanded.get(key) /this.locTypeCapGiven.get(key)));
			}
			else{
	...
	public void updateNewDemand(){
		...
		for(Integer key: this.locTypeCapDemanded.keySet()){
			if(this.locTypeCapDemanded.get(key)>0 && this.locTypeCapGiven.get(key)>0){
				...
				query = "UPDATE core."+locationTable+" set loc_capacity = "+capFactor+"*loc_capacity WHERE loc_code = "+key+" and "+capFactor+"*loc_capacity >= 1";
				this.dbCon.executeUpdate(query, this);
				query = "UPDATE core."+locationTable+" set loc_capacity = 1 WHERE loc_code = "+key+" and "+capFactor+"*loc_capacity < 1";
				this.dbCon.executeUpdate(query, this);
			}
		}
