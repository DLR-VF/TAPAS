...
public class TPS_LocationGroupCreator extends TPS_BasicConnectionClass {
	...
	public void groupLocations(String table){
		...
		try{
			...
			query = "SELECT loc_coordinate, array_agg(loc_id) as ids, count(loc_id) as num FROM "+table+" WHERE loc_group_id<0 GROUP BY loc_coordinate";				
			...
			System.err.println(this.getClass().getCanonicalName()+" groupLocations: SQL-Error during statement: "+query);
			e.printStackTrace();
		}		
	}
