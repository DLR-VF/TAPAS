...
public class TPS_FocusPointModel {
	...
	public TPS_Stay getFocusPoint(TPS_Stay from, TPS_Stay to, TPS_ActivityConstant code){
		if(STATUS){
			if(TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) { // TODO: what's this double-check stuff? Is STATUS needed?
				TPS_Logger.log(SeverenceLogLevel.DEBUG, "You want me to do the Fancy Chicken?");
			}
		}
				
		if(from.getActCode().equals(code)){ //if the activity at the last location is the same e.g. shopping than this is the focus point
			return from;
		}
