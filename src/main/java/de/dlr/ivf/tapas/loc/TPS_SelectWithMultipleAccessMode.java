...
public class TPS_SelectWithMultipleAccessMode extends TPS_SelectLocationWeigthBased {
	...
	public double getTravelTime(TPS_Plan plan, TPS_PlanningContext pc, TPS_ModeChoiceContext prevMCC, TPS_ModeChoiceContext nextMCC, TPS_TrafficAnalysisZone taz, TPS_ParameterClass parameterClass) {
		...
		ShoppingPreferenceAccessibility currentAccessibilityPreference = pc.pe.getPerson().currentAccessibilityPreference;
		if(currentAccessibilityPreference.equals(ShoppingPreferenceAccessibility.Naehe)) {
			weightedTT = Math.max(	parameterClass.getDoubleValue(ParamValue.MIN_DIST),
									TPS_Mode.get(ModeType.WALK).getDistance(pc.pe.getPerson().getHousehold().getLocation(), taz, SimulationType.SCENARIO,null));		
			connectionFound = true;
		}
		else if(currentAccessibilityPreference.equals(ShoppingPreferenceAccessibility.Erreichbarkeit)) {

			double ttCarFrom = TPS_Mode.get(ModeType.MIT).getTravelTime(prevMCC.fromStayLocation, prevMCC.toStayLocation, (int) prevMCC.fromStay.getOriginalEnd(),
							SimulationType.SCENARIO, prevMCC.fromStay.getActCode(), prevMCC.toStay.getActCode(), plan.getPerson(), 
							null);
			double ttCarTo   = TPS_Mode.get(ModeType.MIT).getTravelTime(nextMCC.fromStayLocation, nextMCC.toStayLocation, (int) nextMCC.fromStay.getOriginalEnd(),
								SimulationType.SCENARIO, nextMCC.fromStay.getActCode(), nextMCC.toStay.getActCode(), plan.getPerson(), 
								null);
			double ttPTFrom = TPS_Mode.get(ModeType.PT).getTravelTime(prevMCC.fromStayLocation, prevMCC.toStayLocation, (int) prevMCC.fromStay.getOriginalEnd(),
								SimulationType.SCENARIO, prevMCC.fromStay.getActCode(), prevMCC.toStay.getActCode(), plan.getPerson(), 
								null);
			double ttPTTo   = TPS_Mode.get(ModeType.PT).getTravelTime(nextMCC.fromStayLocation, nextMCC.toStayLocation, (int) nextMCC.fromStay.getOriginalEnd(),
								SimulationType.SCENARIO, nextMCC.fromStay.getActCode(), nextMCC.toStay.getActCode(), plan.getPerson(), 
								null);
			...
			double distanceNetTo   = Math.max(parameterClass.getDoubleValue(ParamValue.MIN_DIST), walkMode.getDistance(prevMCC.fromStayLocation, prevMCC.toStayLocation, SimulationType.SCENARIO,null));
			double distanceNetFrom = Math.max(parameterClass.getDoubleValue(ParamValue.MIN_DIST), walkMode.getDistance(nextMCC.fromStayLocation, nextMCC.toStayLocation, SimulationType.SCENARIO,null));
			...
			for (ModeType mt : TPS_Mode.MODE_TYPE_ARRAY) {
				...
				if (pc.carForThisPlan != null && pc.carForThisPlan.isRestricted() && taz.isRestricted() && mode.isType(ModeType.MIT)) {
					continue;
				}
	
				if(mt.equals(ModeType.WALK) && (distanceNetTo+distanceNetFrom)>parameterClass.getIntValue(ParamValue.MAX_WALK_DIST) ) {
					continue;
				}
				// get the traveltime
				plan.setAttributeValue(TPS_Attribute.CURRENT_MODE_CODE_VOT, mode.getCode(TPS_ModeCodeType.VOT));
	
				ttC0 = mode.getTravelTime(prevMCC.fromStayLocation, prevMCC.toStayLocation, (int) prevMCC.fromStay.getOriginalEnd(),
						SimulationType.SCENARIO, prevMCC.fromStay.getActCode(), prevMCC.toStay.getActCode(), plan.getPerson(), 
						null);
						//prevMCC.carForThisPlan);
				ttC1 = mode.getTravelTime(nextMCC.fromStayLocation, nextMCC.toStayLocation, (int) nextMCC.fromStay.getOriginalEnd(),
						SimulationType.SCENARIO, nextMCC.fromStay.getActCode(), nextMCC.toStay.getActCode(), plan.getPerson(), 
						null);
