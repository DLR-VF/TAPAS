...
public class TPS_Region implements ExtendedWritable, Iterable<TPS_TrafficAnalysisZone> {
	...
	private SortedMap<Integer, TPS_TrafficAnalysisZone> TAZ_Map;

	/// The map of the differentiated values of time
	private TPS_VariableMap valuesOfTime;

	// global locations storage
	private HashMap<Integer, TPS_Location> locations = new HashMap<>();
	...
	public void initLocationSelectModel(){
		try {
			String locationSelectModelClass =
					this.PM.getParameters().getString(ParamString.LOCATION_SELECT_MODEL_CLASS);
			locationSelectModelClass = locationSelectModelClass.substring(locationSelectModelClass.lastIndexOf('.') + 1);
	...
	public void initLocationChoiceSet(){
		try {
			...
			TPS_Logger.log(HierarchyLogLevel.APPLICATION, SeverenceLogLevel.FATAL, "Error in instantiating the " +
					"utility function: "+ExceptionUtils.getStackTrace(e));
		}
	}
	 ...
	 * 
	 * @param taz
	 *            the traffic analysis zone to add
	 * 
	 ...
	 * 
	 * @param plan
	 * 			the plan for holding the attributes
	 * @param costs
	 *            travel costs / general costs
	 * 
	...
	private double calculateVOTadds(double costs, TPS_Plan plan) {
		...
		if (TPS_Logger.isLogging(this.getClass(), SeverenceLogLevel.FINE)) {
			TPS_Logger.log(this.getClass(), SeverenceLogLevel.FINE, "Calculated vot: " + vot + " with request:" + this.getValuesOfTime().getLastRequest());
		}
	 ...
	 * 
	 * @param id
	 *            id of the traffic analysis zone asked for
	...
	private double manipulateCNF4ByPersongroup(TPS_ActivityConstant confSel, TPS_PersonGroup personGroup, double cfn4,
	 ...
	 * @param comingFromTVZ
	 *            traffic analysis zone of the prior stay
	 * @param goingToTVZ
	 *            traffic analysis zone of the subsequent stay
	 * @return travel time increase in seconds
	 */
	double relocateLocationMIVArr(TPS_Plan plan, double beelineDistance, TPS_ActivityConstant actCode, double durationStay,
                                  TPS_TrafficAnalysisZone comingFromTVZ, TPS_TrafficAnalysisZone goingToTVZ) {
		double costs = 0.0;
		...
		if (goingToTVZ.getSimulationTypeValues(SimulationType.SCENARIO).hasParkingFee()) {
			...
			if(TPS_Logger.isLogging(SeverenceLogLevel.FINE)) {
				TPS_Logger.log(SeverenceLogLevel.FINE, "GoingTo hast parkingfee of" + goingToTVZ.getSimulationTypeValues(SimulationType.SCENARIO).getFeeParking()
					+ "; duration is " + durationStay + "; parkingcosts: " + parkingCosts + "; sum resulting costs " + costs);
			}
		}
		if (costs > 0.0) {
			resultingTravelTimePlus = calculateVOTadds(costs, plan);
			if(TPS_Logger.isLogging(SeverenceLogLevel.FINER)) {
				TPS_Logger.log(SeverenceLogLevel.FINER, "taz (id=" + goingToTVZ.getTAZId() + ") with costs:" + costs + " and values: " + goingToTVZ.getSimulationTypeValues(SimulationType.SCENARIO));
			}
		}
	 ...
	 * @return travel time increase in seconds
	 */
	double relocateLocationMIVDep(TPS_Plan plan, double beelineDistance, TPS_ActivityConstant actCode, double durationStay,
                                  TPS_TrafficAnalysisZone comingFromTVZ, TPS_TrafficAnalysisZone goingToTVZ) {
		double costs = 0.0;
		double resultingTravelTimePlus = 0.0;
		...
		if (costs > 0.0) {
			resultingTravelTimePlus = calculateVOTadds(costs, plan);
			if(TPS_Logger.isLogging(SeverenceLogLevel.FINER)) {
				TPS_Logger.log(SeverenceLogLevel.FINER, "taz (id=" + goingToTVZ.getTAZId() + ") with costs:" + costs + " and values:"
	...
	TPS_Location selectDefaultLocation(TPS_Plan plan, TPS_PlanningContext pc, TPS_LocatedStay locatedStay) {
		...
		if (actCode.hasAttribute(TPS_ActivityConstantAttribute.DEFAULT)) {
			TPS_Logger.log(SeverenceLogLevel.ERROR, "Can't continue without locations for at least the default activity "
	...
	public TPS_Location selectLocation(TPS_Plan plan, TPS_PlanningContext pc, TPS_LocatedStay locatedStay) {
		...
		Collection<TPS_LocationConstant> connectedLocCodesToActCode =
				TPS_TrafficAnalysisZone.ACTIVITY2LOCATIONS_MAP.get(actCode);
		...
		if (connectedLocCodesToActCode == null || connectedLocCodesToActCode.isEmpty()) {
			TPS_Logger.log(SeverenceLogLevel.SEVERE, "No location codes for activity code " + actCode.getId() + " " +
					"(ZBE:" + actCode.getCode(TPS_ActivityCodeType.ZBE) + ") found");
			return this.selectDefaultLocation(plan, pc, locatedStay);
		}

		// get the location choice set
		TPS_RegionResultSet resultSet = this.getLocationChoiceSet().getLocationRepresentatives(plan, pc, locatedStay, this.PM.getParameters());
