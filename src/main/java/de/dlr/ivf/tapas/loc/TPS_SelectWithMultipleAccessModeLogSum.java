...
public class TPS_SelectWithMultipleAccessModeLogSum  extends TPS_SelectWithMultipleAccessMode{
	...
	public double getModeLogSum(TPS_Plan plan, TPS_PlanningContext pc, TPS_TrafficAnalysisZone taz, double distanceNet, double distanceNetInclReturn, TPS_ModeChoiceContext mcc, TPS_ParameterClass parameterClass, double mu) {
		...
		plan.setAttributeValue(TPS_Attribute.CURRENT_DISTANCE_CLASS_CODE_MCT ,TPS_Distance.getCode(TPS_DistanceCodeType.MCT, distanceNet));
		...
		for(int i=0; i< utilities.length; ++i) {
			...
			//calc the value
			utilities[i] = ((TPS_UtilityMNL)(TPS_Mode.getUtilityFunction(parameterClass))).getCostOfMode(mode, plan, distanceNet, travelTime, mcc, SimulationType.SCENARIO);
			...
			if(mode.isType(ModeType.BIKE) && distanceNetInclReturn>parameterClass.getIntValue(ParamValue.MAX_WALK_DIST) ) {
				continue;
			}
	...
	public Double getLogSum(TPS_Plan plan, TPS_PlanningContext pc, TPS_ModeChoiceContext prevMCC, TPS_ModeChoiceContext nextMCC, TPS_TrafficAnalysisZone taz, TPS_ParameterClass parameterClass, double mu) {
		...
		double distanceNetTo = Math.max(parameterClass.getDoubleValue(ParamValue.MIN_DIST), TPS_Mode.get(ModeType.WALK).getDistance(prevMCC.fromStayLocation, prevMCC.toStayLocation, SimulationType.SCENARIO,null));
		double distanceNetFrom = Math.max(parameterClass.getDoubleValue(ParamValue.MIN_DIST), TPS_Mode.get(ModeType.WALK).getDistance(nextMCC.fromStayLocation, nextMCC.toStayLocation, SimulationType.SCENARIO,null));

		arrModeLogSum = this.getModeLogSum(plan, pc, taz, distanceNetFrom, distanceNetFrom+distanceNetTo, prevMCC,parameterClass,mu);
		depModeLogSum = this.getModeLogSum(plan, pc, taz, distanceNetTo, distanceNetFrom+distanceNetTo, nextMCC,parameterClass,mu);
	...
	public TPS_Location selectLocationFromChoiceSet(TPS_RegionResultSet choiceSet, TPS_Plan plan, TPS_PlanningContext pc, TPS_LocatedStay locatedStay) {
		...
		if(!(TPS_Mode.getUtilityFunction(this.PM.getParameters()) instanceof TPS_UtilityMNL)){ // no LogSum for non MNL-Models!
		...
		double muScale=this.PM.getParameters().getDoubleValue(ParamValue.LOGSUM_WEIGHT_MU); // should be 1 if the logsums are in a normal range and not completely out of range
		double muInteract=this.PM.getParameters().getDoubleValue(ParamValue.LOGSUM_INTERACT_MU); // this should be estimated in the logit model
