...
public class TPS_SelectWithSingleAccessMode extends TPS_SelectLocationWeigthBased {
	...
	public double getTravelTime(TPS_Plan plan, TPS_PlanningContext pc, TPS_ModeChoiceContext prevMCC, TPS_ModeChoiceContext nextMCC, TPS_TrafficAnalysisZone taz, TPS_ParameterClass parameterClass) {
		...
		distanceNetFrom = Math.max(parameterClass.getDoubleValue(ParamValue.MIN_DIST), walkMode.getDistance(prevMCC.fromStayLocation, prevMCC.toStayLocation, SimulationType.SCENARIO,null));
		distanceNetTo = Math.max(parameterClass.getDoubleValue(ParamValue.MIN_DIST), walkMode.getDistance(nextMCC.fromStayLocation, nextMCC.toStayLocation, SimulationType.SCENARIO,null));
		...
		TPS_DiscreteDistribution<TPS_Mode> dist = new TPS_DiscreteDistribution<>(
