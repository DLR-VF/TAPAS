...
public class TPS_MultipleTAZRepresentant extends TPS_LocationChoiceSet {
	...
	public TPS_RegionResultSet getLocationRepresentatives(TPS_Plan plan, TPS_PlanningContext pc, TPS_LocatedStay locatedStay, TPS_ParameterClass parameterClass) {
		...
		for (; i <= parameterClass.getIntValue(ParamValue.MAX_TRIES_LOCATION_SELECTION) + 1
			...
			for (TPS_TrafficAnalysisZone taz : region) {
				...
				if(parameterClass.isTrue(ParamFlag.FLAG_FILTER_SHOPPING_CHOICE_SET) || !stay.isShopping()) {
					actDistance = TPS_Mode.get(ModeType.WALK).getDistance(taz, locGoingTo, SimulationType.SCENARIO, null);
					actDistance += TPS_Mode.get(ModeType.WALK).getDistance(locComingFrom, taz, SimulationType.SCENARIO,null);

					
					if (i <= parameterClass.getIntValue(ParamValue.MAX_TRIES_LOCATION_SELECTION)) {
						arrivalDistance = arrivalDuration * parameterClass.getDoubleValue(ParamValue.MAX_SYSTEM_SPEED)* incFactor * (double) i;
						departureDistance = departureDuration * parameterClass.getDoubleValue(ParamValue.MAX_SYSTEM_SPEED)* incFactor * (double) i;
					} else {
		...
		if(TPS_Logger.isLogging(SeverenceLogLevel.DEBUG)) {
			TPS_Logger.log(SeverenceLogLevel.DEBUG, "Selected traffic analysis zones (size=" + regionRS.size() + ") in " + (time / 1000000.0) + "ms");
		}
		return regionRS;
	}
