...
public class SimpleCollectorWriter implements Runnable {
    ...
    /**
     * @param outputFilePath Path to the file to written. This file will be overwritten.
     * @param queue          A queue of all entries to be written. This queue is supposed to be filled by another thread and is
     *                       expected to end with {@link AddressPojo#POISON_ELEMENT}.
     * @throws IOException
     */
    public SimpleCollectorWriter(String outputFilePath, BlockingQueue<AddressPojo> queue) throws IOException {
        super();
        this.queue = queue;
    ...
    public void run() {
        ...
        try {
            while ((ap = queue.take()) != AddressPojo.POISON_ELEMENT) {
                try {
                    writer.write(ap.getKey() + DELIMITER + ap.getInhabitants());
                    writer.newLine();
        ...
        System.out.println("I distributed " + inhabitants + " inhabitants to " + counter + " addresses.");
