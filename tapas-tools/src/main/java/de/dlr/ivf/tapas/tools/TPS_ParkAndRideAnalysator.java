...
//        this.dbCon.execute(query, this);
        ...
        for (Entry<Integer, Integer> e : this.interchangeCounter.entrySet()) {
            ...
            if (tmp != null) {
                query = "INSERT INTO " + tableName + " VALUES (	" + e.getKey() + "," + e.getValue() + "," +
                        tmp.maxParkingOccupancy() + "," + (tmp.peakTimeParking() / 60) + ")";
            } else {
                query = "INSERT INTO " + tableName + " VALUES (	" + e.getKey() + "," + e.getValue() + ",0" + ",0" +
                        ")";
