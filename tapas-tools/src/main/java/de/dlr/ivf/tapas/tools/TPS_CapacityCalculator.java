...
//        }
    ...
    public void printLocations() {
        for (TargetLocation e : locations) {
            if (e.loc_code > 0) System.out.println(
                    e.loc_id + ": " + e.loc_enterprise + " " + e.loc_type + " " + e.loc_capacity + " " + e.loc_code);
...
//        }
    ...
    public void updateLocCodesInBosserhofMapingValues() {
        ...
        for (Entry<String, Integer> e : codeMap.entrySet()) {
            ...
            if (cat.length == 3) {
                query = "UPDATE core.global_bosserhof_ws_mapping set loc_code=" + e.getValue() + " WHERE kategorie='" +
                        cat[0] + "' and " + "unterkategorie_1='" + cat[1] + "' and " + "unterkategorie_2='" + cat[2] +
                        "'";
            }
            if (cat.length == 2) {
                query = "UPDATE core.global_bosserhof_ws_mapping set loc_code=" + e.getValue() + " WHERE kategorie='" +
                        cat[0] + "' and " + "unterkategorie_1='" + cat[1] + "' and " + "unterkategorie_2=NULL";
            }
            if (cat.length == 1) {
                query = "UPDATE core.global_bosserhof_ws_mapping set loc_code=" + e.getValue() + " WHERE kategorie='" +
...
//            this.dbCon.executeUpdate(query, this);
    ...
    public void updateLocationCapacity(String tableName) {
        ...
        for (Entry<String, CapacityType> e : this.wzMap.entrySet()) {
            ...
            if (capaTyp.cathegory[0].equals("Arbeit")) continue;
...
//            this.dbCon.executeUpdate(query, this);
    ...
    class CapacityType {
        ...
        public void addCapacityInfo(CapacityInfo info) {
            this.infos.add(info);
            if (info.tapasCode == WORK) this.workFactor = info.max;
            else if (info.tapasCode != UNKNOWN) this.userFactor = info.max;
        }
