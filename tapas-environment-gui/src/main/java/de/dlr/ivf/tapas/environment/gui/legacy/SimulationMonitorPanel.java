...
public class SimulationMonitorPanel extends JPanel {
    ...
    private String[] getColumnNames() {
        MultilanguageSupport.init(SimulationMonitorOld.class);
        ...
        columnNames[SIM_INDEX.FINISHED.ordinal()] = MultilanguageSupport.getString("SIM_HEADER_FINISHED");
        columnNames[SIM_INDEX.PROGRESS.ordinal()] = MultilanguageSupport.getString("SIM_HEADER_PROGRESS");
        columnNames[SIM_INDEX.COUNT.ordinal()] = MultilanguageSupport.getString("SIM_HEADER_COUNT");
        columnNames[SIM_INDEX.ELAPSED.ordinal()] = MultilanguageSupport.getString("SIM_HEADER_ELAPSED");
        columnNames[SIM_INDEX.ESTIMATED.ordinal()] = MultilanguageSupport.getString("SIM_HEADER_ESTIMATED");
...
//        data[SIM_INDEX.FINISHED.ordinal()] = simulation.minimumState(TPS_SimulationState.FINISHED);
    ...
    private HashMap<String, Integer> getSimulations() {
        ...
        for (int i = 0; i < table.getRowCount(); ++i) {
            result.put((String) table.getModel().getValueAt(SIM_INDEX.KEY.ordinal(), i), i);
        }
        return result;
    }
...
//				new StringArrayRenderer(true), null);
        ...
        table.initTableColumn(SIM_INDEX.PROGRESS.ordinal(), 100, new ProgressRenderer(true), null);
        ...
        table.initTableColumn(SIM_INDEX.ELAPSED.ordinal(), 75, new AlignmentRenderer(JLabel.RIGHT, true), null);
    ...
    private void removeSimulation(String simkey) {
        for (int i = 0; i < table.getRowCount(); ++i) {
            if (table.getValueAt(i, SIM_INDEX.KEY.ordinal()).equals(simkey)) {
    ...
    private String textToHTML(String s) {
        return "<HTML>" + Arrays.toString(s.split("\n")).replace(", ", "<br />").replaceAll("[\\[\\]]", "") + "</HTML>";
    }
...
//								addWorkingKey(key);
//								SimulationMonitorOld.this.control
