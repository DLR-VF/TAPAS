...
import de.dlr.ivf.tapas.model.MatrixLegacy.MatrixPrint;
 ...
 * @author hein_mh
 */
public class MatrixMapLegacy {

    static int oneDay = 24 * 60 * 60;
    public MatrixTuple[] matrices = new MatrixTuple[0];

    public MatrixMapLegacy(double[] distribution, MatrixLegacy[] matrices) {
        if (distribution.length != matrices.length) {
            TPS_Logger.log(SeverityLogLevel.ERROR,
                    "Length of matrices and distribution differs: MatrixLegacy elements:" + matrices.length +
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < matrices.length; i++) {
            matrices[i] = new MatrixLegacy(2, 1);
            matrices[i].setValues(i);
        }

        MatrixMapLegacy map = new MatrixMapLegacy(distribution, matrices);
     ...
     *
     */
    public MatrixMapLegacy clone() {
        MatrixLegacy[] matrixClones = new MatrixLegacy[this.matrices.length];
        ...
        return new MatrixMapLegacy(distribution, matrixClones);
    }
     ...
     */
    public MatrixLegacy getMatrix(int time) {
    ...
    public void logStatistics() {
        for (MatrixTuple e : this.matrices) {
            TPS_Logger.log(SeverityLogLevel.INFO,
                    "MatrixLegacy values: Time: " + e.value + " average Time: " + e.matrix.getAverageValue(true, true));
        }
    }
    ...
    public class MatrixTuple implements Comparable<MatrixTuple> {
        public int value;
        public MatrixLegacy matrix;

        public MatrixTuple(int value, MatrixLegacy matrix) {
